...
class Chat:
    ...
    * [获取链接2](https://www.baidu.com/s?wd=%E8%8E%B7%E5%8F%96%20openai%20api_key)
    ...
    def dalle(self,
        ...
        if images_count and images_count != 1: kvs['n'] = images_count
        if quality and quality != 'standard': kvs['quality'] = quality
        return [b16decode(img.b64_json) for img in OpenAI(api_key=api_key, **self._kwargs).images.generate(**kvs).data]
    ...
    async def async_dalle(self,
        ...
        if images_count and images_count != 1: kvs['n'] = images_count
        if quality and quality != 'standard': kvs['quality'] = quality
        return [b16decode(img.b64_json) for img in (await AsyncOpenAI(api_key=api_key, **self._kwargs).images.generate(**kvs)).data]
