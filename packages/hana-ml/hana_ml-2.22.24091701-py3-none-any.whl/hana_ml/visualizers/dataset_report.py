...
class DatasetAnalyzer(object):
    def __init__(self, data: dataframe.DataFrame, key):
        ...
        variable_data = data
        if key:
            variable_data = data.deselect([key])
        self.variable_data = DatasetAnalyzer.rename_ID_column(variable_data)

        # step 2
        self.input_df: dataframe.DataFrame = data
        temp_df = VariableType.cast_variable_type(self.variable_data)
        self.variable_df: dataframe.DataFrame = temp_df
        self.variable_df_without_missing: dataframe.DataFrame = temp_df.dropna()
        self.variable_df_row_num = temp_df.count()

        # step 3
        self.variable_type: VariableType = None
    ...
    def add_variable_type(self):
        ...
        self.variable_type.init(self.variable_data)
    ...
    @staticmethod
    def rename_ID_column(data: dataframe.DataFrame):
        # When the name 'ID' already exists in the analyzed columns
        new_ID_column = 'ID'
        index = 1
        while True:
            if new_ID_column in data.columns:
                new_ID_column = 'ID_{}'.format(index)
                index = index + 1
            else:
                break
        if new_ID_column != 'ID':
            data = data.rename_columns({'ID': new_ID_column})
        return data
...
class HTMLBuilderUtils(object):
    ...
    def get_dataset_info(dataset_analyzer: DatasetAnalyzer):
        ...
        dataset_info.append(['Number of columns', len(dataset_analyzer.variable_df.columns)])
        dataset_info.append(['Number of numerical', len(dataset_analyzer.variable_type.numeric_variables)])
        dataset_info.append(['Number of categorical', len(dataset_analyzer.variable_type.categorical_variables)])
        dataset_info.append(['Number of datetime', len(dataset_analyzer.variable_type.datetime_variables)])
        dataset_info.append(['Number of other types', len(dataset_analyzer.variable_type.unknown_type_variables)])
...
class DatasetReportBuilder(object):
    ...
    def build(self, data: dataframe.DataFrame, key=None, scatter_matrix_sampling: Sampling = None,
        ...
        """
        pbar = tqdm(total=6, desc="Analyzing dataset...", file=sys.stdout, position=0, leave=False)
        ...
        self.dataset_analyzer.add_variable_type()
        pbar.update(1)
