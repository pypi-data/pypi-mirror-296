...
class ZGUploader(CloudUploader):
    """Copy file from one local directory to ZGCloudstorage.
    ...
    def __init__(self,
        ...
        self.index_file = os.getenv("ZEROG_INDEX_FILE")
    ...
    def upload_file(self, filename: str):
        """Copy file from one local path to ZGCloudStorage.

        Args:
            filename (str): Relative filepath to copy.
        """
        from zerog_python_api import upload   
        ...
        sk = os.getenv("ZEROG_TESTNET_SK")
        if sk is None:
            raise ValueError(f"To upload to zerog storage, you must specify your testnet sk using the shell command `export ZEROG_TESTNET_SK=0x...`")

        @retry(num_attempts=self.retry)
        def _upload_file():
            merkle_root, status = upload(
                ...
                while True:
                    try:
                        ...
                        if e.errno != errno.EAGAIN:
                            raise  # Re-raise if it's not a "file locked" error
    ...
    def list_objects(self, prefix: Optional[str] = None) -> List[str]:
        ...
        """
        try:
            with open(self.SHARED_LOG_FILE, 'r') as f:
                data = json.load(f)
