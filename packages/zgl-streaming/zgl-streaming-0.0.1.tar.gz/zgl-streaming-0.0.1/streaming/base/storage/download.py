...
#
# Modifications Copyright 2024 Zero Gravity Labs
...
def download_from_zg(remote: str, local:str) -> None:
    """Download a file from remote ZG storage to local.
    Hardcode for the demo, but will later require some configuration + a python API.
    Will overwrite local.
    Args:
        remote (str): Remote path (Alipan).
        local (str): Local path (local filesystem).
    """
    index_file = os.getenv("ZEROG_INDEX_FILE")
    if index_file is None:
        raise ValueError(f"To use zerog storage, you must specify the index file using the shell command `export ZEROG_INDEX_FILE=<file_name>`")
    if not os.path.exists(index_file):
        raise ValueError (f"Invalid index file: you specified {index_file}, which did not exist")
    async def _async_download(remote: str, local: str) -> None:
        try:
            with open(index_file, 'r') as f:
                index = json.load(f)
            
            if remote not in index:
                raise ValueError(f"Requested {remote}, not found in {index_file}")
            
            root = index[remote]
            
            # Run the CPU-bound download operation in a thread pool
            loop = asyncio.get_running_loop()
            await loop.run_in_executor(None, zerog_python_api.download, local, root)
            
        except json.JSONDecodeError:
            raise IOError(f"Error: Unable to parse {index_file}")
        except FileNotFoundError:
            raise IOError(f"Error: {index_file} not found")
        except Exception as e:
            raise IOError(f"Unexpected error: {str(e)}")

    try:
        # Run the async function in the asyncio event loop
        asyncio.run(_async_download(remote, local))
    except Exception as e:
        # Re-raise any exceptions to maintain the original error handling behavior
        raise
...
def download_file(remote: Optional[str], local: str, timeout: float):
    ...
    elif remote.startswith('0g:/'):
        download_from_zg(remote, local)
    else:
