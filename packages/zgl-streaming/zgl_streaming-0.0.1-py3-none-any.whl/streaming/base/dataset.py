...
class StreamingDataset(Array, IterableDataset):
    ...
    * How to iterate:
    ...
    def __init__(self,
        ...
        self._download_executor = ThreadPoolExecutor(max_workers=self.num_download_threads)
    ...
    def __del__(self) -> None:
        ...
        if hasattr(self, '_download_executor'):
    ...
    def _prepare_thread(self, it: _Iterator) -> None:
        ...
        while True:
            if it.should_exit() or it.prepare_index == it.total or self._event.is_set():
                break

            if self.predownload is None or self.predownload >= (it.prepare_index - it.yield_index):
                end_index = min(it.prepare_index + self.num_download_threads, it.total)
                shard_ids = [self.spanner[it.sample_ids[i]][0] for i in range(it.prepare_index, end_index) if it.sample_ids[i] != -1]
                
                futures = [self._download_executor.submit(download_shard, shard_id) for shard_id in shard_ids]
                wait(futures)

                it.prepare_index = end_index
            else:
                sleep(TICK)
