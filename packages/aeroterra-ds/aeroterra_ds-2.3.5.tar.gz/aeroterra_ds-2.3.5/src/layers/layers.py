...
def create_layer(gdf: pd.DataFrame,
    ...
    if len(gdf) == 0:
        raise Exception("Can't Be An Empty GDF, Must Provide Geometry")
    ...
    if len(empty_cols) == len(sdf.columns):
        sdf = create_fake_line()
    ...
    item = sdf.spatial.to_featurelayer(gis=gis, title=title, folder=folder, sanitize_columns=True)
    layer = item.layers[0]
    fields = get_fields_aux(layer)
    renames = {}
    offset = 1
    ...
    if one_columned:
        delete_field_aux(layer, aux_field)
...
def read_full_layer_aux_sdf(layer: FeatureLayer,
    ...
    while len(new_data) > 0:
        ...
        new_data = layer.query(where = "1=1", out_sr=out_crs, result_offset=len(total_data), return_all_records=False)
        new_data = parse_features_to_gdf(new_data)
...
def read_layer_gdf_aux(layer: FeatureLayer,
    ...
    try:
        ...
        while len(new_data) > 0:
            ...
            new_data = layer.query(where = query, geometry_filter=geo_filter, out_fields=out_fields, out_sr=out_crs, result_offset=len(total_data), return_all_records=False)
            new_data = parse_features_to_gdf(new_data)
