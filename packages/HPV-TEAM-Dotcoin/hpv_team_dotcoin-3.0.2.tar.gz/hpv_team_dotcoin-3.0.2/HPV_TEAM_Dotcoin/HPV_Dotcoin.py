...
class HPV_Dotcoin:
    ...
    [6] - `Получение кол-ва доступных игр и запуск их прохождения`
    
    [7] - `Просмотр рекламы и отыгрыш полученных игр`
    
    [8] - `Ожидание от 10 до 13 часов`
    
    [9] - `Повторение действий через 10-13 часов`
    '''
    ...
    def AutoSpin(self) -> None:
        '''Автоматический прокурт спина'''

        URL = 'https://api.dotcoin.bot/rest/v1/rpc/spin'
        HEADERS = {'User-Agent': self.USER_AGENT, 'x-client-info': 'postgrest-js/0.0.0-automated', 'sec-ch-ua': self.SEC_CH_UA, 'sec-ch-ua-mobile': self.SEC_CH_UA_MOBILE, 'authorization': f'Bearer {self.Token}', 'content-profile': 'public', 'apikey': 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Impqdm5tb3luY21jZXdudXlreWlkIiwicm9sZSI6ImFub24iLCJpYXQiOjE3MDg3MDE5ODIsImV4cCI6MjAyNDI3Nzk4Mn0.oZh_ECA6fA2NlwoUamf1TqF45lrMC0uIdJXvVitDbZ8', 'x-telegram-user-id': self.TG_ID, 'sec-ch-ua-platform': self.SEC_CH_UA_PLATFORM, 'origin': 'https://app.dotcoin.bot', 'x-requested-with': self.X_REQUESTED_WITH, 'sec-fetch-site': 'same-site', 'sec-fetch-mode': 'cors', 'sec-fetch-dest': 'empty', 'referer': 'https://app.dotcoin.bot/', 'accept-language': self.ACCEPT_LANGUAGE}

        self.Empty_Request('Spin') # Пустой запрос

        try:
            HPV = self.HPV_PRO.post(URL, headers=HEADERS, json={}, proxies=self.Proxy).json()

            if HPV['success']:
                self.Logging('Success', '🟢', f'Прокрут спина успешен! +{HPV["amount"]} {HPV["symbol"]}')
        except:
            pass
        ...
        while True:
            try:
                if self.Token: # Если аутентификация успешна
                    ...
                    # Рандомное выполнение действий: просмотр рефералов, выполнение заданий, апдейт бустов, получение бонуса, апдейт DTC майнера, прохождение игр, просмотр рекламы и прокурт спина
                    Autos = [self.AutoCheckRef, self.AutoRunTasks, lambda: self.AutoUpdateBoosts({'Click_LVL': Click_LVL, 'Limit_LVL': Limit_LVL}), self.AutoClaimBonus, self.AutoDTCUpdate, lambda: self.AutoPlay(Plays), self.AutoViewAds, self.AutoSpin]
