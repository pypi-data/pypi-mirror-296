...
def test_create_dai(dai_client):
    ...
    assert (
        engine.api_url
        == f"{external_scheme}://{external_host}/workspaces/create-dai/daiEngines/engine1"
    )
    assert (
        engine.login_url
        == f"{external_scheme}://{external_host}/workspaces/create-dai/daiEngines/engine1/oidc/login"
    )
