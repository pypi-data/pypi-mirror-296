...
def test_create(h2o_engine_client):
    ...
    assert (
        engine.api_url
        == f"{external_scheme}://{external_host}/workspaces/create-h2o-engine/h2oEngines/engine1"
    )
    assert (
        engine.login_url
        == f"{external_scheme}://{external_host}/workspaces/create-h2o-engine/h2oEngines/engine1/flow/index.html"
    )
...
def test_create_default_values(h2o_engine_client):
    ...
    assert (
        engine.api_url
        == f"{external_scheme}://{external_host}/workspaces/create-h2o-engine/h2oEngines/engine-default-values"
    )
    assert (
        engine.login_url
        == f"{external_scheme}://{external_host}/workspaces/create-h2o-engine/h2oEngines/engine-default-values/flow/index.html"
    )
    assert engine.creator.startswith("users/") and len(engine.creator) > len("users/")
    assert engine.creator_display_name == "test-user"
