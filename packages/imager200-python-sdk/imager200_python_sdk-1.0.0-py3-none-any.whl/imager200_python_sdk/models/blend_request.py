...
class BlendRequest(BaseModel):
    ...
    BlendRequest
    """ # noqa: E501
    alpha: Optional[Union[Annotated[float, Field(le=1, strict=True, ge=0)], Annotated[int, Field(le=1, strict=True, ge=0)]]] = None
    image1: StrictStr
    image2: StrictStr
    mode: Optional[StrictStr] = 'rts'
    target_format: Optional[StrictStr] = Field(default='jpeg', alias="targetFormat")
    __properties: ClassVar[List[str]] = ["alpha", "image1", "image2", "mode", "targetFormat"]
    ...
    def mode_validate_enum(cls, value):
        ...
        if value not in set(['rts', 'rtl']):
            raise ValueError("must be one of enum values ('rts', 'rtl')")
        return value

    @field_validator('target_format')
    def target_format_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['jpeg', 'png', 'bmp']):
            raise ValueError("must be one of enum values ('jpeg', 'png', 'bmp')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "mode": obj.get("mode") if obj.get("mode") is not None else 'rts',
            "targetFormat": obj.get("targetFormat") if obj.get("targetFormat") is not None else 'jpeg'
        })
        return _obj
