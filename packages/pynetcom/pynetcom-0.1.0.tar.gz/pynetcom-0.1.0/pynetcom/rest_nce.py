...
class RestNCE(object):
    """docstring for RestNCE"""
    data = list() # Лист который содержит результат каждого из запросов
    limit = "1000"
    is_trunked = False
    token = None
    ...
    def update_request_header(self):
        """Обновление заголовка запроса с токеном авторизации."""
        self.header = { "X-Auth-Token": self.token, "content-type":"application/json" }
    ...
    def auth(self):
        ...
        if response.status_code == 200:
            ...
            if response_json['exceptionId'] == 'user.user.policy_violation_stop':
                print('Check used status on NCE. May be it disabled')
            if response_json['exceptionId'] == 'user.pwd.expired':
                print('Check used status on NCE. User password is expiried')
            return False
        # Вытащить токен из полученных данных
        self.token = response_json ["accessSession"]
        self.write_token()
        self.update_request_header()
