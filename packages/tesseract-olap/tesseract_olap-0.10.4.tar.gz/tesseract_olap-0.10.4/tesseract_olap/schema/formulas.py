...
ISNULL, ISNOTNULL, TOTAL = (
    Keyword(token) for token in "ISNULL ISNOTNULL TOTAL".split()
)
unary_func = Group(
    (ISNULL | ISNOTNULL | TOTAL) + Suppress("(") + column + Suppress(")")
).setName("unary function")

operand = column | number | string | null_value | unary_func
