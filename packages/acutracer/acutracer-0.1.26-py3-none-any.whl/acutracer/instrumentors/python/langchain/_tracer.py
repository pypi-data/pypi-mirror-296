...
def audit_timing(wrapped: Any, _: Any, args: Any, kwargs: Any) -> Any:
    ...
    try:
        ...
        logger.info(f"{wrapped.__name__}: {latency_ms:.2f}ms")
...
class OpenInferenceTracer(BaseTracer):
    ...
    def __init__(self, tracer: trace_api.Tracer, *args: Any, **kwargs: Any) -> None:
        logger.info("ABHI in INIT OpenInferenceTracer")
        ...
        self._app_trace_id: Optional[str] = None
        self._propagator = TraceContextTextMapPropagator()
        # Store the token so we can detach it later
        self._context_tokens: Dict[UUID, Any] = _DictWithLock[UUID, Any]()

    def set_app_trace_id(self, trace_id: str) -> None:
        self._app_trace_id = "00-1234"

    def inject_trace_id(self, headers: Dict[str, str]) -> None:
        logger.info(f"\n in inject_trace_id with headers  {headers}")
        if self._app_trace_id:
            span_context = SpanContext(
                trace_id=int(self._app_trace_id, 16),
                span_id=trace_api.generate_span_id(),
                is_remote=True,
                trace_flags=TraceFlags(TraceFlags.SAMPLED),
            )
            context = trace_api.set_span_in_context(trace_api.NonRecordingSpan(span_context))
            self._propagator.inject(headers, context=context)
            logger.info(f"\n After inject_trace_id with headers {headers}")

    def get_current_trace_context(self):
    ...
    def inject_trace_context(self, headers: Dict[str, str]) -> None:
        ...
        self._propagator.inject(headers, context=context)
        logger.info(f"\n After inject_trace_context with headers {headers}")
    ...
    def _start_trace(self, run: Run) -> None:
        logger.info(f"ABHI STARTING TRACE for trace-id -> {run.id} parent run-id -> {run.parent_run_id}")
        import traceback
    ...
    def _end_trace(self, run: Run) -> None:
        ...
        if span:
            try:
                logger.info(f"\n Before _update_span for {run.name}, before update span {span}")
                _update_span(span, run)
                logger.info(f"\n After _update_span for {run.name}, after update span {span}")
            except Exception:
            ...
            logger.info(f"\n END SPANS --> {self._spans_by_run} \n")
        if token := self._context_tokens.pop(run.id, None):
...
def stop_on_exception(
    ...
    def wrapper(*args: Any, **kwargs: Any) -> Iterator[Tuple[str, Any]]:
        ...
        try:
            ...
            if _AUDIT_TIMING:
                latency_ms = (time.perf_counter() - start_time) * 1000
                logger.info(f"{wrapped.__name__}: {latency_ms:.3f}ms")
