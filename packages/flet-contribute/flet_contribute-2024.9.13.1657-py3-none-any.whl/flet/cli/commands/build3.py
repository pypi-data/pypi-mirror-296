...
class Command(BaseCommand):
    ...
    def handle(self, options: argparse.Namespace) -> None:
        ...
        # Extraer el contenido de app.zip en {flutter_dir}/app/app
        app_extract_path = self.flutter_dir.joinpath("app", "app")
        with zipfile.ZipFile(app_zip_path, 'r') as zip_ref:
            zip_ref.extractall(app_extract_path)

        print(f"Descomprimido {app_zip_path} en {app_extract_path}")
        print(f"Ubicación completa de la carpeta descomprimida: {app_extract_path.resolve()}")


        # Borrar el archivo app.zip y el archivo hash
        try:
            os.remove(app_zip_path)  # Borrar app.zip
            os.remove(app_hash_path)  # Borrar app.zip.hash
            print(f"Borrados {app_zip_path} y {app_hash_path}")
        except OSError as e:
            print(f"Error al intentar borrar archivos: {e}")

        
        # Listar el contenido de la carpeta app/app
        print("Listado de archivos y carpetas en la ubicación descomprimida:")
        for root, dirs, files in os.walk(app_extract_path):
            for dir_name in dirs:
                print(f"Directorio: {os.path.join(root, dir_name)}")
            for file_name in files:
                print(f"Archivo: {os.path.join(root, file_name)}")
