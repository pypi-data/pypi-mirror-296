        ...
        with open(outfile, 'w') as of:
            ...
            for record in records:
                ...
                if len(record.pass_list) > 0:
                    ...
                    if record.name == "NEG and NTC no quantity means checks":
                        neg_pass_list = [item for item in record.pass_list if "is empty for sample 'NEG" in item]
                        if len(neg_pass_list) > 0:
                            of.write(f"\n# The following {len(neg_pass_list)} NEG samples have no quantity:\n")
                            for item in neg_pass_list:
                                of.write(f"\t{item}\n")

                        ntc_pass_list = [item for item in record.pass_list if "is empty for sample 'NTC" in item]
                        if len(ntc_pass_list) > 0:
                            of.write(f"\n# The following {len(ntc_pass_list)} NTC samples have no quantity:\n")
                            for item in ntc_pass_list:
                                of.write(f"\t{item}\n")
                    else:
                ...
                if len(record.fail_list) > 0:
                    ...
                    if record.name == "NEG and NTC no quantity means checks":
                        neg_fail_list = [item for item in record.fail_list if "should be empty for sample 'NEG" in item]
                        if len(neg_fail_list) > 0:
                            of.write(f"\n# The following {len(neg_fail_list)} NEG samples have some quantity:\n")
                            for item in neg_fail_list:
                                of.write(f"\t{item}\n")

                        ntc_fail_list = [item for item in record.fail_list if "should be empty for sample 'NTC" in item]
                        if len(ntc_fail_list) > 0:
                            of.write(f"\n# The following {len(ntc_fail_list)} NTC samples have some quantity:\n")
                            for item in ntc_fail_list:
                                of.write(f"\t{item}\n")
                    else:
                        for item in record.pass_list:
                            of.write(f"\t{item}\n")
