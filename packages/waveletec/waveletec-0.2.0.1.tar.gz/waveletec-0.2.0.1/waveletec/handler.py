...
def main(sitename, inputpath, outputpath, datetimerange, acquisition_frequency=20, fileduration=30, 
    ...
    if method == 'cov':
        configure.output_path = str(os.path.join(outputpath, str(sitename)+'{}_{}.csv'))
    else:
    ...
    wavelet_functions.load_data_and_loop(**vars(configure), verbosity=5)
...
def __partition__(sitename, outputpath, variables_available=['u', 'v', 'w', 'ts', 'co2', 'h2o'], **kwargs):
    # RUN PARTITIONING
    dst_path = os.path.join(outputpath, str(sitename)+f'_CDWT_full_cospectra.csv')
