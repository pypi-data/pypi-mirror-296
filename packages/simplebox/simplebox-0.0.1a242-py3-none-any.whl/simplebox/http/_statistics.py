...
class _UrlMeta:
    ...
    def __eq__(self, other: '_UrlMeta'):
...
class _StatsSentUrl:
    ...
    def __init__(self):
        self.__urls_stats: dict[str, _StatsUrlHostView] = {}
    ...
    def add(self, *reqs: tuple[str, str]):
        for url, method in reqs:
            meta = _UrlMeta(url, method)
            if meta.host not in self.__urls_stats:
                self.__urls_stats[meta.host] = _StatsUrlHostView(meta.host)
            self.__urls_stats[meta.host].add(meta)

    @property
    def urls_stats(self) -> dict[str, _StatsUrlHostView]:
        return self.__urls_stats

    def get_url_stats_by_host(self, host) -> _StatsUrlHostView:
        return self.__urls_stats.get(host)


def _aggregation(*rests: Rest):
    stats_sent_urls: dict[str, _StatsUrlHostView] = {}

    for rest in rests:
        for k, v in rest.stats_datas.urls_stats.items():
            if k not in stats_sent_urls:
                stats_sent_urls[k] = _StatsUrlHostView(k)
