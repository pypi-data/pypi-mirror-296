...
class MachClient:
    ...
    def __init__(self, root_url: str = config.backend_url):
        ...
        for chain_name, chain_data in raw_deployments.items():
            ...
            # TODO: remove this when config is fixed
            if (gas_token_2 := next(gas_tokens, None)) != "ETH":
                self.gas_tokens[chain_id] = gas_token_2  # type: ignore
            # assert not (gas_token_2 := next(gas_tokens, None)), f"Multiple gas tokens on {chain_id}: {gas_token}, {gas_token_2}"
    ...
    def submit_order(self, chain: ChainId, place_taker_tx: str) -> dict:
        data = {
            "chain": str(chain),
