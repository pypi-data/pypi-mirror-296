...
class MachClient:
    ...
    def __init__(self, root_url: str = config.backend_url):
        ...
        for chain_name, chain_data in raw_deployments.items():
            ...
            assert not next(gas_tokens, None), f"Multiple gas tokens on {chain_id}"
    ...
    def get_token_balances(self, wallet_address: str) -> dict[ChainId, dict[str, int]]:
        ...
        return {
            ChainId(chain_name): value
            for chain_name, value in full_balances.items()
            if ChainId(chain_name) in self.chains
        }
