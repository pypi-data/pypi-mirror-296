...
class WebApp(SupportsWithKeyword):
    ...
    def __call__(self, *_) -> Callable:
        logger().error("'WebApp' object cannot be called directly. Maybe you forgot to export it?\n"
                       "Incase this is true, we'll export this WebApp automatically using export.standard.")
    ...
    def export_call_method(self, *_) -> Callable:
        logger().error("'WebApp.export' does not accept arguments.\n"
                       "Maybe you forgot to add an export method (standard, void, etc.)?\n"
                       "Incase this is true, export.standard will be used to prevent unwanted behaviour."
        )
    ...
    def export_getattr(self, attr: str, *_) -> Callable:
        logger().error(f"'{attr}' is not a valid export method.\n"
                       f"See the documentation for a list of valid export methods.\n"
                       "export.standard will be used to prevent unwanted behaviour."
        )
    ...
    def export_void(self, fn: Callable) -> Callable:
        return lambda *args, **kwargs: handle(fn)(*args, **kwargs)
