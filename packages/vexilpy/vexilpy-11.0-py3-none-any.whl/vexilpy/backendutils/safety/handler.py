...
def handle(fn: Callable) -> Callable:
    ...
    def wrapper(*args: Any, **kwargs: Any) -> Any:
        ...
        try:
            ...
            from vexilpy import verbose

            if verbose:
                logger().fatal(f"An error was caught but the handler is offline. Recovery process aborted.\n"
                               f"hint: this might be because the current running VexilPy instance is Verbose.")
                ...
                logger().error(f"VexilPy encountered an error `{type(err).__name__}` in function `{fn.__name__}`,\n"
                               f"but the system managed to recover.\n"
                               f"Use `python {" ".join(sys.argv)} --Verbose` to disable error recovery.")
