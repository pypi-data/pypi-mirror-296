...
JWT_ACCESS_KEY: Final[str] = env_get_str(key=f"{APP_PREFIX}_JWT_ACCESS_KEY")
JWT_SECRET_KEY: Final[str] = env_get_str(key=f"{APP_PREFIX}_JWT_SECRET_KEY")
...
def jwt_service() -> Response:
    ...
    a JSON containing:
    {
      "service-url": "<url>",                   - the JWT reference URL
      "access-key": JWT_ACCESS_KEY,             - a pre-defined access key
      "secret-key: JWT_SECRET_KEY               - a pre-defined secret key
    }
    ...
    if params and \
       params.get("access-key") == JWT_ACCESS_KEY and \
       params.get("secret-key") == JWT_SECRET_KEY:
