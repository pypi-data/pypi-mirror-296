...
def add_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        "-i", "--input-path", dest="path", help="Root path to search the *.json files in"
    )
    parser.add_argument("--log-level", choices=['DEBUG', 'INFO', 'WARNING', 'ERROR'],dest="log_level",help="Logger Level" ,default="WARNING")
    parser.add_argument("--git-commit", dest="git_commit",help="Git commit to put into the metadata")
    parser.add_argument("--git-branch", dest="git_branch",help="Git branch to put into the metadata")
    parser.add_argument("--is-merge-request", action=argparse.BooleanOptionalAction,dest="is_merge_request",help="Git branch to put into the metadata")
...
class Metadata:
    ...
    def dict(self) -> dict:
        return {"gitCommit": self.git_commit,
                "gitBranch": self.git_branch,
                "defaultGitBranch": self.default_git_branch,
                "isMergeRequest": self.is_merge_request}


UNKNOWN_METADATA = Metadata("unkown commit","unkown branch","unkown default branch",False) 
...
def get_metadata(args : Union[argparse.Namespace,None] ) -> Metadata:
    ...
    if set_from_cli:
        logging.debug("User specified command line arguments, so we dont autodetect the metadata")
        return metadata
    # gitlab Merge request 
    if "CI_MERGE_REQUEST_ID" in os.environ:
        metadata.git_commit = os.environ("CI_COMMIT_SHA",UNKNOWN_METADATA.git_commit)
        metadata.git_branch = os.environ("CI_MERGE_REQUEST_SOURCE_BRANCH_NAME",UNKNOWN_METADATA.git_branch)
        metadata.default_git_branch = os.environ("CI_DEFAULT_BRANCH",UNKNOWN_METADATA.default_git_branch)
        metadata.is_merge_request = True
        logging.debug("Auto detected Metadata from GITLAB Merge Request")
        return metadata
    
    # gitlab normal pipeline
    if "CI_COMMIT_SHA" in os.environ:
        metadata.git_commit = os.environ("CI_COMMIT_SHA",UNKNOWN_METADATA.git_commit)
        metadata.git_branch = os.environ("CI_COMMIT_BRANCH",UNKNOWN_METADATA.git_branch)
        metadata.is_merge_request = False
        logging.debug("Auto detected Metadata from GITLAB normal pipeline")
        return metadata

    # Github Pull request
    if "GITHUB_HEAD_REF" in os.environ:
        metadata.git_commit = os.environ("GITHUB_SHA",UNKNOWN_METADATA.git_commit)
        metadata.git_branch = os.environ("GITHUB_HEAD_REF",UNKNOWN_METADATA.git_branch)
        metadata.default_git_branch = "GitHub doenst expose the default branch via ENV"
        metadata.is_merge_request = True
        logging.debug("Auto detected Metadata from Github Pull Request")
        return metadata

     # Github Normal action
    if "GITHUB_REF_NAME" in os.environ:
        metadata.git_commit = os.environ("GITHUB_SHA",UNKNOWN_METADATA.git_commit)
        metadata.git_branch = os.environ("GITHUB_REF_NAME",UNKNOWN_METADATA.git_branch)
        metadata.default_git_branch = "GitHub doenst expose the default branch via ENV"
        metadata.is_merge_request = False  
        logging.debug("Auto detected Metadata from Github normal Action")
        return metadata

    logging.error("Couldnt determine the metadata. Please use the command line interface to specify them.")
    return metadata
