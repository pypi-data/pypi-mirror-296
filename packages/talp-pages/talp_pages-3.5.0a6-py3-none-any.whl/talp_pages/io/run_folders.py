...
from talp_pages.talp_common import (TALP_JSON_TIMESTAMP_KEY)
import logging 
import json
    ...
    for json_entry in run_folder.jsons:
        with open(json_entry) as file:
            ...
            try: 
                ...
                logging.critical(f"Reading the {TALP_JSON_TIMESTAMP_KEY} key failed in {json_entry}. Falling back to the OS file creating date")
            ...
            if timestamp.replace(tzinfo=None) > newest_json_timestamp.replace(tzinfo=None):
                newest_json = json_entry
                newest_json_timestamp = timestamp
    ...
    for root, _, files in os.walk(base_path):
        ...
        if json_files:
            ...
            if(str(relative_root) != "."):
                run_folder = RunFolder(relative_path=relative_root, jsons=json_files,latest_json=None)
