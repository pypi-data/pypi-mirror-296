...
class AbstractRuntimeFactory(Generic[S], ABC):  # pragma: no cover
    ...
    def with_app_event_bus(self, for_cli=False) -> Self:
        if self._settings is None:
            # to avoid mypy error settings can be None
            raise self._settings_missing_exception()
        middlewares = (
    ...
    # noinspection PyMethodMayBeStatic
    def _app_event_bus_middlewares(self) -> list[Type[MessageMiddleware]]:
    ...
    def _cli_app_event_bus_middlewares(self) -> list[Type[MessageMiddleware]]:
