...
# ajout L1test
from thonnycontrib.properties import L1TEST_EXCEPTION_SYMBOL, L1TEST_PROCEDURE_SYMBOL, L1TEST_SYMBOL1, L1TEST_SYMBOL2, L1TEST_SYMBOL3
...
class Example(ABC):
    ...
    def _execute_statement(self, source:str, globals:dict, is_want:bool):
        """
        ...
        Args:
            ...
            is_want (bool): Set to true if the `source` argument represents a <want> of
            this Example. Otherwise, set to False. This param is used to pass the line
            number of the statement through the exception in case the execution is failed.
        Raises:
        ...
        try:
            ...
            if has_error_lineno:
                ...
                if error_line == 1: 
                    lineno = self.lineno if not is_want else self.lineno + 1
    ...
    def _create_affectation_statement(self, statement1, statement2):
        ...
        The result will be : "statement1 = statement2"
...
class ExampleWithoutExpected(Example):
    ...
    def _exec_and_computes_verdict(self, globals:dict={}):
        try:  
            ...
            self._execute_statement(self.source, globals, False)
...
class ExampleWithExpected(Example):
    ...
    def _exec_and_computes_verdict(self, globals:dict={}):
        ...
        try:             
            ...
            self._check_syntax(self.str_comparaison(self.source, self.want), self.lineno, "single")
...
class ExampleProcedureExpected(Example):
    """This class represents an example that expects a procedure to be executed.
    It reads the stdout of the executed procedure and compares it with the expected
    """
    def is_a_test(self) -> bool:
        return True
    
    def _exec_and_computes_verdict(self, globals:dict={}):
        """Should read the stdout of the executed procedure and compares it with the expected.
        Use StringIO to store the stdout of the executed procedure.
        """
        executed, wanted = add_random_suffix("executed"), add_random_suffix("wanted")
        want = self.want.strip("\n")  
        ...
        try:             
            ...
            self._execute_statement(self.source, globals, False) # execute the procedure to get what is printed on stdout
            sys.stdout = sys.__stdout__
            
            self._execute_statement(self._create_affectation_statement(executed, '"%s"'%(temp_out.getvalue().rstrip())), globals, False)
            self._execute_statement(self._create_affectation_statement(wanted, self.want), globals, True)
...
class DocTestParser:
    ...
    _EXAMPLE_RE = re.compile(r'''
        ...
        (?P<source>
            (?:^(?P<indent> [ ]*) (?P<invite> %s|%s|%s|%s|%s) .*))    # PS1 line
        \n?
        # Want consists of zero or one non-blank lines that do not start with PS1.
        (?P<want> (?:(?![ ]*$)    # Not a blank line
                     (?![ ]*(%s|%s|%s|%s|%s))  # Not a line starting with PS1
                     .+$\n?       # But any other line
                  )?) # 0 ou 1 é
        '''%(L1TEST_SYMBOL1, L1TEST_SYMBOL2, L1TEST_SYMBOL3, L1TEST_EXCEPTION_SYMBOL, L1TEST_PROCEDURE_SYMBOL,   # source
             L1TEST_SYMBOL1, L1TEST_SYMBOL2, L1TEST_SYMBOL3, L1TEST_EXCEPTION_SYMBOL, L1TEST_PROCEDURE_SYMBOL),  # want
...
def ExampleFactory(invite:str, filename:str, source:str, want:str, exc_msg:str, lineno:int, indent, options) -> Example:
    ...
    procedure_invite = re.search(L1TEST_PROCEDURE_SYMBOL, invite)
    ...
    if exception_invite: 
        example = ExampleExceptionExpected(filename, source, want, invite, exc_msg,
                                            ...
                                            indent=indent,
                                            options=options)
    elif procedure_invite:
        example = ExampleProcedureExpected(filename, source, want, invite, exc_msg,
                                            lineno=lineno,
                                            indent=indent,
                                            options=options)
    else:
