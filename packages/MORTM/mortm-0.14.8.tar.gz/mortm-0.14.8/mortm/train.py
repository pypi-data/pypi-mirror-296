...
def _train(save_directory, ayato_dataset, message: Messenger, vocab_size: int, num_epochs: int, weight: Tensor, progress: LearningProgress, trans_layer=6,
           ...
           position_length=2048, accumulation_steps=4, batch_size=16, num_workers=0, warmup_steps=4000, is_use_mort_loss=False):
    ...
    if not is_use_mort_loss:
        criterion = nn.CrossEntropyLoss(ignore_index=0, weight=weight.to(progress.get_device())).to(progress.get_device())  # 損失関数を定義
    else:
        criterion = MORTCrossEntropyLoss(progress.get_device(),penalty=1, ignore_index=0,
                                         weight=weight.to(progress.get_device())).to(progress.get_device())

    optimizer = torch.optim.Adam(model.parameters(), lr=0.5, betas=(0.9, 0.98), weight_decay=0.01)  # オプティマイザを定義
