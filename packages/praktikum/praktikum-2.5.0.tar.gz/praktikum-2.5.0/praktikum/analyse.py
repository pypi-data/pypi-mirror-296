...
def fit_gedaempfte_schwingung(t, y, ey, plot_fit=False, einheit_x='s', symbol_y='y', einheit_y='[y]', fehler_skalierung=False, fig_name=None):
    ...
    amp_fit   = ufloat(popt[0], perr[0] * scaling_factor)
    T_fit     = ufloat(popt[1], perr[1] * scaling_factor)
    phase_fit = ufloat(popt[2], perr[2] * scaling_factor)
    delta_fit = ufloat(popt[3], perr[3] * scaling_factor)
    y_offset  = ufloat(popt[4], perr[4] * scaling_factor)
