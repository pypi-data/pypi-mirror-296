...
from DLMSAdapter.main import AdapterException
...
class InitType(ExTask):
    ...
    async def exchange(self, c: Client) -> bool:
        ...
        for adapter in get_adapters():
            try:
                c.objects = adapter.get_collection(m, f_id, f_ver)
                break
            except AdapterException as e:
                c.log(logL.WARN, F"{adapter}.get_collection error: {e}")
                continue
        else:
            c.log(logL.WARN, "not find collection by adapter")
