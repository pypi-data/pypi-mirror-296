...
# Set modern style for the visualization
rcParams.update({
    "font.size": 12,
...
# Grid size and initial simulation parameters
GRID_SIZE = 50
simulation_started = False
steps_to_simulate = 200  # Default number of steps

# Initialize the grid with dead cells (all 0s)
def initialize_empty_grid(size):
    """
    Initialize a grid of given size with all cells set to dead (0).

    Args:
        size (int): The size of the grid (size x size).
    
    Returns:
        np.array: A grid initialized with all dead cells.
    """
...
def add_glider(grid):
    ...
    Returns:
        np.array: The modified grid with the 'Glider' pattern added.
    """
...
def add_blinker(grid):
    ...
    Returns:
        np.array: The modified grid with the 'Blinker' pattern added.
    """
...
def add_still_life_block(grid):
    ...
    Returns:
        np.array: The modified grid with the still life block pattern added.
    """
...
def update_grid(grid):
    ...
    Args:
        grid (np.array): The current grid.

    Returns:
        np.array: The updated grid for the next step.
    """
    new_grid = grid.copy()
...
# Interactive grid modification
def on_click(event, grid, ax, fig):
    """
    Handle user clicks to toggle cell states between alive and dead.

    Args:
        event: Matplotlib click event.
        grid (np.array): The grid.
        ax: Axis for plotting.
        fig: The figure to update the display.
    """
    if event.inaxes == ax and not simulation_started:
        x, y = int(event.xdata), int(event.ydata)
        grid[y, x] = 1 - grid[y, x]  # Toggle between alive and dead
        ax.clear()
        ax.imshow(grid, cmap='Blues')  # Update color map for a modern look
...
# Update the number of steps for the simulation
def update_steps(text):
    """
    Update the number of steps for the simulation based on user input.

    Args:
        text (str): Input from the user for the number of steps.
    """
    global steps_to_simulate
    try:
        steps_to_simulate = int(text)
    except ValueError:
        steps_to_simulate = 200  # Default if input is invalid

# Animate the Game of Life
def animate(grid, steps=100):
    """
    Animate the Game of Life for a given number of steps.

    Args:
        grid (np.array): The initial grid.
        steps (int): The number of steps to animate.
    """
    fig, ax = plt.subplots(figsize=(10, 8))  # Match the size of the initial configuration

    def update(frame):
        nonlocal grid
        grid = update_grid(grid)
        ax.clear()
        ax.imshow(grid, cmap='Blues')  # Update color map for modern look
...
def setup_initial_configuration(grid):
    """
    Set up the grid configuration interactively by allowing the user to add initial patterns.

    Args:
        grid (np.array): The initial grid.

    Returns:
        np.array: The modified grid with initial setup.
    """
    fig = plt.figure(figsize=(10, 8))
    gs = gridspec.GridSpec(2, 1, height_ratios=[5, 1])
    ax = fig.add_subplot(gs[0])

    ax.imshow(grid, cmap='Blues')  # Update color map
    ax.grid(True, color='#d3d3d3', linestyle='--', linewidth=0.5)
    ax.set_xticks(np.arange(-0.5, GRID_SIZE, 1))
    ax.set_yticks(np.arange(-0.5, GRID_SIZE, 1))
    ax.set_xticklabels([])
    ...
    # Glider button
    glider_ax = plt.axes([0.05, 0.02, 0.15, 0.08])
    ...
    # Blinker button
    blinker_ax = plt.axes([0.25, 0.02, 0.15, 0.08])
    ...
    # Block button
    block_ax = plt.axes([0.45, 0.02, 0.15, 0.08])
    ...
    # Start button
    start_ax = plt.axes([0.75, 0.02, 0.15, 0.08])
    ...
    # Steps input box
    text_box_ax = plt.axes([0.65, 0.12, 0.1, 0.05])
    steps_box = TextBox(text_box_ax, 'Steps:', initial="200")
    steps_box.on_submit(update_steps)
...
def run_game():
    """
    Run the Game of Life simulation with user-defined or predefined patterns.
    """
    global steps_to_simulate
    grid = initialize_empty_grid(GRID_SIZE)
    grid = setup_initial_configuration(grid)

    if simulation_started:
        animate(grid, steps=steps_to_simulate)

# Main function
def main():
    """
    Main function to initialize and run the Game of Life.
    """
