...
cmd_len={len(cmd)}{Style.reset}'''
            ...
            if cmd.startswith('#hw1602ug#') and not cmd.endswith('#hw#'):
                print(f"{Fore.orange_red_1}An Imcomplete Scan Occurred, please {Fore.light_green}Try Again!{Style.reset}")
                msg=f'''
                {Fore.light_steel_blue}
                Generate the Barcodes for using Code128 as the Code Type
                MNUSAV - saves honeywell 1602ug settings
                MNUABT - discards honeywell 1602ug settings
                RESET_ - reset scanner
                SUFBK2 - add Suffix
                SUFCA2 - Clear All Suffixes
                SUFCL2 - Clear One Suffix
                PREBK2 - Add Prefix
                PRECA2 - Clear all Prefixes
                PRECL2 - Clear One Prefix
                {Fore.lgith_green}Use a Hex editor to convert alpha-numeric values to hex and use the below codes
                ...
                KEK - E
                KFK - F
                {Fore.light_red}{Style.bold}You WILL need the scanners programming chart until further notice and the hineywell set prefix/suffix is #hw#
                ...
                print(f)
                continue
            elif cmd.startswith('#hw#') and cmd.endswith('#hw#'):
                cmd=cmd.replace('#hw#','')



            cmd=honeywellfilter(cmd)
