...
def stray_field(
    ...
    """
    u1 = compute_internal(mag_func, mesh, prep["internal"], check_error=check_error, exac=exacu1)
    g = DL_boundary_two(u1, mesh, prep["DL"], check_error=check_error)
    u2 = compute_external(g, mesh, prep["external"], check_error=check_error)
...
def compute_internal(
    ...
    with TaskManager():
        ...
        gfu.vec.FV().NumPy()[:], exitCode = scipy.sparse.linalg.gmres(
            A, F, M=dict_int["precon_int"], rtol=1e-13
        )
    u = gfu.components[0]
...
def DL_boundary_two(
    ...
    with TaskManager():
        ...
        F = rhs.FV().NumPy()[:]
        g.vec.FV().NumPy()[:], exitCode = scipy.sparse.linalg.cg(A, F, rtol=1e-10)
        if check_error is True:
            print(f"exitCode = {exitCode}")
            print(
