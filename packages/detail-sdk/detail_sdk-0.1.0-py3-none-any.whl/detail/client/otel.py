...
class JsonLSpanExporter(SpanExporter):
	...
	def __init__(self,output_path):
		A=self;super().__init__();A.output_path=output_path;A.lock=FileLock(f"{A.output_path}.lock")
		if isfile(A.output_path):raise RuntimeError(f"{A.output_path} cannot be used as an export file since it already exists. Choose a new path or delete the existing file.")
	@DisableDetail()
	def export(self,spans):
