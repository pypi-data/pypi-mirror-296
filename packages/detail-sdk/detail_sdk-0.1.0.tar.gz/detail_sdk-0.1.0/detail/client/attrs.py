...
def build_attributes(library,library_attrs):
	A=library;B={f"{NS}.library":A,f"{NS}.context.thread_index":ThreadIndex.get(),f"{NS}.context.pid":os.getpid(),f"{NS}.context.ppid":os.getppid(),f"{NS}.context.wsgi_span_id":WsgiSpanId.get()or''}
