...
class CustomCloudLoggingHandler(CloudLoggingHandler):
    ...
    def emit(self, record):
        try:
            ...
            # 3. Construct a structured log entry
            log_entry = {
            ...
            # 4. Handle large log messages
            log_message = json.dumps(log_entry)
            if len(log_message.encode("utf-8")) > self.MAX_LOG_SIZE:
                # Upload the full message to GCS
                gcs_uri = self.upload_large_log_to_gcs(log_message, record.__dict__)
                # Truncate the message and include the GCS URI
                log_entry["message"] = self.truncate_log_message(log_message, gcs_uri)
                log_entry["additional_info"] = f"Full log at: {gcs_uri}"

            # 5. Assign the structured log entry to record.msg
            record.msg = log_entry
            record.args = ()

            # 6. Proceed with the standard CloudLoggingHandler emit
            super().emit(record)
        except Exception:
