...
def kernel_setup(t1, t2, T1_min, T1_max, T2_min, T2_max, DataType, N_T1, N_T2):
    """
    This returns the kernel matrices K1/K2 and vectors of N_T1/N_T2 log spaced 
    relaxation times or diffusion coefficients ranging from T1_min/T2_min to 
    T1_max/T2_max as T1/T2. If data type "custom" is used only T1/T2 is returned. 
    
    Parameters
    ----------
    t1/t2 : Logarithmically spaced time delays used for relaxation encoding or diffusion
    encodings of the type gamma^2 delta^2 (big_delta - delta / 3) g^2 (for reference:
    https://doi.org/10.1063/1.1673336 )
    
    T1_min/T2_min : Minimum relaxation time or diffusion coefficient to consider
    
    T1_max/T2_max : Maximum relaxation time or diffusion coefficient to consider
    
    DataType : Type of experiment. Options are 'T1T2sat' for  T1-T2 saturation 
    recovery, 'T1T2inv' for T1-T2 inversion recovery, 'T1Dsat' for T1-D encoded
    in the time dimension via saturation recovery and a Stejskal-Tanner like 
    encoding in the diffusion dimension (see above reference), 'T1Dinv' for 
    T1-D encoded in the time dimension via inversion recovery and a Stejskal-Tanner 
    like encoding in the diffusion dimension (see above reference), 'T2T2' for
    T2-T2 exchange with monoexponential encodings in both dimensions, 'T1T1' for 
    a T1-T1 exchange like experiment with a saturation recovery encoding in both
    dimensions, 'DD' for D-D exchange with Stejskal-Tanner like encodings in both
    dimensions and 'custom' if an user-defined custom kernel is provided. In general,
    the order of the encodings in the experimental data should follow the order 
    of the encodings in the strings. 
    
    N_T1/N_T2 : Number of points in the vector T1/T2

    """
    T1 = np.logspace(np.log10(T1_min), np.log10(T1_max), N_T1)
    T2 = np.logspace(np.log10(T2_min), np.log10(T2_max), N_T2)

    if DataType == 'T1Dsat':
        Kernel_1 = lambda Tau, TimeConst: 1 - np.exp(-Tau * (1 / TimeConst))
        Kernel_2 = lambda Tau, Diff: np.exp(-Tau * Diff)
        
    elif DataType == 'T1Dinv':
        Kernel_1 = lambda Tau, TimeConst: 1 - 2 * np.exp(-Tau * (1 / TimeConst))
        Kernel_2 = lambda Tau, Diff: np.exp(-Tau * Diff)
        
    elif DataType == 'DT2':
        Kernel_1 = lambda Tau, Diff: np.exp(-Tau * Diff)
        Kernel_2 = lambda Tau, TimeConst: np.exp(-Tau * (1 / TimeConst))
        
    elif DataType == 'T1T2sat':
        Kernel_1 = lambda Tau, TimeConst: 1 - np.exp(-Tau * (1 / TimeConst))
        Kernel_2 = lambda Tau, TimeConst: np.exp(-Tau * (1 / TimeConst))
        
    elif DataType == 'T1T2inv':
        Kernel_1 = lambda Tau, TimeConst: 1 - 2 * np.exp(-Tau * (1 / TimeConst))
        Kernel_2 = lambda Tau, TimeConst: np.exp(-Tau * (1 / TimeConst))
        ...
        Kernel_1 = lambda Tau, TimeConst: 1 - np.exp(-Tau * (1 / TimeConst))
        Kernel_2 = lambda Tau, TimeConst: 1 - np.exp(-Tau * (1 / TimeConst))
    ...
    K2 = Kernel_2(t2[:, None], T2)
        
    return K1, K2, T1, T2


def svd_kernel_compression(T1, T2, NormNoise, N_max, NormData, K1, K2):
    """
    Firstly, this function calculates the 2d kernel matrix, which is given as the
    Kronecker product of K1 and K2 (K1 and K2 being the 1d kernels):
        
        K = kron(K2, K1)
        
    Subsequently, this function uses singular value decomposition (SVD) to compress 
    the kernel matrix (K) as well as the experimental data (NormData). Using 
    SVD K1 can be rearranged to:
    ...
    Hence K1 can be rewritten:
        ...
        K = U_com @ S_com @ V_com.T  with _com indicating compression
    ...
    data can be compressed as follows:
       ...
       CompressedData =  U_com.T @ NormData = S @ V.T @ Dis
       
    In this case, it was used that U/U_com (and V/V_com) are unitary and real 
    ...
    Parameters
    ----------
    T1/T2 : Logarithmically spaced relaxation times or diffusion coefficient.
    ...
    U2, S2, V2 = svd(K2, full_matrices=False)
    V1 = V1.T
    V2 = V2.T
    S21 = np.outer(S1, S2).flatten('F')
    index = S21 > NormNoise
    S = S21[index]
    if len(S) > N_max**2:
        S = S[:N_max**2]
        index[(N_max**2):] = False
    n = len(S)
    i2 = np.ceil((np.where(index)[0] + 1) / len(S1)).astype(int) 
    i1 = np.mod((np.where(index)[0] + 1), len(S1))
    i1[i1 == 0] = len(S1)
    i2 = i2 - 1 #due to 0 ind in py
    i1 = i1 - 1
    U = np.zeros((len(NormData.flatten('F')), n))
    V = np.zeros((len(T1) * len(T2), n))
    for i in range(n):
        U[:, i] = np.squeeze(kron(np.expand_dims(U2[:, i2[i]], 1), 
                                  np.expand_dims(U1[:, i1[i]], 1)))
        V[:, i] = np.squeeze(kron(np.expand_dims(V2[:, i2[i]], 1), 
                                  np.expand_dims(V1[:, i1[i]], 1)))
...
def calc_d2_mat(N_T1, N_T2):
    ...
    Parameters
    ----------
    N_T1/N_T2 : Number of relaxation times or diffusion coefficients

    """
    D1 = np.diag(-1 * np.ones(N_T1), 0) + np.diag(np.ones(N_T1 - 1), 1)
    D1[-1, :] = 0
    D_h = kron(D1, np.eye(N_T2))
    D1 = np.diag(-1 * np.ones(N_T2), 0) + np.diag(np.ones(N_T2 - 1), 1)
    D1[-1, :] = 0
    D_v = kron(np.eye(N_T1), D1)
    D = np.vstack((D_v.T, D_h.T))
    G = np.block([
        [-D_v.T, np.zeros((N_T1 * N_T2, N_T1 * N_T2))],
        [-D_h.T, -D_v.T],
        [np.zeros((N_T1 * N_T2, N_T1 * N_T2)), -D_h.T]
        ])
    D2 = G @ D
...
def init_primal_dual(K, N_T1, N_T2):
    """
    Initializes variables for 2D primal dual algorithm.
    ...
    """
    Y1 = np.zeros((N_T1 * N_T2, 1))
    Y2 = np.zeros((3 * N_T1 * N_T2, 1))
...
def primal_dual_mtgv(CompressedData, Y1, Y2, F, W, F_tilde, W_tilde, K, B, D2, 
                     alpha, beta, tau, sigma, N_T1, N_T2):
    """
    2D primal dual algorithm for MTGV regularization. 
    For reference: https://doi.org/10.1016/j.jmr.2017.08.017 
    ...
    X = np.expand_dims(np.sqrt(np.sum(Y2_tilde.reshape(
        (N_T1 * N_T2, 3), order = 'F')**2, axis=1)), 1)
    Y2 = Y2_tilde / np.maximum(1, np.tile(X, (3, 1)) / beta)
    W_ = np.copy(W)
    W = W + tau * (Y1 - D2.T @ Y2)
    F_tilde = 2 * F - F_
    W_tilde = 2 * W - W_
...
def mtgv_gcv_brd_optim(NormNoise, n, N_max, beta_guess, N_T1, N_T2, K, 
                       alpha_guess, CompressedData, tol_alpha, tol, beta_lim):
    """
    ...
    i = 0
    D2 = calc_d2_mat(N_T1, N_T2)
    options = {'disp': False, 'maxiter': 100}

    while con_beta:
        Y1, Y2, F, W, F_tilde, W_tilde = init_primal_dual(K, N_T1, N_T2)
        con = True
        ...
        Dis[i] = F.reshape((N_T1, N_T2), order = 'F')
...
def mtgv_gcv_bp_optim(NormNoise, n, N_max, beta_guess, N_T1, N_T2, K, 
    ...
    F = F / np.sum(F)
    Dis = F.reshape((N_T1, N_T2), order = 'F')

    return Dis


def mtgv_gcv_fb_optim(NormNoise, n, N_max, beta, N_T1, N_T2, K, alpha_guess, 
...
def l1_gcv_optim(NormNoise, n, N_max, N_T1, N_T2, K, alpha_guess, 
                 CompressedData, tol_alpha, tol):
    """
    ...
    i = 0
    options = {'disp': False, 'maxiter': 100}
    Y1, _, F, _, F_tilde, _ = init_primal_dual(K, N_T1, N_T2)
    con = True
...
def l2_optim(N_max, CompressedData, K, alpha_guess, n, N_T1, N_T2, tol, tol_alpha, *args):
    """
...
def mtgv_gcv_brd(opt, *args):
    ...
    see the following example:
        ...
        data: Experimental data given as a numpy array of shape (len(t1), len(t2)) 
        
        #T1/#T2: Number of values in the to-be-estimated distribution in the 1st
        and 2nd dimension 
        
        noise: Noise estimate of your data. This should be a single number.

        T1/T2 bounds: The lower and upper limit of the to-be-estimated distribution. 

        t1/t2: Logarithmically spaced time delays used for relaxation encoding or 
        ...
        exp type: String which defines the type of kernel used. Options are 'T1T2sat' 
        for  T1-T2 saturation recovery, 'T1T2inv' for T1-T2 inversion recovery, 
        'T1Dsat' for T1-D encoded in the time dimension via saturation recovery 
        and a Stejskal-Tanner like encoding in the diffusion dimension (see above 
        reference), 'T1Dinv' for T1-D encoded in the time dimension via inversion 
        recovery and a Stejskal-Tanner like encoding in the diffusion dimension 
        (see above reference), 'T2T2' for T2-T2 exchange with monoexponential 
        encodings in both dimensions, 'T1T1' for a T1-T1 exchange like experiment
        with a saturation recovery encoding in both dimensions, 'DD' for D-D 
        exchange with Stejskal-Tanner like encodings in both dimensions and 
        'custom' if an user-defined custom kernel is provided. In general, the 
        order of the encodings in the experimental data should follow the order 
        of the encodings in the strings.
    ...
    *args : Additional arguments. Used to pass a custom kernel if DataType is
    ...
    T1/T2 : Logarithmically spaced relaxation times or diffusion coefficients.
    ...
    T2_min = np.copy(opt['T2 bounds'][0])
    T2_max = np.copy(opt['T2 bounds'][1])
    N_T1 = np.copy(opt['#T1'])
    N_T2 = np.copy(opt['#T2'])
    t1 = np.copy(opt['t1'])
    t2 = np.copy(opt['t2'])
    ...
    if DataType == 'custom':   
        T1, T2 = kernel_setup(t1, t2, T1_min, T1_max, T2_min, T2_max, 
                                    DataType, N_T1, N_T2)
        K1 = np.copy(args[0])
        K2 = np.copy(args[1])

    else:
        K1, K2, T1, T2 = kernel_setup(t1, t2, T1_min, T1_max, T2_min, T2_max, 
                                      DataType, N_T1, N_T2)
        
    NormData, NormNoise = ut.norm_noise_and_data(Data, NoiseStd)
    K, CompressedData, alpha_guess, n = svd_kernel_compression(T1, T2, NormNoise,
                                                               N_max, NormData, K1, K2)
    
    if pen_type == 'uniform':
        Dis, Score, Hy_Par = mtgv_gcv_brd_optim(NormNoise, n, N_max, beta_guess, N_T1,  N_T2, 
                                                K, alpha_guess, CompressedData, tol_alpha,
                                                tol, beta_lim)
    
    elif pen_type == 'gaussian':        
        F_l1 = l1_gcv_optim(NormNoise, n, N_max, N_T1, N_T2, K, alpha_guess, 
                            CompressedData, tol_alpha, tol)
        F_l1 = gaussian_filter(F_l1, sigma = 2.5)
        F_l1 = F_l1.flatten('F')
        F_l1 /= np.max(F_l1) 
        pen = np.diag(1 / (0.1 + F_l1)) 
        F_l2 = l2_optim(N_max, CompressedData, K, alpha_guess, n, N_T1, N_T2, tol, 
                        tol_alpha, pen)
        Dis, Score, Hy_Par = mtgv_gcv_brd_optim(NormNoise, n, N_max, beta_guess, N_T1, N_T2,  
                                                K, alpha_guess, CompressedData, tol_alpha,
                                                tol, beta_lim)
...
def mtgv_gcv_bp(opt, *args):
    ...
    *args : Additional arguments. Used to pass a custom kernel if DataType is
    ...
    if pen_type == 'uniform':       
        Dis = mtgv_gcv_bp_optim(NormNoise, n, N_max, beta_guess, N_T1, N_T2, K, 
                                alpha_guess, CompressedData, tol_alpha, tol, gamma)
...
def mtgv_gcv_fb(opt, *args):
    ...
    *args : Additional arguments. Used to pass a custom kernel if DataType is
    ...
    K, CompressedData, alpha_guess, n = svd_kernel_compression(T1, T2, NormNoise,
                                                               N_max, NormData, K1, K2)
    _, Score, Hy_Par = mtgv_gcv_brd_optim(NormNoise, n, N_max, beta_guess, N_T1, N_T2, 
    ...
    if pen_type == 'uniform':
        Dis = mtgv_gcv_fb_optim(NormNoise, n, N_max, beta, N_T1, N_T2, K, 
...
def l2_gcv(opt, *args):
    ...
    *args : Additional arguments. Used to pass a custom kernel if DataType is
    ...
    if pen_type == 'uniform':
        Dis = l2_optim(N_max, CompressedData, K, alpha_guess, n, N_T1, N_T2, tol, tol_alpha)
        ...
        F_l1 /= np.max(F_l1) 
        pen = np.diag(1 / (0.1 + F_l1)) 
        Dis = l2_optim(N_max, CompressedData, K, alpha_guess, n, N_T1, N_T2, tol, 
                       tol_alpha, pen)
...
def l1_gcv(opt, *args):
    ...
    *args : Additional arguments. Used to pass a custom kernel if DataType is
    ...
    K, CompressedData, alpha_guess, n = svd_kernel_compression(T1, T2, NormNoise,
                                                               N_max, NormData, K1, K2)
    Dis = l1_gcv_optim(NormNoise, n, N_max, N_T1, N_T2, K, alpha_guess, 
                       CompressedData, tol_alpha, tol)
...
def simulate(opt, *args):
    ...
    see the following example:
        ...
        #T1/#T2: Number of relaxation times or diffusion coefficients in the simulated
        distribution
        
        #t1/#t2: Number of time delays used for relaxation encoding or diffusion 
        ...
        log mean T1/T2: log10(T1/T2) mean for every relaxation or diffusion component
        
        log std T1/T2: log10(T1/T2) standard deviation for every relaxation or 
        diffusion component
    ...
    T1/T2: Logarithmically spaced relaxation times or diffusion coefficient in the
    range of T1 bounds
    ...
    log_mean_T2 = np.copy(opt['log mean T2'])
    log_std_T1 = np.copy(opt['log std T1'])
    log_std_T2 = np.copy(opt['log std T2'])
    snr = np.copy(opt['SNR'])
    del opt
    
    if DataType == 'T1Dsat' or DataType == 'T1Dinv':
        t1 = np.logspace(np.log10(T1_min), np.log10(T1_max), N_t1)
        t2 = np.logspace(np.log10(1 / T2_max), np.log10(1 / T2_min), N_t2)
        
    elif DataType == 'DT2':
        t1 = np.logspace(np.log10(1 / T1_max), np.log10(1 / T1_min), N_t1)
        t2 = np.logspace(np.log10(T2_min), np.log10(T2_max), N_t2)
        
    elif (DataType == 'T1T2sat' or DataType == 'T1T2inv' or 
          DataType == 'T2T2' or DataType == 'T1T1'):
        t1 = np.logspace(np.log10(T1_min), np.log10(T1_max), N_t1)
        t2 = np.logspace(np.log10(T2_min), np.log10(T2_max), N_t2)
    ...
    for k in range(N_comp):
        ...
        pdf_T2 = norm.pdf(np.log10(T2), log_mean_T2[k], log_std_T2[k])
        
        if k == 0:
            Dis = w[k] * np.outer(pdf_T1, pdf_T2)
        else:
            Dis += w[k] * np.outer(pdf_T1, pdf_T2)
    ...
    Sig = Sig + NoiseStd * np.random.randn(N_t1 * N_t2)
    NoiseStd = NoiseStd / np.max(np.abs(Sig))
    Sig = Sig / np.max(np.abs(Sig))
    Sig = Sig.reshape((N_t1, N_t2), order = 'F')

    return Sig, Dis, T1, T2, t1, t2, NoiseStd
