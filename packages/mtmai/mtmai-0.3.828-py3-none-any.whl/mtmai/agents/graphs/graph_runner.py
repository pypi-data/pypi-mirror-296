...
from mtmai.models.agent import AgentMeta, AgentTask, Chatbot, UiMessage
...
def get_agent_by_name_v3(agent_name: str):
    ...
    if not a:
        agents[agent_name] = get_graph_by_name(agent_name)
    ...
    async for event in graph.astream_events(
        ...
        if kind == "on_chain_end":
            ...
            if node_name == "LangGraph":
                logger.info("中止节点")
                if data and (output := data.get("output")) and (final_messages := output.get("messages")):
