...
from mtmai.api import (agent, auth, blog, bot, chat, doccolls, image, items,
                       metrics, openai, train, uimessages, users, utils,
                       webhook, workspace)
...
from mtmlib.env import (is_in_docker, is_in_huggingface, is_in_testing,
                        is_in_windows)
...
def build_app():
    ...
    if settings.BACKEND_CORS_ORIGINS:
        ...
        app.add_middleware(
            CORSMiddleware,
            allow_origins=["*"] if settings.BACKEND_CORS_ORIGINS=="*" else [str(origin).strip("/") for origin in settings.BACKEND_CORS_ORIGINS],
