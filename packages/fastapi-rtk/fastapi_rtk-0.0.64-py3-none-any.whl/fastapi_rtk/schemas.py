...
def generate_user_create_schema(model: typing.Type[User]):
    if "CreateUserSchema" in cached_schema:
        return cached_schema["CreateUserSchema"]
    ...
    schema = merge_schema(
        ...
        name=f"{model.__name__}-CreateUserSchema",
    )
    cached_schema["CreateUserSchema"] = schema
    return schema


def generate_user_update_schema(model: typing.Type[User]):
    if "UpdateUserSchema" in cached_schema:
        return cached_schema["UpdateUserSchema"]
    ...
    schema = merge_schema(
        ...
        name=f"{model.__name__}-UpdateUserSchema",
    )
    cached_schema["UpdateUserSchema"] = schema
    return schema
