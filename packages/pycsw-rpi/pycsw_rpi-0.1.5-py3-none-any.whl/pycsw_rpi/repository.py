...
def execute_transaction_webhook(context, transaction, records):
    """
    ...
    summary = [
        {
            ...
            "record_id": getattr(record, get_column_name(context, "pycsw:Identifier")),
            "resource_type": getattr(record, get_column_name(context, "pycsw:Type")),
        }
    ...
    if (
        ...
        try:
            r = requests.post(
                context.transactions_webhook_url, json=summary, timeout=10
            )
            r.raise_for_status()
