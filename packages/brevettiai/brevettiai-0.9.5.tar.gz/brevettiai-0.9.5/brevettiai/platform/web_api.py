...
from brevettiai.datamodel import web_api_types as api_types
...
class PlatformAPI:
    ...
    def __init__(self, username=None, password=None, organization="default", host=None,
    ...
    def api(self, v: Literal[0, 1] = 0, organization: Optional[str] = None):
        if v == 1:
            organization = self.get_organization_id(organization) or self._context_organization
            if organization:
    ...
    def session(self):
        if self._session is None:
            self._session = OAuth2Session(
                client=LegacyApplicationClient(client_id=OAUTH_CLIENT_ID, scope=OAUTH_SCOPE),
                auto_refresh_kwargs={
    ...
    def login(self, username=None, password=None, organization="default"):
        ...
        token_id = "/".join((("webapi." + self.host.split('://', 1)[-1]), f"{username or '*'}", organization))
        if username and password:
            self._token_path = token_dir / token_id
            token = self.session.fetch_token(
                token_url=self.host + OAUTH_TOKEN_URL, client_id=OAUTH_CLIENT_ID,
                username=username, password=password, organization_id=organization
            )
            ...
            token_paths = token_paths_map.get(token_id, token_paths_map.get("default", token_paths))
            try:
                for token_path in token_paths:
                    ...
                    if token["expires_at"] < time.time():
                        token = self.session.refresh_token(token_url=self.host + OAUTH_TOKEN_URL)
                        self.session.token = token
                        self.token_updater(token)
                    return
            except Exception:
    ...
    def get_organization_id(self, organization: str):
        ...
        try:
            ...
            raise StopIteration(f"Organization does not match any available organizations {org_names}")

    def get_dataset(self, id: str = None, write_access=False, extended_details=False, *, organization: str = None,
                    **kwargs) -> Union[
    ...
    def get_model(self, id=None, **kwargs) -> Union[api_types.Model, List[api_types.Model]]:
        """
    ...
    def get_report(self, id=None, **kwargs) -> Union[api_types.Report, List[api_types.Report]]:
        """
    ...
    def get_artifacts(self, obj: Union[api_types.Model, api_types.Report], prefix: str = '', recursive: bool = False,
                      add_prefix=False) -> List[api_types.FileEntry]:
        """
        ...
        if isinstance(obj, api_types.Model):
            r = self.session.get(f"{self.host}{self.api(0)}/models/{obj.id}/artifacts?prefix={prefix}")
        elif isinstance(obj, api_types.Report):
            r = self.session.get(f"{self.host}{self.api(0)}/reports/{obj.id}/artifacts?prefix={prefix}")
        else:
        ...
        for a in artifacts:
            if a.mime_type == "folder":
                all_artifacts.extend(
                    self.get_artifacts(obj, prefix=f"{prefix}{a.name}/", recursive=True, add_prefix=add_prefix))
            else:
                if add_prefix:
                    a.name = prefix + a.name
    ...
    def get_application_classification(self, application: api_types.Application = None,
                                       project: api_types.Project = None) -> Union[
        api_types.Application, List[api_types.Application]]:
    ...
    def get_application(self, id=None, *, organization: Optional[str] = None) -> Union[
        api_types.Application, List[api_types.Application]]:
        """
        ...
        applications = [a for p in projects for a in p.applications if id in {a.id, a.name}]
    ...
    def get_project(self, id=None, *, organization: str = None) -> Union[api_types.Project, List[api_types.Project]]:
        url = f"{self.host}{self.api(1, organization=organization)}/resources/projects"
        url = url if id is None else url + "/" + id
        r = self.session.get(url)
        return parse_raw_as(Union[api_types.Project, List[api_types.Project]], r.content)

    def get_modeltype(self, id=None, master=False) -> Union[api_types.ModelType, List[api_types.ModelType]]:
        """
        Get model type
        ...
        r = self.session.get(url)
        return parse_raw_as(Union[api_types.ModelType, List[api_types.ModelType]], r.content)

    def get_reporttype(self, id=None, master=False) -> Union[api_types.ReportType, List[api_types.ReportType]]:
        """
        Get report type
        ...
        r = self.session.get(url)
        return parse_raw_as(Union[api_types.ReportType, List[api_types.ReportType]], r.content)
    ...
    def get_dataset_annotations(self, dataset, **kwargs):
        ...
        return annotations.get_dataset_annotations(self.host, dataset_id, get_fn=self.session.get, **kwargs)
    ...
    def update_annotation(self, dataset, image_path, annotation, **kwargs):
        ...
        return annotations.update_annotation(self.host, dataset_id,
                                             ...
                                             post_fn=self.session.post, **kwargs)
    ...
    def create_annotation(self, dataset, image_path, annotation_name=None):
        return annotations.create_annotation(self.host, dataset=dataset, image_path=image_path,
                                             annotation_name=annotation_name, post_fn=self.session.post)
    ...
    def delete_annotation(self, dataset, image_path, annotation_name):
        return annotations.delete_annotation(self.host, dataset=dataset, image_path=image_path,
                                             annotation_name=annotation_name, delete_fn=self.session.delete)
    ...
    def get_items(self, dataset=None, item_id=None, filter: dict = None, **kwargs):
        return get_items(host=self.host, get_fn=self.session.get,
    ...
    def delete_item(self, dataset_id, item_id, **kwargs):
        return delete_item(host=self.host, delete_fn=self.session.delete,
                           dataset_id=dataset_id, item_id=item_id, **kwargs)

    def create(self, obj: Union[Dataset, Tag, api_types.Model, api_types.Report], **kwargs):
        if isinstance(obj, Dataset):
            ...
            r = self.session.post(f"{self.host}{self.api(0)}/models", json=payload)
    ...
    def update(self, obj, master=False):
        if isinstance(obj, Dataset):
            ...
            payload = obj.dict(include={"name", "parent_id"}, by_alias=True, exclude_none=True)
            self.session.post(f"{self.host}{self.api(0)}/resources/tags/{obj.id}", json=payload)
        elif isinstance(obj, api_types.ModelType):
            if not master:
                raise PermissionError("Not authorized")
            self.session.post(f"{self.host}{self.api(0)}/master/modeltypes/update", json=obj.dict(by_alias=True))
        elif isinstance(obj, api_types.ReportType):
            if not master:
                raise PermissionError("Not authorized")
            self.session.post(f"{self.host}{self.api(0)}/master/reporttypes/update", json=obj.dict(by_alias=True))
        else:
            raise NotImplementedError(f"create not implemented for type {type(obj)}")
    ...
    def update_application_datasets(self, app: api_types.Application):
        ...
        self.session.post(f"{self.host}{self.api(0)}/resources/projects/{project_id}/applications/{app.id}/datasets",
                          json=app.dict(by_alias=True, include={"name", "training_dataset_ids", "test_dataset_ids"}))

    def delete(self, obj: Union[
        Dataset, Tag, api_types.Model, api_types.Report, api_types.SftpUser, api_types.ReportType, api_types.ModelType]):
        if isinstance(obj, Dataset):
            ...
            self.session.delete(f"{self.host}{self.api(0)}/data/{obj.folder}/sftp/{obj.user_name}")
        elif isinstance(obj, api_types.ModelType):
            self.session.delete(f"{self.host}{self.api(0)}/resources/modeltypes/{obj.id}")
        elif isinstance(obj, api_types.ReportType):
            self.session.delete(f"{self.host}{self.api(0)}/resources/reporttypes/{obj.id}")
        else:
    ...
    def update_dataset_permission(self, id, user_id, group_id=None, permission_type="Editor"):
        ...
        r = self.session.post(f"{self.host}{self.api(0)}/admin/datasets/{id}/permissions", json=payload)
        return r
    ...
    def get_userinfo(self):
        ...
        Get info on user
        """
        url = f"{self.host}{self.api(0)}/manage/index"
        r = self.session.get(url)
        return api_types.User.parse_raw(r.content)

    def get_sftp_users(self, dataset, **kwargs) -> List[api_types.SftpUser]:
        r = self.session.get(f"{self.host}{self.api(0)}/data/{dataset.id}/sftp", **kwargs)
    ...
    def create_sftp_user(self, dataset, **kwargs) -> api_types.SftpUser:
        r = self.session.post(f"{self.host}{self.api(0)}/data/{dataset.id}/sftp", **kwargs)
        return api_types.SftpUser.parse_raw(r.content)

    def create_model(self, name, datasets, settings: JobSettings = None,
                     model_type=None, tags=None, application: api_types.Application = None):
        """
        ...
        model = api_types.Model(name=name, dataset_ids=[x.id for x in datasets], model_type_id=model_type.id,
    ...
    def create_testreport(self, name, model, datasets, report_type=None, settings=None, tags=None, submitToCloud=False):
        ...
        report = api_types.Report(name=name, model_ids=[model.id], dataset_ids=[x.id for x in datasets],
    ...
    def initialize_training(self, model: Union[str, api_types.Model], job_type: Type[Job] = None,
        ...
        r = self.session.post(f"{self.host}{self.api(0)}/models/{model.id}/start", params=payload)
        if submitToCloud:
            return None
    ...
    def initialize_report(self, report: Union[str, api_types.Report], job_type: Type[Job] = None) -> Union[Job, None]:
        """
        Start training flow of a model
