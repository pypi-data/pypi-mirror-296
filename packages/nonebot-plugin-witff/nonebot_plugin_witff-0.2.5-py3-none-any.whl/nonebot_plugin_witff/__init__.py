...
async def fusion_activity_function(event: MessageEvent):
    ...
    if url_get.status_code != 200:
        await fusion_activity.finish("www失败,稍后再试试吧", at_sender=True)
        return
    ...
    if len(command_text) >= 2 and command_text[1] == "搜索":
        ...
        if not keyword:
            await fusion_activity.finish("咱好像没有说要查询什么呢?是咱听错了吗 ovo", at_sender=True)
            return
        ...
        if not filtered_data:
            await fusion_activity.finish(f"未找到名称或主题包含关键词 '{keyword}' 的兽聚呜...", at_sender=True)
            return
    else:
        filtered_data = data

    # 分段/分页
    items_per_page = 3  # 每页展示的数量[自定义]
    ...
    # 检查输入
    if len(command_text) >= 2 and command_text[1].isdigit():
        page_num = max(1, min(num_pages, int(command_text[1])))  # 确保页码在有效范围内
