...
LastEditTime: 2024-09-17 17:27:08
...
# ** 二维插值函数
def interpolate_2d(target_x, target_y, origin_x, origin_y, data, method='linear'):
    """
    二维插值函数
    参数:
    ...
    if origin_x.shape != data.shape or origin_y.shape != data.shape:
        print(origin_x.shape, data.shape)
        raise ValueError(
...
# ** 高维插值函数，插值最后两个维度
def interpolate_2d_high(target_x, target_y, origin_x, origin_y, data, method='linear'):
    """
...
def interpolate_2d_high_parallel(target_x, target_y, origin_x, origin_y, data, method='linear'):
...
if __name__=='__main__':
    ...
    # 二维插值
    start = time.time()
    interpolated_data = interpolate_2d(target_x, target_y, origin_x, origin_y, data)
    print(f'Interpolation time: {time.time()-start:.2f}s')

    # 绘图
    plt.figure()
    plt.contourf(target_x, target_y, interpolated_data)
    plt.colorbar()
    plt.show()
    ...
    start = time.time()
    interpolated_data = interpolate_2d_high(target_x, target_y, origin_x, origin_y, data)
    print(f'Interpolation time: {time.time()-start:.2f}s')
    ...
    # 高维插值多线程
    start = time.time()
    interpolated_data = interpolate_2d_high_parallel(target_x, target_y, origin_x, origin_y, data)
    print(f'Interpolation time: {time.time()-start:.2f}s')
