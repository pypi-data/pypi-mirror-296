...
#ifndef POLYSTAR_TRIANGULATION_SIMPLE_HPP_
#define POLYSTAR_TRIANGULATION_SIMPLE_HPP_
/*! \file
    \author Greg Tucker
...
`TetGen` is much more flexible than `polystar` requires. This class exists to
  ...
  : vertex_positions(0u,3u), vertices_per_tetrahedron(0u,4u)
  {
    ...
    if (addGamma) {
      for (ind_t i=0; i<verts.size(0); ++i){
        bool isGamma{true};
        for (ind_t j=0; j<numel; ++j)
          isGamma &= polystar::approx_float::scalar(tgi.pointlist[i*numel+j], 0.);
...
#endif // POLYSTAR_TRIANGULATION_SIMPLE_HPP_
