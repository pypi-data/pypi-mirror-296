...
""".format(customPreambleFileNameWithoutExtension = self.CustomPreambleFileNameWithoutExtension(),
    ...
    def InstitutionETNA(self, institutionTag):
        emailString = self.CommandWrapper(self.AuthorEMailsIfFirstFromInstitutionTag(institutionTag), "\\detokenize", separator = None)
        emailString = self.CommandWrapper(emailString, "\\texttt", separator = ", ")
        if emailString:
