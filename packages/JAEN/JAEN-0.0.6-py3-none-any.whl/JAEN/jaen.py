...
class Exam:
    ...
    def submit(self, question_id, answer=None, dataset_class=None, dataloader=None, model=None):
        ...
        if question_id == 1 and answer is not None:  # 함수 구현 제출
            ...
            try:
                payload["dataset_code_init"] = self.extract_code(dataset_class.__init__)
                payload["dataset_code_len"] = self.extract_code(dataset_class.__len__)
                payload["dataset_code_getitem"] = self.extract_code(dataset_class.__getitem__)
                payload["dataloader_info"] = self.extract_dataloader_info(dataloader)
            except:
                payload["dataset_code_init"] = 'Error'
                payload["dataset_code_len"] ='Error'
                payload["dataset_code_getitem"] = 'Error'
                payload["dataloader_info"] = 'Error'
