...
def _energy_res(df, mats):
    ...
    for i, mat in enumerate(mats):
        ...
        choices.append((en_res_p[0]*(1000*df["light"])+en_res_p[1]*np.sqrt(1000*df["light"])+en_res_p[2])/1000)
        
        if mat_type == 1:
            en_res_n = mat["resolution"]["energy"]["neutron"]
            conditions.append((df["mat_num"] == i) & (df["particle_type"] == 1))
            choices.append((en_res_n[0]*(1000*df["light"])+en_res_n[1]*np.sqrt(1000*df["light"])+en_res_n[2])/1000)
    return np.select(conditions, choices)