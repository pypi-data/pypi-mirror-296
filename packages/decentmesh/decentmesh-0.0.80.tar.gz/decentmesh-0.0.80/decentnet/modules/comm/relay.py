...
class Relay:
    ...
    def __init__(self, client_socket, beam_pipe_comm: dict, beam_msg_queue: dict,
        ...
        if self.do_metrics:
            ...
            logger.warning("Metrics will not be collected.. FAIL")
        ...
        request = asyncio.run(recv_all(self.socket, self.client_ip[0], self.local_port))
        if not request:
    ...
    async def do_relaying(self, t: Timer, relay_counter: RelayCounter):
        ...
        request = await recv_all(self.socket, self.client_ip[0], self.local_port)
