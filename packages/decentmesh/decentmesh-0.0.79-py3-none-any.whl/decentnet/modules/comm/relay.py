...
class Relay:
    ...
    def do_relaying(self, t: Timer, relay_counter: RelayCounter):
        self.alive = True
        logger.debug(
        ...
        try:
            self.relay_request(request, t, relay_counter)
    ...
    def relay_request(self, request, t: Timer, relay_counter: RelayCounter):
        t.stop()
        ...
        if relay_counter.count % RELAY_FREQUENCY_DB_ALIVE_UPDATE == 0:
            Thread(target=asyncio.run, args=(self.update_beacon_connection(beacon_pub_key),),
                   name="Update beacon", daemon=True).start()
            relay_counter.reset()
        ...
        logger.debug(f"Total block process time: {block_process_time_timer:.2f} ms")
