...
def test_void_element() -> None:
    ...
    assert isinstance(element, MarkupyElement)
...
def test_list_children() -> None:
    children: list[MarkupyElement] = [li["a"], li["b"]]
    result = ul[children]
...
def test_generator_children() -> None:
    gen: Generator[MarkupyElement, None, None] = (li[x] for x in ["a", "b"])
    result = ul[gen]
    assert str(result) == "<ul><li>a</li><li>b</li></ul>"
...
def test_custom_element() -> None:
    ...
    assert isinstance(el, MarkupyElement)
...
def test_iter() -> None:
    ...
    def generate_list() -> Generator[MarkupyElement, None, None]:
        nonlocal trace
...
def test_callable() -> None:
    ...
    def generate_img() -> MarkupyElement:
        nonlocal called
        called = True
        return img
