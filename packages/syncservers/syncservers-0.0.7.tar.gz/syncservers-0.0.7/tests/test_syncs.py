...
class FakeSchedule:
    ...
    def schedule(func, cron, args_dict):
        pass
...
class Test(IsolatedAsyncioTestCase):
    ...
    def test_add_retry_task_to_schedule(self):
        schedule = FakeSchedule()
        sync = BaseSync(None, None)

        # case: retry not on
        mocked_schedule = schedule.schedule = mock.Mock()
        mocked_get_config = sync._get_config = mock.Mock(return_value={})
        sync.add_retry_task_to_schedule(schedule)
        self.assertEqual(mocked_schedule.call_count, 0)
        mocked_get_config.close()
        mocked_schedule.close()
        ...
        sync.add_retry_task_to_schedule(schedule)
        mocked_schedule.assert_called_once_with(mock.ANY, "my_retry_cron")
        mocked_get_config.close()
        mocked_schedule.close()
    ...
    def test_cron_add_to_schedule(self):
        ...
        mocked_schedule = schedule.schedule = mock.Mock()
        sync.add_to_schedule(schedule)
        self.assertEqual(mocked_schedule.call_count, 2)
        mocked_schedule.assert_any_call(
            mock.ANY,
            "*/4 * * * *",
            {
                'kwargs': {
