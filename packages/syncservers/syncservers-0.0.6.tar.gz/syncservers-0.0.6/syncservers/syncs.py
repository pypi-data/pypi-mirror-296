...
class BaseSync:
    ...
    def add_retry_task_to_schedule(self, schedule):
        ...
        schedule.schedule(self._retry_queue.add_retries_to_sync, retry_cron)
...
class CronSync(BaseSync):
    ...
    def add_to_schedule(self, schedule):
        for base_sync_path, (path_configs, _) in self._get_path_mappings().items():
            ...
            schedule.schedule(self._add_to_sync, cron_str, {
                'kwargs': {
                    "base_sync_path": base_sync_path,
                    "sync_path": base_sync_path,
                    "is_folder": True,
