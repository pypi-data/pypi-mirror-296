...
from asynclcron.lcron import CronJob
...
class BaseSync:
    ...
    def add_retry_task_to_schedule(self, schedule_jobs):
        ...
        logger.info(f"[{self.PATH_TYPE}] adding service retry task to schedule, cron: {retry_cron}")
        schedule_jobs.append(CronJob(self._retry_queue.add_retries_to_sync, retry_cron))
    ...
    async def _add_to_sync(self, base_sync_path, sync_path, is_folder):
        """
...
class CronSync(BaseSync):
    ...
    def add_to_schedule(self, schedule_jobs):
        for base_sync_path, (path_configs, _) in self._get_path_mappings().items():
            ...
            schedule_jobs.append(CronJob(self._add_to_sync, cron_str, kwargs={
                "base_sync_path": base_sync_path,
                    "sync_path": base_sync_path,
                    "is_folder": True,
            }))
