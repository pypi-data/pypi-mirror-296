...
class Ashes:
    ...
    def connect(self, callbackFunction: Callable[[Model], Model], serverName: str = "") -> None:
        ...
        the server with the given serverName. If the first command line argument
        is given, it will be used instead of serverName. If neither the command line
        argument or serverName is given, "ASHES-SERVER" is used.
        The callback function receives a Model() class instance.
        """

        # Check if command line argument is given, in which case it will be used.
        if len(sys.argv) >= 2:
