...
from yta_multimedia.video.edition.effect.moviepy.position_effects.utils import get_moviepy_position, get_moviepy_position_by_coords, get_random_moviepy_position_in_bounds
from yta_multimedia.video.edition.effect.moviepy.position_effects.utils.shake import shake_decreasing
...
class ShakeDecreasingAtPositionMoviepyEffect(BasePositionMoviepyEffect):
    ...
    def apply_over_video(self, background_video: Union[str, VideoFileClip, CompositeVideoClip, ImageClip, ColorClip], position: Union[ScreenPosition, tuple] = ScreenPosition.RANDOM_INSIDE):
        ...
        effect = self.video.set_position(lambda t: shake_decreasing(t, position[0], position[1])).set_start(0).set_duration(self.video.duration)
