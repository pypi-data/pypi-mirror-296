...
from yta_multimedia.video.edition.effect.moviepy.position_effects.utils.position import get_moviepy_position, position_video_in
...
class StayAtPositionMoviepyEffect(BasePositionMoviepyEffect):
    ...
    def apply_over_video(self, background_video: Union[str, VideoFileClip, CompositeVideoClip, ImageClip, ColorClip], position: Union[ScreenPosition, CoordinateCorner, CoordinateCenter] = ScreenPosition.RANDOM_INSIDE):
        ...
        effect = position_video_in(self.video, background_video, position).set_start(0).set_duration(self.video.duration)
