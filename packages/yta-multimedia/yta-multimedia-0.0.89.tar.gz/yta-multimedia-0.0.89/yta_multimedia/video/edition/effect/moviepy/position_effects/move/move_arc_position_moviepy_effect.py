...
from yta_multimedia.video.edition.effect.moviepy.position_effects.utils.move import arc_movement
...
class MoveArcPositionMoviepyEffect(BasePositionMoviepyEffect):
    ...
    """
    def apply(self, initial_position: Union[ScreenPosition, tuple] = ScreenPosition.OUT_LEFT, final_position: Union[ScreenPosition, tuple] = ScreenPosition.CENTER, arc_is_bottom: bool = False, max_arc_height: float = 300):
        """
    ...
    def apply_over_video(self, background_video: Union[str, VideoFileClip, CompositeVideoClip, ImageClip, ColorClip], initial_position: Union[ScreenPosition, tuple] = ScreenPosition.OUT_LEFT, final_position: Union[ScreenPosition, tuple] = ScreenPosition.CENTER, arc_is_bottom: bool = False, max_arc_height: float = 300):
        """
        TODO: Write well
        ...
        if isinstance(initial_position, ScreenPosition):
            ...
            initial_position = get_moviepy_position_by_coords(self.video, background_video, initial_position[0], initial_position[1])
        ...
        if isinstance(final_position, ScreenPosition):
            ...
            final_position = get_moviepy_position_by_coords(self.video, background_video, final_position[0], final_position[1])

        effect = self.video.set_position(lambda t: arc_movement(t, initial_position, final_position, self.video.duration, arc_is_bottom, max_arc_height)).set_start(0).set_duration(self.video.duration)
