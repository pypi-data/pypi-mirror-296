...
from yta_multimedia.video.edition.effect.moviepy.position_effects.utils.move import linear_movement
...
class MoveLinearPositionMoviepyEffect(BasePositionMoviepyEffect):
    ...
    def apply_over_video(self, background_video: Union[str, VideoFileClip, CompositeVideoClip, ImageClip, ColorClip], initial_position: Union[ScreenPosition, tuple] = ScreenPosition.OUT_LEFT, final_position: Union[ScreenPosition, tuple] = ScreenPosition.CENTER):
        ...
        effect = self.video.set_position(lambda t: linear_movement(t, initial_position, final_position, self.video.duration)).set_start(0).set_duration(self.video.duration)
