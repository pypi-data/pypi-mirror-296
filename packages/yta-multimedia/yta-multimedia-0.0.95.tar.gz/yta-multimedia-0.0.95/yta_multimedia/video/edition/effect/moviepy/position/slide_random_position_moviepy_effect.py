...
class SlideRandomPositionMoviepyEffect(BasePositionMoviepyEffect):
    ...
    def apply_over_video(self, background_video: Union[str, VideoFileClip, CompositeVideoClip, ImageClip, ColorClip]):
        ...
        effect = concatenate_videoclips([
            MoveLinearPositionMoviepyEffect(self.video.subclip(0, movement_time)).apply_over_video(background_video, random_position[0], ScreenPosition.CENTER),
            StayAtPositionMoviepyEffect(self.video.subclip(movement_time, movement_time + stay_time)).apply_over_video(background_video, ScreenPosition.CENTER),
            MoveLinearPositionMoviepyEffect(self.video.subclip(movement_time + stay_time, self.video.duration)).apply_over_video(background_video, ScreenPosition.CENTER, random_position[1])
        ])
