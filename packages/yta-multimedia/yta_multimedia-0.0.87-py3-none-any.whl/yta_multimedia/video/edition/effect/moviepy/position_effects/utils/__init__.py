...
# TODO: Maybe this can be removed because of the new ScreenPosition.RANDOM
# that have been created with this purpose (and the unification purpose)
def get_random_moviepy_position_in_bounds(video, background_video = None):
    ...
    lower_limit = (video.w / 2, video.h / 2)
    if background_video:
        lower_limit = get_moviepy_position(video, background_video, ScreenPosition.TOP_LEFT)
    upper_limit = (1920 - (video.w / 2), 1080 - (video.h / 2))
    if background_video:
        upper_limit = get_moviepy_position(video, background_video, ScreenPosition.BOTTOM_RIGHT)
