from yta_multimedia.audio import audio_to_audiosegment
from yta_multimedia.video import video_to_moviepy_video
from yta_general_utils.file_processor import filename_is_type, FileType, replace_file_extension, get_file_extension
...
def adjust_audio_channels(audio, channel_pan: float = 0.0, volume_adjustment: Union[float, None] = None, start_time = None, end_time = None):
    """
    ...
    if volume_adjustment and not isinstance(volume_adjustment, float):
        raise Exception('The "volume_adjustment" parameter must be a valid number.')
...
def synchronize_audio_pan_with_video(audio, video: Union[str, VideoFileClip, CompositeVideoClip, ImageClip, ColorClip]):
    """
    ...
    """
    audio = audio_to_audiosegment(audio)
    video = video_to_moviepy_video(video)
    ...
    for i in range(frames_number):
        ...
        audio = adjust_audio_channels(audio, x_coordinate_to_channel_pan(video_x), None, t * 1000, (t + frame_duration) * 1000)
