...
def_opt = {
    ...
    "benchmark": False,
    "training":[],
    "training_weights": [x for x in map(str, [1, 2, 3, 7, 10, 25, 50, 75, 100, 128, 250, 500 , 750, 1000])],
    "debug": False,
    "timeout": 2.0,
    "time_mem": False,
    "model": "d",
...
def main():
    ...
    if opt['training']:
        ...
        training_db = training_db.drop_duplicates(subset = ['Chromosome', 'sec_start', 'sec_end']).reset_index(drop=True)
        write('')
        if len(training_db)!=nrows:
            write(f'Removed {nrows-len(training_db)} genes with identical Sec positions')
        ...
        pd.concat( [y, training_db], axis=1 ).to_csv( training_filepath, sep='\t', index=False)

        # fit, write scaler
        scaler = StandardScaler()
        scaler.fit(training_db) #computes the mean and std to be used for later scaling
        ...
        df = pd.DataFrame(columns=['True_weight', 'tn', 'fp', 'fn', 'tp', 'precision', 'sensitivity_regression', 'sensitivity_global'])
        for x in map(int, opt['training_weights']):
            ...
            write(f'Precision: {precision:.2%}, Sensitivity: {sensitivity_regr:.2%} (regression) {sensitivity_global:.2%} (global)')
            df.loc[df.shape[0]] = [x, tn_ml, fp_ml, fn_ml, tp_ml, precision, sensitivity_regr, sensitivity_global]
        ...
        write(f"Writing pickled regressor filters to files: {output_folder+'/lr_selenoproteins.*.pkl'}")
