...
-training  <list>   Train new regression filters on your unfiltered results. See details with: twinstop -h training
-custom_model <str> Provides a custom regressor filter in form of two files: a pickled regressor and a scaler
"""
...
training_help = """
### Training usage:
# If -training is active, the usual twinstop workflow is not run so that the typical compulsory options are not read.
...
def main():
    ...
    if benchmark or opt['training']:
        ...
        write(f"Reading ground truth (expected selenoproteins) from file(s): {' '.join(opt['benchmark_subject_gff'])}")

        # read table of the Selenoprofiles4 selenocysteines prediction for subject (control selenoproteins)
        sp4_subj_gff = pd.concat([
          pd.read_csv(ffile,
              sep="\t", header=None, index_col=False, names=[
              "Chromosome",
              "source",
              "feature",
              "sec_start",
    ...
    if opt['training']:
        ...
        write(f"-training worflow active, reading unfiltered twinstop prediction file(s): {' '.join(opt['training'])}")
        table_list=[]
        for ffile in opt['training']:
            check_file_presence(ffile, '-training file')
            table_list.append( read_table(ffile) )
        training_db = pd.concat(table_list)
        del table_list

        ## remove REDUNDANCY TO DOOO
        ...
        true_positives = training_db[["ID", "Chromosome", "sec_start", "sec_end"]].merge(
            sp4_subj_gff[["Chromosome", "sec_start", "sec_end"]],
            on=["Chromosome", "sec_start", "sec_end"],
            how="inner",
        )
        ...
        write(f"Writing final training data used to file: '{training_filepath}'")
        training_db.to_csv(training_filepath, sep='\t', index=False)

        # add export here if necessary for debug
        training_db = training_db.drop(columns=["true_positive"])

        # how many of the expected selenoproteins are simply not present in the training DB?
        universal_false_negatives = training_db[["ID", "Chromosome", "sec_start", "sec_end"]].merge(
            sp4_subj_gff[["Chromosome", "sec_start", "sec_end"]],
            on=["Chromosome", "sec_start", "sec_end"],
            how="outer",
        ).ID.isnull().sum()
        write(f"Number of universal false negatives (i.e. missing from training data altogether): {universal_false_negatives}")

        X_train, X_test, y_train, y_test = train_test_split(training_db, y, train_size=0.2)
        X_train_transformed = scaler.transform(X_train)
        X_test_transformed = scaler.transform(X_test)

        scaler = StandardScaler()
        scaler.fit(ml_candidates_df) #computes the mean and std to be used for later scaling
        scaler_filepath = output_folder+'scaler.pkl'
        write(f"Writing sklearn scaler to file: {scaler_filepath}")
        with open(scaler_filepath, "wb") as f:
            pickle.dump(scaler, f)

        # commencing regression and benchmarking, varying weight of True
        df = pd.DataFrame(columns=['True_weight', 'tf', 'fp', 'fn', 'tp', 'precision', 'sensitivity_regression', 'sensitivity_global'])
        for x in [1, 2, 3, 7, 10, 25, 50, 75, 100, 128, 250, 500 , 750, 1000, 1024]:
            ...
            with open(lr_filepath, "wb") as f:
                pickle.dump(lr, f)
            _, fp_ml, fn_ml, tp_ml = confusion_matrix(y_test, lr_preds).ravel()

            print(f'True class weight: {x}\n')
            print(f'TF, FP, FN, TP = {confusion_matrix(y_test, lr_preds).ravel()}\n\n')
            ...
            print(f'Precision: {precision:.2%}, Sensitivity: {sensitivity_regr:.2%} (regression) {sensitivity_global:.2%} (global\n')
            df.loc[df.shape[0]] = [x, _, fp_ml, fn_ml, tp_ml, precision, sensitivity_regr, sensitivity_global]

        training_benchmarking_table_filepath = output_folder + 'training_benchmark.tsv'
        write(f"Writing table with benchmarking results: {training_benchmarking_table_filepath}")
        df.to_csv(path_or_buf=training_benchmarking_table_filepath, sep="\t", index=False)
        sys.exit()
    ...
    if benchmark:
        ...
        if opt["unavailable_families"]:
          ...
          removed_unavailable = selenos_annot.Sel_family.isin(unavailable_Sel_families)
          if removed_unavailable.any():
              removed_which_fams=selenos_annot[removed_unavailable].Sel_family.drop_duplicates()
