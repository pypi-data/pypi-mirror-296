...
from .mock_base_chat_model import MockBaseChatModel
...
class MockLlm:
    def __init__(self, *, name: str, cache_dir: str | Path, llm: BaseChatModel | MockBaseChatModel):
        """
        Initialize the wrapper with a name, cache directory, and real LLM object.
        """
        self.name = name
    ...
    def load_cache(self):
        ...
        if cache_file.exists():
            with jsonlines.open(cache_file) as reader:
                ...
                try:
                    first_cache = rows[0]
                    self.cached_responses = first_cache
