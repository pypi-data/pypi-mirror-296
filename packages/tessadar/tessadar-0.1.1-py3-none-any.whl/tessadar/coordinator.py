...
class Coordinator:
    ...
    def __init__(self, proxy: tessadar.Proxy, batch_size: int = 64):
        self.proxy = proxy
        self.callbacks = self.proxy.callbacks

        self.scheduler = tessadar.Scheduler(proxy=self.proxy, batch_size=batch_size)
        self.engine = tessadar.Engine(proxy=self.proxy, scheduler=self.scheduler)
