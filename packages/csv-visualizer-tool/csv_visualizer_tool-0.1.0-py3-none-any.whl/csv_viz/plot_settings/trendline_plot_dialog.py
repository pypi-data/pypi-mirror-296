...
from ..visualizations import plot_trendline
from PyQt5.QtWidgets import QWidget


class TrendlinePlotDialog(QDialog):
    def __init__(self, df, canvas):
        super().__init__()
        self.df = df
        self.canvas = canvas
        self.setWindowTitle('Trendline Plot Settings')
        ...
        button_box.accepted.connect(self.plot_trendline)
    ...
    def _create_general_tab(self):
        """Create the general tab for trendline plot settings."""
        general_tab = QWidget()
    ...
    def _create_aesthetics_tab(self):
        """Create the aesthetics tab for trendline plot settings."""
    ...
    def plot_trendline(self):
        """Plot the trendline plot based on user settings."""
        ...
        plot_trendline(self.canvas.figure, self.df[[x_column, y_column]], marker=marker, color=color, log_scale=logscale)
        self.canvas.draw()

        print(f"Trendline plot created with X: {x_column}, Y: {y_column}, Marker: {marker}, Color: {color}")
        self.accept()
