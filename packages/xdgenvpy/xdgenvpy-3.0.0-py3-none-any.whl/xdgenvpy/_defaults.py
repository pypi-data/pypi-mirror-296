...
_USER_NAME = os.getenv("USERNAME", "xdgenvpy")
...
_PREFIX_PATHS = collections.defaultdict(__unsupported_system)
if not _PREFIX_PATHS:
    _PREFIX_PATHS.update(
        {
            "FreeBSD": pathlib.Path("~").expanduser(),
            "Linux": pathlib.Path("~").expanduser(),
            "Darwin": pathlib.Path("~").expanduser(),
            "Windows": pathlib.Path(
                os.getenv("APPDATA", f"C:\\Users\\{_USER_NAME}\\AppData\\Roaming")
            ),
        }
    )

# Mapping between system type and multiple directory path separators.
_PATH_SEPARATORS = collections.defaultdict(__unsupported_system)
if not _PATH_SEPARATORS:
    _PATH_SEPARATORS.update(
        {
            "FreeBSD": pathsep_posix,
            "Linux": pathsep_posix,
...
def _get_system_default(freebsd, linux, mac, windows, multiple_dirs, system_type):
    """
    ...
    :param str freebsd: The FreeBSD default value.
    ...
    """
    dirs = collections.defaultdict(__unsupported_system)
    dirs.update(
        {
            "FreeBSD": freebsd,
            "Linux": linux,
...
def _normalize(freebsd, linux, mac, windows, multiple_dirs):
    """
    ...
    :param str freebsd: The Linux default value.
    :param str linux: The Linux default value.
    :param str mac: The Mac OS default value.
    :param str windows: The Windows default value.
    :param bool multiple_dirs: Flag indicating if normalization should process
    ...
    dirs = _get_system_default(freebsd, linux, mac, windows, multiple_dirs, system_type)
...
def XDG_RUNTIME_DIR():
    ...
    """
    return _normalize(
        freebsd=f"/run/user/{_USER_ID}",
        linux=f"/run/user/{_USER_ID}",
...
def XDG_DATA_DIRS():
    ...
    """
    return _normalize(
        freebsd="/usr/local/share:/usr/share",
