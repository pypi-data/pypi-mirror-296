...
class TestEDRI(unittest.TestCase):
    ...
    @patch('edri.Broker')
    def test_api_broker_start(self, mock_api_broker) -> None:
        mock_api_broker_instance = mock_api_broker.return_value
        with patch("edri.Queue") as mock_queue:
            self.edri.api_broker_start()
            mock_api_broker.assert_called_once_with(self.edri.router_queue, mock_queue.return_value, [])
        mock_api_broker_instance.start.assert_called_once()
        self.assertEqual(self.edri.api_broker, mock_api_broker_instance)

    @patch('edri.Listener')
    def test_start_api(self, mock_listener) -> None:
        self.edri.api_broker = MagicMock(spec=Broker)
        self.edri.api_broker.api_broker_queue = MagicMock(spec=Queue)
        mock_listener_instance = mock_listener.return_value
        self.edri.api_broker_start = MagicMock()
        with patch("edri.hasattr") as mock_hasattr:
            mock_hasattr.return_value = False
            self.edri.start_api()
        mock_listener.assert_called_once_with(self.edri.api_broker.api_broker_queue,[])
