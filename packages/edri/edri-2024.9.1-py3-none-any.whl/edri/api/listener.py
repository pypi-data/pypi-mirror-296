...
class Listener(Process):
    ...
    class App:
        ...
        def parse_headers(headers) -> Headers:
            parsed_headers = NormalizedDefaultDict[*Headers](list)
        ...
        @staticmethod
        async def is_rest_request(headers: Headers) -> bool:
        ...
        async def http(self, scope, receive, send, headers) -> None:
            ...
            if await self.is_rest_request(headers):
                handler = RESTHandler(scope, receive, send, headers)
            else:
                handler = HTMLHandler(scope, receive, send, headers)
            try:
        ...
        async def websocket(self, scope, receive, send, headers) -> None:
            ...
            while True:
                ...
                if event_response.get_response() is None:
                    ...
                    return self.unregister(pipe, key)
                await handler.response("ok", event_response, headers={})
