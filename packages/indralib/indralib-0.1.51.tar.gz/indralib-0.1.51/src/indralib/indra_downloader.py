...
class IndraDownloader:
    ...
    def get(
        ...
        cache_filename = None
        cache_path = None
        self.log.info(f"Url: >{url}<, alt_url: >{alt_url}<")
        if self.use_cache is True:
            self.log.info(f"Checking cache for {url}")
            if url in self.cache_info:
                cache_filename = self.cache_info[url]["cache_filename"]
                cache_path = os.path.join(self.cache_dir, cache_filename)
                cache_time = self.cache_info[url]["time"]
                self.log.info(f"Using cache for {url}: {cache_filename}")
            elif alt_url is not None:
                self.log.info(f"Checking for alt_url {alt_url}")
                ...
                self.log.info(f"Cache miss for {url}")
        else:
            self.log.info("Cache is disabled.")
        self.log.info(f"Downloading {url}, cache_filename: {cache_filename}")
