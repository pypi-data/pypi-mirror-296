...
class iAgent(iRunner):
    ...
    def eval_params(self, expressions, environments, **kw):
            ...
            for env in environments:
                ...
                while remain:
                    for key, source in list(remain.items()):
                        try:
                            value = eval(source, globalns, result)
                            if key not in result:
                                globalns[key] = result[key] = value
                            remain.pop(key)
    ...
    def _render_dict(self, data: Dict, **kw):
        ...
        for key, value in list(data.items()):
            if isinstance(value, str):
                try:
                    data[key] = Template(value).render(kw)
                except Exception as why:
                    foo = 1
...
class iBot(iAgent):
    ...
    async def old_process_stream(self, stream, context, **task):
        """TBD"""
...
class SortPlugin(iPlugin):
    ...
    SCORE = 960  # before NormalizePlugin

    @staticmethod
    def differentiable(item):
    ...
    def find_sort_key(self, stream: List[Dict], context: Dict):
        """
        ...
        for klass, candidates in list(universe.items()):
            ...
            if candidates:
                ...
                candidates.sort(key=self.differentiable)
                universe[klass] = candidates[0]
            else:
                universe.pop(klass)
        for type_ in wanted_types:
    ...
    async def handle(self, stream: List[Dict], context: Dict):
        ...
        if not sort_key:
            ...
            sort_key = self.find_sort_key(stream, context)
        if sort_key:
...
class HashStreamPlugin(iPlugin):
    ...
    async def handle(self, stream: List[Dict], context: Dict):
        ...
        for data in stream:
            ...
            blueprint += raw
            blueprint = hashlib.sha1(blueprint).hexdigest()
        ...
        if bounding:
            ...
            for data in stream:
                ...
                if geometry:
                    ...
                    warnings.add(
                        f"stream doesn't provide bounding geoquery: {GEOMETRY_SHAPE_KEY}"
                    )
                    _stream.append(data)
            stream[:] = _stream
        else:
            warnings.add(
                f"stream doesn't provide bounding geoquery: {GEOQUERY_COMP_KEY}"
            )
...
class MetadataPlugin(iPlugin):
    ...
    async def handle(self, stream: List[Dict], context: Dict):
        ...
        for data in list(stream):
            ...
            geojson = data[GEOJSON_KEY]  # TODO: fix, coords are tuples!
            kind = geojson['type']
...
class iAsyncCrawler(iCrawler):
    ...
    async def _get_initial_wave(self, task):
        ...
        if waves:
            ...
            early_wave = waves[0][MONOTONIC_KEY]  # must exists
            waves = [x for x in waves if x[MONOTONIC_KEY] == early_wave]
        return waves
    ...
    async def old_inject(self, task, data, *args, **kw):
