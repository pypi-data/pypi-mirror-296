...
def paper_detail_args2querys(args: object) -> List[Tuple[str, Dict]]:
    ...
    return [
        ...
        (f"MATCH (b:Person)-[:WRITE]->(a:Publication) WHERE a.{paper_k}=$value "
         "OPTIONAL MATCH (b:Person)-[:WRITE]->(p:Publication) "
         "RETURN b, count(DISTINCT p) AS citation ORDER BY citation DESC", {"value": paper_v}),
        # (f"MATCH (b:Person)-[:WRITE]->(a:Publication) WHERE a.{paper_k}=$value " +
...
def papers_detail_results2message(res: List, args: object):
    ...
    keyboards = [[
        InlineKeyboardButton(
            f"References",
            switch_inline_query_current_chat=f'/references "{paper_k}" "{paper_v}"'),
        InlineKeyboardButton(
            f"Citations",
            switch_inline_query_current_chat=f'/citations "{paper_k}" "{paper_v}"'),
    ]]
    ...
    for author, n_papers in authors:
        ...
        if k is not None:
            author_keyboards.append(
                InlineKeyboardButton(
                    f"{author['name']}",
                    switch_inline_query_current_chat=f'/author_papers "{k}" "{v}"'
                )
            )
    N = 3
