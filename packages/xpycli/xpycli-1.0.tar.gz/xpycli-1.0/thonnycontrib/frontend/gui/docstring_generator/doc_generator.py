...
from thonnycontrib.exceptions import BackendException, DocGeneratorParserException, FrontendException, NoFunctionSelectedToDocumentException
from thonnycontrib.utils import replace_error_line, get_last_exception
from thonnycontrib.frontend.gui.ThonnyLogsGenerator import log_doc_in_thonny
import thonnycontrib.frontend.gui.docstring_generator as docstring_generator
...
class DocGenerator():
    ...
    def __run(self, selected_lineno:int, text_widget:EditorCodeViewText):
        ...
        if self._strategy.can_generate(selected_lineno):
            ...
            if get_l1test_gui_runner().has_exception() or self.has_exception(): # si docgen avait lancé une exception avant
                # si les deux ont lancé une exception, on supprime l'exception de docgen car elle a été déja montrée
                if get_l1test_gui_runner().has_exception() and self.has_exception(): 
                    get_l1test_gui_runner().clean_error_view()
                    get_l1test_gui_runner().get_reporter().get_error_view().hide_view()
                    get_l1test_gui_runner().get_reporter().get_treeview().show_view()
                    get_l1test_gui_runner().get_reporter().get_treeview().clear_tree()
                elif get_l1test_gui_runner().has_exception(): # si docgen n'avait pas lancé d'exception
    ...
    def _show_treeview(self):
        ...
        """
        get_l1test_gui_runner().hide_errorview_and_show_treeview()
    
    def show_error(self, exception:Exception, error_title:str=CANNOT_GENERATE_THE_DOCSTRING):
        """
        Shows the error in the ErrorView if the docstring generator raises an exception.
        """
        reporter = get_l1test_gui_runner().get_reporter()
        if self.has_exception():
            exception_response = ExceptionResponse(exception)
            reporter.display_error_msg(str(exception_response), title=error_title)
            reporter.get_error_view().show_view()   
            reporter.get_treeview().hide_view()     
