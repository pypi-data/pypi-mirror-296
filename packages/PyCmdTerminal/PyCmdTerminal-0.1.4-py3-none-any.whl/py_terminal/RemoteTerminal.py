...
class RemoteTerminal(SSHClient, _Terminal):
    ...
    def __init__(self,
        ...
        """
        Initializes the Ssh class with connection parameters and sets up the SSH client.

        Args:
            host (Optional[str]): The hostname or IP address of the SSH server.
            username (Optional[str]): The username for authentication.
            password (Optional[Union[str, None]]): The password for authentication (default None).
            port (int): The port number for the SSH connection (default is 22).
            try_retry_password (bool): Whether to retry password input on failure.
            try_retry_password_times (int): The number of retry attempts for password input (default is 3).
            use_terminal_input_password (bool): Whether to allow terminal input for passwords (default is False).
            terminal_password_show (bool): Whether to show the password while typing in the terminal (default is False).
            timeout (int): The timeout for the SSH connection in seconds (default is 10).
            allow_agent (bool): Whether to allow the use of SSH agents (default is True).
            look_for_keys (bool): Whether to search for discoverable private key files (default is True).
            key_filename (Optional[Union[str, List[str]]]): The filename, or list of filenames, of the private key(s) (default is None).
            host_key_policy (Optional[Type[HostKeyPolicy]]): Custom host key policy (default is `WarningPolicy`).
            known_hosts_file (Optional[str]): Path to known hosts file for host key validation (default is None).
            logger (logging.Logger): Logger instance for logging messages (default is a logger named 'RemoteTerminal').
            log_level (int): Logging level (default is NOTSET).
        """
    ...
    def check_connect(self) -> bool:
        try:
            stdin, stdout, stderr = self._exec_command("echo test", timeout=3)
    ...
    def async_execute_command(self, command: str) -> Optional[int]:
        ...
        full_command = f"nohup {stdbuf}{command} 1> {out_tmp} 2> {err_tmp} & echo $!"
        try:
    ...
    def get_transport(self) -> Transport:
        """
        Get the custom SSH transport.

        Returns:
            Transport: The custom SSH transport.
        """
