...
class Thread(threading.Thread):
    ...
    def run(self):
        try:
            if self._target:
                self._kwargs['event'] = self._event
                self._target(*self._args, **self._kwargs)
        except Exception as e:
            raise RuntimeError(f"Exception occurred in thread '{self.name}'") from e
...
class _Terminal:
    def __init__(self, logger=logging.getLogger('Terminal'), log_level=logging.NOTSET):
        self.logger = logger
    ...
    def execute_command(self, command: str, timeout: Optional[int] = None, password: Optional[str] = None) -> dict:
        """
        Execute a command on the remote server and return detailed information about the execution.

        Args:
          command (str): The command to execute on the remote server.
          timeout (Optional[int]): The timeout for command execution, in seconds (default is None).
          password (Optional[str]): The password to use for authentication (default is None).
        ...
        Returns:
          dict: A dictionary containing the following keys:
              ...
              - 'exit_status': The exit status code of the command execution.

        Raises:
          RuntimeError: If there is a general error executing the command.
          SSHException: If there is an SSH-specific error during command execution.
          IOError: If there is an I/O error during command execution.
        """
        result = {
            'stdout': '',
            'stderr': '',
    ...
    def async_execute_command(self, command: str) -> Optional[int]:
        ...
        self._temp_list.append(out_temp)
        self._temp_list.append(err_temp)
    ...
    def read(self, path: str, mode="r", chunk_size: int = -1, position: int = 0) -> Union[Iterator[Any], str, Generator[str, Any, Any]]:
    ...
    async def _sync_process_std(self, pid: int, _callable: Callable[[bytes, bytes], None] = print, *, event: asyncio.Event = None):
        if pid not in self._async_log:
            raise ValueError(f"Log file path for PID {pid} is not defined.")
    ...
    def _exec_command(command, timeout=None):
        """
        Executes a command in a terminal and returns the standard output and standard error.

        Parameters:
        command (str): The command to execute in the terminal.
        timeout (int): The maximum time to wait for the command to complete. If not specified, the default timeout is used.

        Returns:
            subprocess.CompletedProcess: An object containing the exit code, standard output, and standard error of the command execution.
            str: Standard output of the command execution.
            str: Standard error of the command execution.
        """
    ...
    def close(self):
        if self._async_t.t_run:
            self._async_t.stop()
            self._async_t.join()
        while self._temp_list:
            try:
                self._temp_list.pop().close()
            except (Exception,):
                pass
