...
def updateData(
    ...
    table_update_sql = create_sql_update(sqlupdateDict["sql_update"][f"{table}_upd_{farmtype}"])
...
def updateMilking(
    ...
    if len(milking_update_new) > 0:
        ...
        df_lac_farm_corr['lactation_length'] = df_lac_farm_corr.groupby('animal_id')['prev_lactation_length'].shift(-1)
        df_lac_farm_corr = df_lac_farm_corr.loc[
            ...
            & (df_lac_farm_corr.prev_lactation_length >= 200)) | 
            (pd.isnull(df_lac_farm_corr.prev_lactation_length)
            & pd.isnull(df_lac_farm_corr.lactation_length))
            | 
            ((df_lac_farm_corr.prev_lactation_length >= 200)
        ...
        df_lac_corrected_2.loc[df_lac_corrected_2.dry_off >= (df_lac_corrected_2.dry_off.max()-dt.timedelta(days=30)),"dry_off"] = None
        ...
        df_merged["dry_off"] = df_merged["dry_off_gap_detection"].fillna(df_merged["dry_off_farmer"])
        ...
        df_merged = df_merged.loc[
            ...
            | (
                ...
                & ((df_merged.calving >= df_merged.dry_off_prev) | pd.isnull(df_merged.dry_off_prev))
        ...
        parity_dict = (
            parity_dict.loc[parity_dict.parity > 0].sort_values("parity").reset_index(drop=True)
        )
        ...
        df_apply_lac_corr = pd.concat(
            [
                milking_db[["farm_id", "animal_id", "milking_oid", "started_at", "ended_at"]],
                milking_update_new[["farm_id", "animal_id", "milking_oid", "started_at", "ended_at"]],
            ]
        )
        ...
        for lacids in lactation_db.sort_values(
            ...
            aniid = lactation_db.loc[lactation_db.lactation_id == lacids, "animal_id"].values[0]
            calvingdate = (
            ...
            lacno = lactation_db.loc[lactation_db.lactation_id == lacids, "parity"].values[0]
        ...
        milking_update_new_data = milking_update_new.merge(
            df_apply_lac_corr[["farm_id", "animal_id", "milking_oid", "started_at", "ended_at", "lactation_id", "parity", "dim"]],
            on=["farm_id", "animal_id", "milking_oid", "started_at", "ended_at"],
        )
        ...
        if len(milking_update_new_data) > 0:
            ...
            milking_update_new_data.loc[
                pd.isnull(milking_update_new_data.milking_system_oid), "milking_system_oid"
            ...
            milking_update_new_data = milking_update_new_data.drop(columns=["milking_system_oid"])
            ...
            for column in datetime_columns:
                milking_update_new_data[column] = pd.to_datetime(milking_update_new_data[column])
                milking_update_new_data[column] = milking_update_new_data[column].dt.strftime(
                    "%Y-%m-%d %H:%M:%S"
                )
        ...
        for table in ['bcs', 'activity', 'herdnavigator', 'insemination']:
            ...
            if not add_table_db.empty:
                for lacids in list(
                    ...
                    add_table_new.loc[
                        (add_table_new.animal_id == aniid) & (add_table_new.measured_on >= calvingdate),
                        "lactation_id",
                    ] = lacids
                    add_table_new.loc[
                        (add_table_new.animal_id == aniid) & (add_table_new.measured_on >= calvingdate),
                ...
                # calculate the dim
                add_table_new["dim"] = (
                    add_table_new.measured_on - pd.to_datetime(add_table_new.calvingdate, format="%Y-%m-%d")
                ) / np.timedelta64(1, "D")
                ...
                add_table_new = add_table_new[
                    ~pd.isnull(add_table_new.animal_id)
                ]
                add_table_new = add_table_new[
                    ~pd.isnull(add_table_new.lactation_id)
                ]
                add_table_new.lactation_id = (
                    add_table_new.lactation_id.astype(int)
                )
                add_table_new.loc[
                    pd.isnull(add_table_new.parity), "parity"
                ] = 0
