...
class Brilcalc:
    ...
    def execute_ssh(self, brilcmd: list[str]):
        if self.ssh_client is None:
            raise ValueError("SSH client is none, possibly missing credentials")
    ...
    def execute_locally(self, brilcmd: list[str]):
        ...
        env = os.environ.copy()
        env["PATH"] = f"{user_home}/.local/bin:{self.BRILCONDA}:{env['PATH']}"

        # Install brilws
        install_cmd = [f"{self.BRILCONDA}/pip", "install", "--user", "--upgrade", f"brilws=={self.brilws_version}"]
