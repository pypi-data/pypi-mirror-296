...
class RunRegistryActions:
    ...
    def refresh_runs(self, runobj_list: list[dict]):
        ...
        1. open in ONLINE RR runs from last step
        2. refresh in ONLINE RR runs from last step
        3. sign-off in ONLINE RR runs from last step
            (when we sign-off the dataset move back to waiting dqm gui)
        4. go to OFFLINE RR GLOBAL workspace and move the respective datasets
            to editable datasets (i.e. move dataset to OPEN)
        """
        for run in runobj_list:
            logging.info(
                "Refreshing run {run_number} in state {state}", run_number=run["run_number"], state=run["state"]
            )
            rr.move_runs(run=run["run_number"], from_=run["state"], to_="OPEN")
            rr.manually_refresh_components_statuses_for_runs(run=run["run_number"])
            rr.move_runs(run=run["run_number"], from_="OPEN", to_=run["state"])
