...
class LumilossAnalyzer:
    def __init__(
        ...
        low_lumi_runs: list,
        ignore_runs: list,
        bril_unit: Optional[str] = None,
        target_unit: Optional[str] = None,
    ):
        """
        - low lumi and special runs compose the delta between
        CMS recorded and DC processed
        - runs not in dcs josn *are* processed by DC, so they
        contribute to the total lumiloss
        """
        self.dc_json = dc_json
        self.ignore_runs = ignore_runs
        self.low_lumi_runs = low_lumi_runs
        ...
        # Bad lumis
        self.bad_lumis = self.parse_and_merge_lumis(rr_oms_lumis, bril_lumis)

    def parse_and_merge_lumis(self, rr_oms_lumis, bril_lumis):
        ...
        if self.bril_unit and self.target_unit:
        ...
        for bril_lumi in bril_lumis:
            ...
            if run_number in self.low_lumi_runs:
                self.total_low_lumi += recorded
                continue
            if run_number in self.ignore_runs:
    ...
    def format_lumiloss_by_run(self, data: dict):
        text = ""
        text += f"Luminosity unit: {self.target_unit}\n\n"
