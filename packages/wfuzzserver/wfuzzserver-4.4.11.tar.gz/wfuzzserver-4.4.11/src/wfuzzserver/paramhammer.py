    ...
    def request(self,params_group,reqtype):
        ...
        if reqtype=="postxml":
            ...
            if self.value_type == "bad":
                ...
                try:
                    resp = requests.post(url=self.url, data=dicttoxml(newgrp, root=self.xmlroot, attr_type=False).decode('utf-8'),headers=self.typesheaders[reqtype])
                    return resp
                except:
                    try:
                        resp = requests.post(url=self.url, data=dicttoxml(newgrp, root=self.xmlroot, attr_type=False).decode('utf-8'),headers=self.typesheaders[reqtype])
                ...
                try:
                    resp = requests.post(url=self.url, data=dicttoxml(params_group, root=self.xmlroot, attr_type=False).decode('utf-8'),headers=self.typesheaders[reqtype])
                    return resp
                except:
                    try:
                        resp = requests.post(url=self.url, data=dicttoxml(params_group, root=self.xmlroot, attr_type=False).decode('utf-8'),headers=self.typesheaders[reqtype])
        ...
        if reqtype=="putxml":
            ...
            if self.value_type == "bad":
                ...
                for param in params_group:
                    newgrp.update({param:params_group[param].replace('>','').replace('<','').replace('嘍嘊','')})
                try:
                    resp = requests.put(url=self.url, data=dicttoxml(newgrp, root=self.xmlroot, attr_type=False).decode('utf-8'),headers=self.typesheaders[reqtype])
                    return resp
                except:
                    try:
                        resp = requests.put(url=self.url, data=dicttoxml(newgrp, root=self.xmlroot, attr_type=False).decode('utf-8'),headers=self.typesheaders[reqtype])
                ...
                try:
                    resp = requests.put(url=self.url, data=dicttoxml(params_group, root=self.xmlroot, attr_type=False).decode('utf-8'),headers=self.typesheaders[reqtype])
                    return resp
                except:
                    try:
                        resp = requests.put(url=self.url, data=dicttoxml(params_group, root=self.xmlroot, attr_type=False).decode('utf-8'),headers=self.typesheaders[reqtype])
        ...
        if reqtype=="patchxml":
            ...
            if self.value_type == "bad":
                ...
                for param in params_group:
                    newgrp.update({param:params_group[param].replace('>','').replace('<','').replace('嘍嘊','')})
                try:
                    resp = requests.patch(url=self.url, data=dicttoxml(newgrp, root=self.xmlroot, attr_type=False).decode('utf-8'),headers=self.typesheaders[reqtype])
                    return resp
                except:
                    try:
                        resp = requests.patch(url=self.url, data=dicttoxml(newgrp, root=self.xmlroot, attr_type=False).decode('utf-8'),headers=self.typesheaders[reqtype])
                ...
                try:
                    resp = requests.patch(url=self.url, data=dicttoxml(params_group, root=self.xmlroot, attr_type=False).decode('utf-8'),headers=self.typesheaders[reqtype])
                    return resp
                except:
                    try:
                        resp = requests.patch(url=self.url, data=dicttoxml(params_group, root=self.xmlroot, attr_type=False).decode('utf-8'),headers=self.typesheaders[reqtype])
