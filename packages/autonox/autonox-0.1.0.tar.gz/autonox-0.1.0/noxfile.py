...
def get_distrib_files():
    ...
    whl_name = f"autonox-{version}-py"
    src_name = f"autonox-{version}.tar.gz"
    return [file for file in files if file.endswith(src_name) or file.find(whl_name) != -1]


def get_new_version_from_git_cliff(bump):
    try:
        version = subprocess.check_output(["git-cliff", "--bumped-version", "--bump", bump], text=True).strip()
...
def prep(session: nox.Session) -> None:
    ...
    new_version = new_version or get_new_version_from_git_cliff(bump)
    session.run(
        "git-cliff",
        "--tag",
        new_version,
    ...
    session.run("autonox", "--", "-f", "--only-local", silent=True, external=True)
    format(session)
