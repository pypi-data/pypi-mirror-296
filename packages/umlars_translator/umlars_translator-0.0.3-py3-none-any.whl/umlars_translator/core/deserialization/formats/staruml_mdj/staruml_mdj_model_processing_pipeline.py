    ...
    def _process(self, data_batch: DataBatch) -> Iterator[DataBatch]:
        ...
        try:
            ...
            optional_attributes = AliasToJSONKey.from_kwargs(
                visibility=StarumlMDJConfig.KEYS["visibility"],
                name=StarumlMDJConfig.KEYS["name"],
                literals=StarumlMDJConfig.KEYS["literals"],
            )
        except KeyError as ex:
            raise ValueError(
                f"Configuration of the data format was invalid. Error: {str(ex)}"
        ...
        self._process_literals(aliases_to_values)

        self.model_builder.construct_uml_enumeration(**aliases_to_values)

        yield from self._create_data_batches([])

    def _process_literals(self, aliases_to_values: dict) -> None:
        literals_dicts = aliases_to_values.get('literals')
        literals_names = list(map(lambda literal_dict: literal_dict['name'], literals_dicts)) if literals_dicts is not None else list()
        aliases_to_values["literals"] = literals_names
...
class UmlMessagePipe(StarumlMDJModelProcessingPipe):
    ...
    def _process(self, data_batch: DataBatch) -> Iterator[DataBatch]:
        ...
        try:
            ...
            self._logger.debug(f"Unable to map message sort: {ex}. Using default value: {UmlMessageSortEnum.SYNCH_CALL}")
