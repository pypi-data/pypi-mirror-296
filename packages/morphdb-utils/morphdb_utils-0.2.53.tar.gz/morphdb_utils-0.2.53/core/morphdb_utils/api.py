...
from morph.task.utils.network import is_cloud
from morph.task.utils.os import OsUtils
from morph.task.utils.profile import CONNECTION_TYPE, Connection, ProfileYaml
...
def __execute_sql_impl(
    ...
    if is_cloud():
        if connection_slug is not None:
            ...
            try:
                _, cloud_connection = ProfileYaml.find_builtin_db_connection()
                connector = Connector(
                    connection_slug if connection_slug is not None else "",
                    cloud_connection,
                    is_cloud=True,
