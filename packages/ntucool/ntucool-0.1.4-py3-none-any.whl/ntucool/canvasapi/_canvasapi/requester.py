...
import httpx
import requests
from .exceptions import BadRequest, CanvasException, Conflict, Forbidden, InvalidAccessToken, RateLimitExceeded, ResourceDoesNotExist, Unauthorized, UnprocessableEntity
from .util import clean_headers
...
class Requester(object):
    ...
    def request(self, method, endpoint=None, headers=None, use_auth=True, _url=None, _kwargs=None, json=False, is_async=False, **kwargs):
        """
        ...
        if is_async:
            response: httpx.Response = awaitlet.awaitlet(req_method(full_url, headers, _kwargs, json=json, is_async=is_async))
        else:
            response: httpx.Response = req_method(full_url, headers, _kwargs, json=json, is_async=is_async)
        logger.info('Response: {method} {url} {status}'.format(method=method, url=full_url, status=response.status_code))
        ...
        if response.status_code == 400:
            ...
            if b'Rate Limit Exceeded' in response.content:
                remaining = str(response.headers.get('X-Rate-Limit-Remaining', 'Unknown'))
                raise RateLimitExceeded('Rate Limit Exceeded. X-Rate-Limit-Remaining: {}'.format(remaining))
            else:
