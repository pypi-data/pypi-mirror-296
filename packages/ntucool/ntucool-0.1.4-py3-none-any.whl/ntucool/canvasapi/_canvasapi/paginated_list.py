...
import typing
import re
from typing import Generator, Iterable, Sequence, overload

class PaginatedList[T](Iterable[T]):
    """
    ...
    def __init__(self, content_class, requester, request_method, first_url, extra_attribs=None, _root=None, _url_override=None, **kwargs):
        """
    ...
    def _get_next_page(self):
        response: 'httpx.Response' = self._requester.request(self._request_method, self._next_url, _url=self._url_override, **self._next_params)
        ...
        if response.links:
            ...
            next_link = None
        regex = '(?:{}|{})(.*)'.format(re.escape(self._requester.base_url), re.escape(self._requester.new_quizzes_url))
    ...
    async def _get_next_page_async(self):
        response: 'httpx.Response' = await self._requester.request_async(self._request_method, self._next_url, _url=self._url_override, **self._next_params)
        data: 'dict' = response.json()
        self._next_url = None
        if response.links:
            next_link = response.links.get('next')
