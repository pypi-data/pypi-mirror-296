...
class OptimizeMultiply( optimizeBase.OptimizeBase ):
  ...
  def optimize( self, symExpr, action ):
    ...
    def _multiplyNumbers():
      ...
      # end = str( symExpr )
      # if ( start != end ):
      #   print( "_multiplyNumbers start: {}".format( start ) )
      #   print( "_multiplyNumbers end  : {}".format( end ) )
      return result
      ...
      while lFound == True :
        ...
        for iCnt, elem1 in enumerate( symExpr.elements ) :
          ...
          if elem1.power != 1:
            continue
          lFound = True
          # for iCnt1 in range( 0, len( elem1.elements )):
          for elem2 in elem1.elements :
            # symExpr.add( elem1.elements[ iCnt1 ] )
            symExpr.add( elem2 )
    ...
    def _multiplyElemUnitExpress():
      ...
      while( lFound == True and len( symExpr.elements ) > 1 ):
        ...
        for iCnt, elem1 in enumerate( symExpr.elements ) :
          ...
          # a root have multiple sulotuins, cannot multiply with + expressions
          ...
          while( lFound == False and iCnt2 < len( symExpr.elements )):
            ...
            # must be the same power
            # if ( elem2.power != elem1.power ):
            #   continue

            # found one
            # delete elements from current exrpession
            if iCnt < iCnt2:
      ...
      while( lFound == True and len( symExpr.elements ) > 1 ):
        ...
        for iCnt in range( 0, len( symExpr.elements ) - 1 ) :
          ...
          while( lFound == False and iCnt2 < len( symExpr.elements )):
            ...
            # for iCntSub1 in range( 0, len( elem1.elements ) ) :
            for elemSub1 in elem1.elements:

              # for iCntSub2 in range( 0, len( elem2.elements ) ) :
              for elemSub2 in elem2.elements:
                elem12 = symexpress3.SymExpress( '*' )
                # elem12.add( elem1.elements[ iCntSub1 ].copy() )
                # elem12.add( elem2.elements[ iCntSub2 ].copy() )
                elem12.add( elemSub1 )
                elem12.add( elemSub2 )
                elemnew.add( elem12 )
    ...
    def _multiplyElemVarVar():
      ...
      # multiple all symunits with same name
      lFound = True
      while( lFound == True and len( symExpr.elements ) > 1 ):
        lFound = False
        for iCnt in range( 0, len( symExpr.elements ) - 1 ) :
          ...
          while iCnt2 < len( symExpr.elements ):
            ...
            # elem1.powerCounter += elem2.powerCounter

            # elem1.onlyOneRoot   = max( elem1.onlyOneRoot, elem2.onlyOneRoot )
    ...
    # multiply number with multip-plus-exression
    def _multiplyNumberExpress():
      ...
      symExpr.elements = []
      symExpr.symType  = '+'
      # for iCnt in range( 0, len( elemExpr.elements)):
      for elemsub in elemExpr.elements :
        # elemsub = elemExpr.elements[ iCnt ]
        elemnew = symexpress3.SymExpress( '*' )
        elemnew.add( elemNum )
        elemnew.add( elemsub )
        symExpr.add( elemnew )
    ...
    def _multplyPlusExpressMultiply():
      ...
      for iCnt, elem in enumerate( symExpr.elements ) :
        ...
        # for iCnt2 in range( 0, len( symExpr.elements ) ) :
        for iCnt2, elemSub2 in enumerate( symExpr.elements ):
          if iCnt2 == iCnt:
            continue
          # symMulti.add( symExpr.elements[ iCnt2 ] )
        ...
        for elemSub2 in elem.elements :
          ...
          symNew.add( symMulti )
          # symNew.add( elem.elements[ iCnt2 ] )
          symNew.add( elemSub2 )
          symExpr.add( symNew )
    ...
    def _multplyPlusMultiplyOnlyRoots():
        ...
        for iCnt, elem in enumerate( symExpr.elements ) :
          ...
          for iCnt2 in range( iCnt + 1 , len( symExpr.elements )):
            ...
            if elem2.onlyOneRoot != 1:
              continue
            # if ( elem2.powerDenominator != elem.powerDenominator ):
            #   continue
            if elem2.powerDenominator == 1:
              continue
              ...
              if ( isinstance( elem, symexpress3.SymNumber ) and isinstance( elem2, symexpress3.SymNumber )):
                ...
                if ( len( dPrimeSet1 ) == 1 and len( dPrimeSet2 ) == 1 ):
                  if list( dPrimeSet1.keys() )[0] == list( dPrimeSet2.keys() )[0] :
                    ...
                    # put the numers in de counters of the power
                    ...
                    # elem.factCounter    = list( dPrimeSet1.keys()  )[ 0 ]
                    # elem2.factCounter   = list( dPrimeSet2.keys()  )[ 0 ]
                    # elem.powerCounter  *= list( dPrimeSet1.values())[ 0 ]
                    # elem2.powerCounter *= list( dPrimeSet2.values())[ 0 ]
      ...
      # if ( dAfter != dBefore ):
      #   print( "Before last: {} = {}".format( dBefore, cBefore ))
      #   print( "After  last: {} = {}".format( dAfter , cAfter  ))
      return result
    ...
    # return
    # check for symType but every routine must check for himsymExpr because the simType can be changed
    if symExpr.symType != '*':
      return result
    if symExpr.numElements() <= 1:
      return result
    ...
    #cAfter = str( symExpr )
    #dAfter = symExpr.getValue()
    #if ( "cos" not in cBefore and dAfter != dBefore ):
    #  print( "Before 4 : {} = {}".format( dBefore, cBefore ))
    #  print( "After  4 : {} = {}".format( dAfter , cAfter  ))
    ...
    #dAfter = symExpr.getValue()
    #if ( "cos" not in cBefore and dAfter != dBefore ):
    #   print( "Before 5 : {} = {}".format( dBefore, cBefore ))
    #   print( "After  5 : {} = {}".format( dAfter , cAfter  ))
    ...
    #dAfter = symExpr.getValue()
    #if ( "cos" not in cBefore and dAfter != dBefore ):
    #   print( "Before 6 : {} = {}".format( dBefore, cBefore ))
    #   print( "After  6 : {} = {}".format( dAfter , cAfter  ))
    ...
    # dAfter = symExpr.getValue()
    # if ( "cos" not in cBefore and dAfter != dBefore ):
    #   print( "Before 7 : {} = {}".format( dBefore, cBefore ))
    #   print( "After  7 : {} = {}".format( dAfter , cAfter  ))
