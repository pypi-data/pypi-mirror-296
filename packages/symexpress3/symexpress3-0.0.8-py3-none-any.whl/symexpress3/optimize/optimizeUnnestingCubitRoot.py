...
class OptimizeUnnestingCubitRoot( optimizeBase.OptimizeBase ):
  ...
  def optimize( self, symExpr, action ):
      ...
      for elem in rootPart.elements:
        ...
        if isinstance( elem, symexpress3.SymNumber ):
          if elem.powerDenominator == 2:
            if foundSqrt == True:
              return result # only 1 sqrt supportd
    ...
    factorCounter.sort( reverse=True ) # greaters counter first

    for facSign in range( 0, 2 ): # principal root, positive before negative
      # TODO make special hash for greaters quotient (factNum/facDenom)
...
def Test( display = False):
  ...
  _Check( testClass, symOrg, symTest, "1 * 5 * 1^^-1 + (-1) * (-12)^^(1/2)" )
  ...
  _Check( testClass, symOrg, symTest, "1 * 3 * 1^^-1 + (-20)^^(1/2)" )


  # (1/9 + 1/11 17^^(1/2))^^3
  symTest = symexpress3.SymFormulaParser( '((4252/88209) + 17^^(1/2) * (580/35937))^^(1/3)' )
  symTest.optimizeNormal()
  symTest = symTest.elements[ 0 ]
  # (1/9) * ((4252/121) + (15660/1331) * 17^^(1/2))^^(1/3)
  # get rid of the 1/9
  # symTest = symTest.elements[ 1 ]
  symOrg = symTest.copy()
  ...
  _Check( testClass, symOrg, symTest, "1 * 1 * 9^^-1 + (1/11) * 17^^(1/2)" )
  ...
  _Check( testClass, symOrg, symTest, "1 * 7 * 1^^-1 + (-3751)^^(1/2)" )
