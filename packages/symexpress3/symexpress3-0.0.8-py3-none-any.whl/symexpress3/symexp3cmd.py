...
"""
    Commandline interface for Symbolic expression 3
...
def OptimzeFunction( cExpress, outputFormat, optimizeActions ):
  """
  Optimze the given expression accoring the optimze actions
  Output the result for the given output types`
  """
  # conver string expression into object
  try:
...
def CheckOptimizeActions( cList ):
  """
  Check of the given optimze actions are valid
  """
...
def DisplayList( listTypes ):
  ...
  for listType in listTypes:
    if listType == "f" :
      print( "Functions:" )
      # funcTable = symexpress3.symexpress3.functionTable
      # funcTable = symexpress3.symtables.functionTable
      funcTable = symexpress3.GetAllFunctions()

      funcTable = dict(sorted(funcTable.items()))

      # for funcKey, funcObj in funcTable.items() :
      # for funcObj in funcTable.values() :
      for optKey, optValue in funcTable.items():
        print( f"  {optKey: <30} - {optValue}" )
        # print( f"  {optKey} - {optValue}" )
        # print( f"  {funcObj.syntax}" )
      ...
      for varKey, varValue in varDict.items():
        # print("  {0: <3} - {1}".format( varKey, varDict[ varKey ] ))
...
def DisplayVersion():
  ...
  """
  # print( "symexp3cmd.py - symexpress3 commandline interface" )
  print( "Version    : " + symexpress3.__version__    )
  # print( "Buildnummer: " + symexpress3.symexpress3.__buildnumber__ )
...
def DisplayHelp():
  ...
  print( "  -a <actions> : Comma seperated list of actions" )
...
def CommandLine( argv ):
  """
  Process the symexpres3 commandline parameters
  """
  outputFormat    = ""
  optimizeActions = []
  ...
  # nrarg = len( sys.argv )
  nrarg = len( argv )
  # print( "Aantal argumenten: " + str(nrarg) )
  ...
  for iCnt in range( 1, nrarg ) :
    # print( "Arg {} = {}".format( iCnt, argv[ iCnt ] ))
    cArg = argv[ iCnt ]
