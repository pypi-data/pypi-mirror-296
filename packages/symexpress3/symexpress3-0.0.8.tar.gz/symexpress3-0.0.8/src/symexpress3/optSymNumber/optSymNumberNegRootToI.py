...
def Test( display = False):
  ...
  # if str( symNew ).strip() != "((((-2) * 3^-1)^^2 + (0)^^2)^^(1/2))^^(1/2) * ( cos( ( atan2( 0,(-2) * 3^-1 ) + 2 * pi * 0) * 2^-1 ) + i *  sin( ( atan2( 0,(-2) * 3^-1 ) + 2 * pi * 0) * 2^-1 ))":
    print( f"Error unit test {testClass.name} number" )
    raise NameError( f'SymNumber optimze {testClass.name}, unit test error: {str( symTest )}, value: {str( symNew )}' )
  ...
  # if str( symNew ).strip() != "(i * (2/3)^^(1/2))^^5":
  # if str( symNew ).strip() != "(((((-2) * 3^-1)^^2 + (0)^^2)^^(1/2))^^(1/2) * ( cos( ( atan2( 0,(-2) * 3^-1 ) + 2 * pi * 0) * 2^-1 ) + i *  sin( ( atan2( 0,(-2) * 3^-1 ) + 2 * pi * 0) * 2^-1 )))^5":
    print( f"Error unit test {testClass.name} number" )
    raise NameError( f'SymNumber optimze {testClass.name}, unit test error: {str( symTest )}, value: {str( symNew )}' )
