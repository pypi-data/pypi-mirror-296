...
"""
    Optimze sin^2 + cos^2 = 1 for Sym Express 3
...
class OptimizeSinTwoCosTwo( optimizeBase.OptimizeBase ):
  ...
  def optimize( self, symExpr, action ):
    ...
    # check 2 symexpress with sin & cos
    ...
    for iCnt, elem in enumerate( symExpr.elements ):
      ...
      if elem.symType != '*':
        continue
      elemsin = None
      # for iCntSin in range( 0, len( elem.elements)):
      # for iCntSin in range( 0, len( elem.elements)):
      iCntSin = -1
      for iCntSin, elemcheck in enumerate( elem.elements):
        # elemcheck = elem.elements[ iCntSin ]
      ...
      for iCnt2, elem2 in enumerate( symExpr.elements ):
        ...
        # elemcos = None
        # for iCntCos in range( 0, len( elem2.elements)):
        for iCntCos, elemcheck in enumerate( elem2.elements):
          # elemcheck = elem2.elements[ iCntCos ]
