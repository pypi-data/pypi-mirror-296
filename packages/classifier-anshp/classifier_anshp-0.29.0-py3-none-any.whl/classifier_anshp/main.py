...
def hyper_tunner(x_train,y_train,x_test,y_test,epoch,mode):
    ...
    if(mode == 0): #binary_Classification
        ...
        def hype(x_train,y_train,x_test,y_test,epoch):
            ...
            print(f'inside hype , epoch_global:{epoch_global}')     
            ...
            for optimizer_suggest_index in range(len(optimizer_set)):
                    for loss_suggest_index in range(len(loss_set)):
                        ...
                        if(suggest_index == indexation):
                            ...
                            print(f'inside hype,optimizer_global:{optimizer_global}')
                            print(f'{"loss setting:":<20}{loss_set[loss_suggest_index]}')
                            loss_global = loss_set[loss_suggest_index]
                            print(f'inside hype,loss_global:{loss_global}')
        ...
        print(f'outside hype,epoch:{epoch_global}')
        print(f'outside hype,optimizer:{optimizer_global}')
        print(f'outside hype,loss:{loss_global}')
    else:
...
def model():
    ...
    print(type(optimizer_global))     
    print(type(loss_global))  
    model.compile(optimizer=optimizer_global,
                    loss=loss_global,
                    metrics=['accuracy'])
    
    print(f"Epoch Global: {epoch_global}")  # Add this before model.fit to debug
