...
from System import Decimal, UInt16, Enum, AppDomain, BitConverter
from System.Text import UTF8Encoding
from Package import *
from Intelligence import Login, DT, COM_STATUS, RECOVER_STATUS, MARKET_FLAG, SIDE_FLAG, TIME_IN_FORCE, PRICE_FLAG, POSITION_EFFECT, OFFICE_FLAG, ORDER_TYPE, ORDER_KIND, CreditQType
...
class TradeAPI():
    ...
    def __init__(self, debug=False, timeout=5000):
        self.is_login = False

        if debug:
            self.ServerHost = "itradetest.kgi.com.tw"
            self.ServerPort = 8000
        else:
            self.ServerHost = "tradeapi.kgi.com.tw"
            self.ServerPort = 8000

        sid = "API"
        self.conn = TaiFexCom(self.ServerHost, self.ServerPort, sid)
        logger.info(f"[KGI] Api version is {self.conn.version}")
        self.conn.Connect(self.ServerHost, self.ServerPort, timeout)
    ...
    def stock_order(self, action, broker_no, account_no, lot_type, order_type, side, symbol, quantity, price, price_flag, time_in_force, sub_account_no="", agent_id="", order_id=""):
        logger.info("[Stock_Order] Send order to server.")
        ...
        logger.info(f"[Stock_Order] RequestId={request_id}, receive reply:{self.conn.GetOrderErrMsg(rtn)}")
    ...
    def futures_order(self, action, branch_no, account_no, sub_account_no, market_type, tb_20_code,
        ...
        logger.info("[Futures_Order] Send order to server.")
        officeFlag = OFFICE_FLAG.OF_SPEEDY

        request_id = self.conn.GetRequestId()  # 取得送單序號
        if action.value == ORDER_TYPE.OT_NEW:  # 新單
            rtn = self.conn.Order(action.value, market_type.value, request_id, branch_no, account_no, sub_account_no, tb_20_code, side.value, order_type.value, Decimal(price), time_in_force.value, UInt16(quantity), writeoff.value, officeFlag)
        else:  # 刪/改單
            rtn = self.conn.Order(action.value, market_type.value, request_id, branch_no, account_no, sub_account_no, tb_20_code, side.value, order_type.value, Decimal(price), time_in_force.value, UInt16(quantity), writeoff.value, officeFlag, web_id, orig_net_no, order_id)

        logger.info(f"[Futures_Order] RequestId={request_id}, receive reply:{self.conn.GetOrderErrMsg(rtn)}")
    ...
    def foreign_futures_order(self, action, market_type, branch_no, account_no, sub_account_no, exchange,
        ...
        rtn = self.conn.FOrder(ord_kind, action.value, request_id, market_type.value, branch_no, account_no, sub_account_no, exchange,
                               ...
                               FCM, FCMAccount, orig_source, orig_seq, orig_net_no, trade_date, web_id, key_in, isMLeg, symbol2, side2.value, ComYM2, Decimal(strike_price2), cp2)
        logger.info(self.conn.GetOrderErrMsg(rtn))
    ...
    def receive_msg_handler(self, sender, package):
        logger.debug(f"[ReceiveMessage] {sender} {package}")

        match package.DT:
            case DT.MESSAGE_MAP:  # 1200 錯誤代碼訊息對應表
                logger.info(f"[ReceiveMessage]{package.ToLog()}")
            case DT.BROKER_MAP:  # 1201 公司代碼與代號對照
                logger.info(f"[ReceiveMessage]{package.ToLog()}")
            case DT.LOGIN:  # 1503 登入
                if package.Code == 0:
                    logger.info(f"[ReceiveMessage]({package.DT}) {package.Code} 登入成功")
                else:
                    logger.error(f"[ReceiveMessage]({package.DT}) {package.Code} 登入失敗 {self.conn.GetMessageMap(package.Code)}")
            case DT.AUTH_EXCHANGE:
                logger.info(f"[ReceiveMessage] {package.ToLog()}")

            # region 公告 2014.12.12 ADD
            case DT.NOTICE_RPT:  # P001701 公告(被動查詢)
                logger.info(f"[ReceiveMessage] 公告：{package.ToLog()}")
            case DT.NOTICE:  # P001702公告(主動)
                logger.info(f"[ReceiveMessage] 公告：{package.ToLog()}")

    def get_status_handler(self, sender, status, msg):
        logger.debug(f"[GetStatus] {sender} {status} {msg}.")

        match status:
            case COM_STATUS.LOGIN_READY:
                logger.info(f"[GetStatus] 登入成功: {sender.Accounts}")
                self.is_login = True
                self.Account = [acc.split(", ") for acc in sender.Accounts.split("\r\n") if acc]
            case COM_STATUS.ACCTCNT_NOMATCH:
                smsg = UTF8Encoding().GetString(msg)
                logger.info(f"[GetStatus] 部份帳號取得失敗: {smsg}")
            case COM_STATUS.LOGIN_FAIL:  # 登入失敗
                smsg = UTF8Encoding().GetString(msg)
                logger.info(f"[GetStatus] 登入失敗:[{smsg}]")
            case COM_STATUS.LOGIN_UNKNOW:  # 登入狀態不明
                smsg = UTF8Encoding().GetString(msg)
                logger.warning(f"[GetStatus] 登入狀態不明:[{smsg}]")
            case COM_STATUS.CONNECT_READY:  # 連線成功
                smsg = UTF8Encoding().GetString(msg)
                logger.info(f"[GetStatus] 伺服器 {sender.ServerHost}:{sender.ServerPort}\n伺服器回應: [{smsg}]\n本身為{'內' if sender.isInternal else '外'}部 IP:{sender.MyIP}")
            case COM_STATUS.CONNECT_FAIL:  # 連線失敗
                smsg = UTF8Encoding().GetString(msg)
                logger.error(f"[GetStatus] 連線失敗: {smsg} {sender.ServerHost}:{sender.ServerPort}")
            case COM_STATUS.DISCONNECTED:  # 斷線
                smsg = UTF8Encoding().GetString(msg)
                logger.info(f"[GetStatus] 斷線: {smsg}")
                self.is_login = False
            case COM_STATUS.AS400_CONNECTED:
                smsg = UTF8Encoding().GetString(msg)
                logger.info(f"[GetStatus] AS400 連線成功: {smsg}")
            case COM_STATUS.AS400_CONNECTFAIL:
                smsg = UTF8Encoding().GetString(msg)
                logger.info(f"[GetStatus] AS400 連線失敗: {smsg}")
            case COM_STATUS.AS400_DISCONNECTED:
                smsg = UTF8Encoding().GetString(msg)
                logger.info(f"[GetStatus] AS400 連線斷線: {smsg}")
            case COM_STATUS.SUBSCRIBE:
                smsg = UTF8Encoding().GetString(msg)
                sender.WriterLog("msg=" + smsg)
                logger.info(f"[GetStatus] 註冊:[{smsg}]")
            case COM_STATUS.UNSUBSCRIBE:
                smsg = UTF8Encoding().GetString(msg)
                logger.info(f"[GetStatus] 取消註冊:[{smsg}]")
            case COM_STATUS.ACK_REQUESTID:  # 下單或改單第一次回覆
                request_id = BitConverter.ToInt64(msg, 0)
                ack_status = msg[8]
                logger.info(f"[GetStatus] 序號回覆: {request_id} 狀態={'成功' if ack_status == 1 else '失敗'}.")

    def receive_time_handler(self, sender, server_time, conn_quality):
        # conn_quality : 本次與上次 HeatBeat 之時間差(milliseconds)
        logger.info(f"[ReceiveServerTime] {server_time:hh:mm:ss.fff}, time_diff={conn_quality} ms.")

    def recover_status_handler(self, sender, topic, status, recover_count):
        logger.debug(f"[RecoverStatus] {sender} {topic} {status} {recover_count}.")

        match status:
            case RECOVER_STATUS.RS_DONE:  # 回補資料結束
                logger.info(f"[RecoverStatus] 結束回補 Topic:[{topic} 筆數:{recover_count}]")
            case RECOVER_STATUS.RS_BEGIN:  # 開始回補資料
                logger.info(f"[RecoverStatus] 開始回補 Topic:[{topic}]")

    def stock_order_handler(self, sender, package):
        # 證券下單回報

        match package.DT:
            case DT.SECU_ORDER_RPT:  # 4010 (證)委託回報
                logger.info(f"[ReceiveMessage]({package.DT}) 委託回報 count={package.CNT}, order_no={package.OrderNo}.")
                ...
                logger.info(dict(zip(header, data)))
            case DT.SECU_DEAL_RPT:  # 4011 (證)成交回報
                logger.info(f"[ReceiveMessage]({package.DT}) 成交回報 count={package.CNT}.")
                ...
                logger.info(dict(zip(header, data)))
            case (DT.SECU_ORDER_ACK, DT.SECU_ORDER_ACK_N):  # 4002, 6002 下單第二回覆 2019.3. 13 Lynn Add
                logger.info(f"[ReceiveMessage]({package.DT}) 下單第二回覆{package.ToLog()} 訊息: {self.conn.GetMessageMap(package.ErrorCode)}.")
            case DT.SECU_EARMARK_SET:  # 4031 圈存
                logger.info(f"[ReceiveMessage]({package.DT}) EarMark Set: {package.ErrCode}")
                ...
                logger.info(dict(zip(header, data)))
            case DT.SECU_CREDITINFO:  # 4033 資券餘額查詢 2017.1.5 Add
                ...
                logger.info(dict(zip(header, data)))
            # 子帳額度控管: 回補
            case DT.SECU_ALLOWANCE_RPT:  # 5002
                logger.info(f"[ReceiveMessage]({package.DT}) {package.ToLog()}")
            case DT.SECU_ALLOWANCE:  # 5003
                logger.info(f"[ReceiveMessage]({package.DT}) {package.ToLog()}")

    def stock_financial_handler(self, sender, package):
        # 帳務中台WebSerivce 查詢
        match package.DT:
            case DT.FINANCIAL_WSSETAMTTRIAL:  # 4302 當日交割金額試算查詢
                logger.info(f"[ReceiveMessage]({package.DT}) 當日交割金額試算查詢 {package.Code} {package.CodeDesc} 筆數: {package.Rows1},{package.Rows2}")
                if package.Code == 0:
                    logger.info(package.Detail1)
                    logger.info(package.Detail2)
            case DT.FINANCIAL_WSSETAMTDETAIL:  # 4304 當日交割金額_沖銷明細(非當沖)查詢
                logger.info(f"[ReceiveMessage]({package.DT}) 當日交割金額_沖銷明細(非當沖)查詢 {package.BrokerID}-{package.Account} {package.Code} {package.CodeDesc} 筆數: {package.Rows}")
                if package.Code == 0:
                    logger.info(package.Detail)
            case DT.FINANCIAL_WSSETTLEAMT:  # 4306 交割金額查詢(3日)查詢
                logger.info(f"[ReceiveMessage]({package.DT}) 交割金額查詢(3日)查詢 {package.BrokerID}-{package.Account} {package.Code} {package.CodeDesc} 筆數: {package.Rows}")
                if package.Code == 0:
                    logger.info(package.Detail)
            case DT.FINANCIAL_WSINVENTORY:  # 4308 庫存損益及即時維持率試算查詢
                logger.info(f"[ReceiveMessage]({package.DT}) 庫存損益及即時維持率試算查詢 {package.BrokerID}-{package.Account} {package.Code} {package.CodeDesc} 筆數: {package.Rows}")
                if package.Code == 0:
                    logger.info(package.Detail)
            case DT.FINANCIAL_WSINVENTORYSUM:  # 4310 證券庫存彙總查詢
                # 庫存資料量大, 分多筆送回, 故須自行判斷是否傳送完畢(NowCount = TotalCount)
                # 須注意, 若同一時間查詢多次時, 會造成封包穿插送回, 資料會異常
                if package.Code != 0:
                    logger.warning(f"[ReceiveMessage]({package.DT}) 證券庫存彙總查詢 {package.Code} {package.CodeDesc}")
                else:
                    logger.debug(f"[ReceiveMessage]({package.DT}) 證券庫存彙總查詢 {package.BrokerID}-{package.Account} {package.Code} {package.CodeDesc}[{package.NowCount}/{package.TotalCount}]")
                    ...
                    if package.NowCount == package.TotalCount:
                        ...
                        logger.info(df)
            case DT.FINANCIAL_WSBALANCESTATEMENT:  # 4312 證券庫存彙總查詢
                # 資料量大, 分多筆送回, 故須自行判斷是否傳送完畢(NowCount = TotalCount)
                # 須注意, 若同一時間查詢多次時, 會造成封包穿插送回, 資料會異常
                if package.Code != 0:
                    logger.warning(f"[ReceiveMessage]({package.DT}) 證券對帳單查詢 {package.Code} {package.CodeDesc}")
                else:
                    logger.debug(f"[ReceiveMessage]({package.DT}) 證券對帳單查詢 {package.BrokerID}-{package.Account} {package.Code} {package.CodeDesc}[{package.NowCount}/{package.TotalCount}]")
                    if package.NowCount == 1:
                        self.list_data = []
                    for i in range(package.Detail.Length):
                        row = package.Detail[i]
                    ...
                    if package.NowCount == package.TotalCount:
                        ...
                        df = pd.DataFrame(self.list_data, columns=header)
                        logger.info(df)
            case DT.FINANCIAL_WSREALIZEPL:  # 4314 證券已實現損益查詢 <2017.9.18>
                # 資料量大, 分多筆送回, 故須自行判斷是否傳送完畢(NowCount = TotalCount)
                # 須注意, 若同一時間查詢多次時, 會造成封包穿插送回, 資料會異常
                if package.Code != 0:
                    logger.debug(f"[ReceiveMessage]({package.DT}) 證券已實現損益 {package.Code} {package.CodeDesc}")
                else:
                    logger.debug(f"[ReceiveMessage]({package.DT}) 證券已實現損益 {package.BrokerID}-{package.Account} {package.Code} {package.CodeDesc}[{package.NowCount}/{package.TotalCount}]")
                    if package.NowCount == 1:
                        self.list_data = []
                    for i in range(package.Detail.Length):
                        row = package.Detail[i]
    ...
    def futures_order_handler(self, sender, package):
        # region 國內下單回報
        match package.DT:
            case DT.FUT_ORDER_ACK:  # PT02002 下單第二回覆
                logger.info(f"[ReceiveMessage] {package.ToLog()} 訊息: {self.conn.GetMessageMap(package.ErrorCode)}")

            case DT.FUT_ORDER_RPT:  # PT02010 委託回報
                logger.info(f"[ReceiveMessage] 2010 [{package.CNT}, {package.OrderNo}]")
                ...
                logger.info(dict(zip(header, data)))
            case DT.FUT_DEAL_RPT:  # PT02011 成交回報
                logger.info(f"[ReceiveMessage] 2011 [{package.OrderNo}]")
    ...
    def foreign_futures_order_handler(self, sender, package):
        # region 國外下單回報
        match package.DT:
            case DT.FFUT_ORDER_ACK:  # PT03302 下單第二回覆
                logger.info(f"[ReceiveMessage] {package.ToLog()} 訊息: {self.conn.GetMessageMap(package.ErrorCode)}")
            case DT.FFUT_ORDER_RPT:  # PT03310 委託回報
                logger.info(f"[ReceiveMessage] 3310 [{package.CNT}, {package.ASorderNo}]")
                ...
                logger.info(dict(zip(header, data)))
            case DT.FFUT_DEAL_RPT:  # PT03311 成交回報
                logger.info(f"[ReceiveMessage] 3311 [{package.CNT}]")
                ...
                logger.info(dict(zip(header, data)))
            case DT.FFUT_ORDER_RPT2:  # PT03314 委託回報-複式單
                logger.info(f"[ReceiveMessage] 3314 [{package.CNT}, {package.ASorderNo}]")
                ...
                logger.info(dict(zip(header, data)))
            case DT.FFUT_DEAL_RPT2:     # PT03315 成交回報-複式單
                logger.info(f"[ReceiveMessage] 3315 [{package.CNT}]")
                header = ["委託型態", "交易所", "上手券商", "上手帳號", "上手委託書號", "AS400委託書號", "分公司代號", "帳號", "子帳帳號", "交易日期", "回報時間", "委託平台", "來源別", "委託網路單號", "商品代碼", "商品年月", "履約價", "CP", "BS", "F/I/R", "市/限價", "新/平倉", "當日沖銷", "成交價格", "成交均價", "成交口數", "成交序號", "剩餘有效口數", "總成交口數", "keyin"]
                data = [package.OrderFunc, package.Exchange, package.FCM, package.FFUT_ACCOUNT, package.ORDNO, package.ASorderNo, package.BrokerId, package.Account, package.AE, package.TradeDate, package.ReportTime, package.WEBID, package.SOURCE, package.CNT, package.Symbol, package.ComYM, package.StrikePrice, package.CP, package.BS, package.TimeInForce, package.PriceFlag, package.PositionEffect, package.DayTrade, package.DealPrice, package.AvgPrice, package.DealQty, package.PATSNo, package.LeavesQty, package.CumQty, package.KeyIn]
                logger.info(dict(zip(header, data)))
    ...
    def futures_financial_handler(self, sender, package):
        # region 帳務查詢
        match package.DT:
            case DT.FINANCIAL_COVER_TRADER:  # P001614
                logger.info(f"[ReceiveMessage] 1614 [{package.Rows}]")
                ...
                if package.Rows > 0:
                    for row in package.p001614_2:
                        ...
                        logger.info(dict(zip(header, data)))
            case DT.INVENTORY_TRADER:  # P001616
                logger.info(f"[ReceiveMessage] 1616 [{package.Rows}]")
                ...
                if package.Rows > 0:
                    for row in package.p001616_2:
                        ...
                        logger.info(dict(zip(header, data)))
            case DT.INVENTORY_DETAIL_TRADER:  # P001618
                logger.info(f"[ReceiveMessage] 1618 [{package.Rows}]")
                header = ["分公司", "帳號", "組別", "交易員", "交易所", "場內編號(外)", "上手帳號(外)", "交易方式", "上手券商", "交割日期(外)", "結算日期(外)", "下單方式", "電子單號", "委託書號", "成交序號", "拆單序號", "成交日期",
                          "商品代碼", "BS", "期/權", "CP", "履約價", "商品年月", "未平倉量", "結算價", "即時價", "未平倉損益", "原始保證金", "維持保證金", "幣別", "成交價", "混合口數一", "當日沖銷 ", "SPREAD", "複式組合單號",
                          "委託書號2", "成交序號2", "拆單序號2", "成交日期2", "商品代碼2", "BS2", "期/權2", "CP2", "履約價2", "商品年月2", "未平倉量2", "結算價2", "即時價2", "未平倉損益2", "原始保證金2", "維持保證金2", "幣別2", "成交價2", "混合口數2", "當日沖銷2"]
                if package.Rows > 0:
                    for row in package.p001618_2:
                        data = [row.BrokerId, row.Account, row.Group, row.Trader, row.Exchange, row.SeqNo, row.FcmActNo, row.TradeType, row.FCM, row.DeliveryDate, row.CloseDate, row.WEB, row.Cnt, row.OrdNo, row.MarketNo, row.sNo, row.TradeDate,
                                row.ComID, row.BS, row.ComType, row.CP, row.StrikePrice, row.ComYM, row.Qty, row.TrdPrice, row.MPrice, row.PRTLOS, row.InitialMargin, row.MTMargin, row.Currency, row.DealPrice, row.MixQty, row.DayTrade, row.SPREAD,
                                row.spKey, row.OrdNo2, row.MarketNo2, row.sNo2, row.TradeDate2, row.ComID2, row.BS2, row.ComType2, row.CP2, row.StrikePrice2, row.ComYM2, row.Qty2, row.TrdPrice2, row.MPrice2, row.PRTLOS2, row.InitialMargin2,
                                row.MTMargin2, row.Currency2, row.DealPrice2, row.MixQty2, row.DayTrade2]
                        logger.info(dict(zip(header, data)))
            case DT.FINANCIAL_COVER_TRADER_Detail:  # P001624
                logger.info(f"[ReceiveMessage] 1624 [{package.Rows}]")
                header = ["分公司", "帳號", "組別", "交易員", "交易所", "平倉日期", "平倉成交日期", "平倉委託編號", "平倉成交序號", "平倉拆單序號", "被平成交日期", "被平委託編號", "被平成交序號", "被平拆單序號", "指定平倉碼", "互抵",
                          "BS", "商品代號", "商品年月", "履約價 ", "CP", "被平商品代號", "平倉口數", "被平口數", "平倉成交價", "被平成交價", "平倉損益", "業務員代號", "幣別", "交易稅", "手續費", "平倉權利金", "被平權利金",
                          "平倉場內編號", "被平場內編號", "平倉電子單號", "被平電子單號", "淨損益"]
                if package.Rows > 0:
                    for row in package.p001624_2:
                        ...
                        logger.info(dict(zip(header, data)))
            case DT.FINANCIAL_TRADERN:  # P001626 可出金金額
                logger.info(f"[ReceiveMessage] 1626 [{package.Count}]")
                header = ["分公司", "帳號", "組別", "交易員", "幣別", "前日餘額", "手續費", "匯率", "期交稅", "存提", "平倉損益", "未平倉損益", "未沖銷買方選擇權市值", "未沖銷賣方選擇權市值", "委託權利金", "權利金收入與支出", "權益數",
                          "原始保證金", "維持保證金", "可用餘額", "可動用(出金)保證金", "委託保證金", "到期履約損益", "變動權利金", "洗價時間", "未沖銷期貨浮動損益", "昨日未沖銷期貨浮動損益", "到期結算保證金", "足額原始保證金", "足額維持保證金",
                          "當沖原始保證金", "多空減收保證金", "當沖應補保證金", "本日餘額", "本日權利金收入", "本日權利金支出", "有價証券價值", "有價証券抵繳金額", "委託抵繳保證金", "剩餘可抵繳金額", "足額風險指標", "足額權益比例", "追繳金額",
                          "賣方垂直價差市價", "履約價款", "權益總值", "本日期貨平倉損益淨額", "超額/追繳保證金", "加收保證金", "可動用(不含 CN$超額)", "可出金金額"]
                if package.Count > 0:
                    for row in package.p001626_2:
                        ...
                        logger.info(dict(zip(header, data)))
            case DT.FINANCIAL_CURRENCY:  # P001628
                logger.info(f"[ReceiveMessage] 1628 CODE = {package.Code} MSG = {package.ErrorMsg}")
            case DT.FINANCIAL_STRIKE:  # 2017.2.4 P001643  無效履約查詢
                header = ["分公司", "帳號", "組別", "交易員", "履約日期", "成交日期", "委託單號", "成交序號", "拆單序號", "交易所", "商品代碼", "商品年月", "履約價", "CP", "BS", "口數", "交易稅幣別", "交易稅", "手續費幣別", "手續費", "交易幣別", "權利金收支/履約盈虧", "結算價"]
                logger.info(f"[ReceiveMessage] 1643 Code =  {package.Code}, Rows[{package.Rows}]")
                if package.Rows > 0:
                    for row in package.Detail:
                        ...
                        logger.info(dict(zip(header, data)))
            case DT.FINANCIAL_COVERDH:  # 2017.2.14  P001645 平倉明細歷史查詢
                logger.info(f"[ReceiveMessage] 1645 Code = {package.Code}, Rows[{package.Rows}]")
                ...
                if package.Rows > 0:
                    for row in package.Detail:
                        ...
                        logger.info(dict(zip(header, data)))
            case DT.FINANCIAL_RECIPROCATE:  # 2018.12 P001647: 大小台互抵
                if package.Status == "0":
                    logger.info(f"{package.BrokerId} {package.Account} 大小台互抵成功: 大台 {package.Qty1}, 小台 {package.Qty2}")
                else:
                    logger.warning(f"{package.BrokerId} {package.Account} 大小台互抵失敗")
            case DT.FINANCIAL_SERVICECHARGE:  # P001631 手續費
                logger.info(f"[ReceiveMessage] 1631 [{package.Count}]")
                ...
                if package.Count > 0:
                    for row in package.p001631_2:
                        ...
                        logger.info(dict(zip(header, data)))
            case DT.FINANCIAL_APPLYWITHDRAW:  # P001633 出金申請
                logger.info("[ReceiveMessage] 1633 ")
                ...
                logger.info(dict(zip(header, data)))
            case DT.FINANCIAL_WITHDRAW:  # P001635
                logger.info(f"[ReceiveMessage] 1635 [{package.Count}]")
