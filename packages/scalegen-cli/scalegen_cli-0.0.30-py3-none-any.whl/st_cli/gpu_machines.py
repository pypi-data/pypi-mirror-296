...
def view_gpu_machine(gpu_machine_id: str):
    ...
    # print(ssh_key_id, username, ip, metadata)
    ...
    print(
        f"ssh -i  {file_path} {username}@{ip} -p {metadata.get('container_ssh_port', 22)}"
    )
    ...
    markdown_content += f"[orange_red1]Cost[/orange_red1] : [yellow]$ {round(response['cost'], 3)}[/yellow]"
