...
def infotuple(entry):
    ...
    return info_dict['multiarch'], info_dict['os'], entry[1], info_dict['sha512']
...
def do_lookup(logger, cache_dir, cache_url, signature_certificate_list,
    ...
    pt = prettytable.PrettyTable(['multiarch', 'distribution', 'version', 'id', 'compatible?', 'verified?', 'sha512'])
    machine_info = networkcache.machine_info_tuple()

    for multiarch, os, verified, sha512 in sorted(map(infotuple, entries)):
        row = [multiarch] + os
