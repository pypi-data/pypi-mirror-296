...
def ahrs_complementary_filter(
    ...
    if magnetic_field is None:
        ...
        estimated_rph[1:, :2] = (
            np.cumsum(angular_rate[:-1, :2] * dt, axis=0) + rph[0, :2]
        ) * gain + rph[:-1, :2] * (1 - gain)
        estimated_rph[1:, [2]] = (
            np.cumsum(angular_rate[:-1, [2]] * dt, axis=0) + initial_heading
        )
    else:
        # Estimate using IMU and magnetometer
        estimated_rph[1:] = (
            np.cumsum(angular_rate[:-1] * dt, axis=0) + rph[0]
        ) * gain + rph[:-1] * (1 - gain)
...
def ahrs_mahony_filter(
    ...
    Args:
        ...
        k1 (Union[float, int], optional): Gain for the accelerometer measurements. Defaults to 1.0.
    ...
    if reference_magnetic_field is not None:
        if reference_magnetic_field.ndim != 1 or reference_magnetic_field.shape[0] != 3:
            raise ValueError(
    ...
    for ix in range(1, samples):
        ...
        wmes1 = k1 * (vec_to_so3(a) @ (rot_mat.T @ g_vec)).flatten()
        if magnetic_field is not None:
            wmes2 = k2 * (vec_to_so3(m) @ (rot_mat.T @ m_vec)).flatten()
        wmes = wmes1 + wmes2 if magnetic_field is not None else wmes1
...
def ahrs_hua_mahony_filter(
    ...
    Args:
        ...
        kp (Union[float, int], optional): Proportional gain. Defaults to 1.0.
        ki (Union[float, int], optional): Integral gain. Defaults to 0.3.
        kb (Union[float, int], optional): Anti-windup gain. Defaults to 1.0.
        delta (Union[float, int], optional): Saturation limit for the gyro biases. Defaults to 1.0.
    ...
    if reference_magnetic_field is not None:
        m_vec = reference_magnetic_field
        m_vec = (m_vec / norm(m_vec)).reshape(3, 1)
        m_vec = (vec_to_so3(g_vec) @ m_vec) / norm(
            vec_to_so3(g_vec) @ m_vec, keepdims=True
        )  # Hua modification
    ...
    for ix in range(1, samples):
        ...
        if magnetic_field is not None:
            m = magnetic_field[ix, :] / norm(magnetic_field[ix, :])
            m = (vec_to_so3(a) @ m) / norm(
                vec_to_so3(a) @ m, keepdims=True
            )  # Hua modification
