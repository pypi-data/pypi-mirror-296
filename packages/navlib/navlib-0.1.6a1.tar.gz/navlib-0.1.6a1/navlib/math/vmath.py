...
def derivative(
    ...
    if not isinstance(time, np.ndarray) and not isinstance(time, list):
        raise TypeError("Input must be a numpy array or a list.")
...
def resample(
    ...
    if not isinstance(xp, np.ndarray) and not isinstance(xp, list):
        raise TypeError("Input xp must be a numpy array or a list.")
    if not isinstance(fp, np.ndarray) and not isinstance(fp, list):
        raise TypeError("Input fp must be a numpy array or a list.")

    # Convert to numpy arrays
    x = np.array(x) if isinstance(x, list) else x
    xp = np.array(xp) if isinstance(xp, list) else xp
