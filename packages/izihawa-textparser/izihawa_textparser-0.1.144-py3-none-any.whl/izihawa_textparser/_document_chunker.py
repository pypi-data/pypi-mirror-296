...
def _length_function(remove_tables: bool, text: str):
    cleaned_chunk = remove_markdown(text, remove_tables=remove_tables)
    ...
    def _splits_to_chunks(self, document: dict, field: str, splits):
        ...
        for chunk in chunks:
            ...
            if self._add_metadata and chunk.title:
                ...
                meta_title = f"\nTITLE: {spaced_title}"
                meta_title_length = self._length_function(meta_title)
                if meta_title_length <= self._chunk_size - chunk_length:
                    chunk.text += meta_title
                else:
                    logging.warning(f"Can't add title metadata due to excessive chunk size: {chunk_length}, {self._chunk_size}, {meta_title_length}")
            if (
                self._add_year
