    ...
    def _splits_to_chunks(self, document: dict, field: str, splits):
        ...
        for chunk in chunks:
            ...
            if self._add_metadata and chunk.title:
                ...
                if meta_title_length > self._max_extra_length:
                    chunk.text += meta_title
                else:
                    logging.warning(
                        f"Can't add title metadata due to excessive chunk size: ({chunk_length}){chunk.text}, ({meta_title_length}){meta_title}"
                    )
            if (
                self._add_year
