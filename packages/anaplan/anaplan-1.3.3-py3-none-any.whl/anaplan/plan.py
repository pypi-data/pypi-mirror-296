...
def learner(x_train, y_train, depth=None, early_stop=False, loss='categorical_crossentropy', show_history=False, interval=33.33, target_acc=None, except_this=None, only_this=None):
    
    """
    Optimizes the activation functions for a neural network by leveraging train data to find the most accurate 
    combination of activation potentiation for the given dataset. This function is designed to be used before model 
    evaluating.
    ...
    bar_format= bar_format_optimizer_with_loss, ncols=70)
    ...
    for i in range(depth):
        ...
        for j in range(len(activation_potentiation)):
            ...
            model = evaluate(x_train, y_train, W=W, acc_bar_status=False, loading_bar_status=False, activation_potentiation=activations)
            ...
            if acc >= best_acc:
                ...
                best_acc = acc
                postfix_dict["Accuracy"] = best_acc
                ...
                if loss == 'categorical_crossentropy':
                    ...
                    train_loss = categorical_crossentropy(y_true_batch=y_train, y_pred_batch=model[get_preds_raw()])
                    postfix_dict["Loss"] = train_loss
                    acc_bar.set_postfix(postfix_dict)

                elif loss == 'binary_crossentropy':

                    train_loss = binary_crossentropy(y_true_batch=y_train, y_pred_batch=model[get_preds_raw()])
                    postfix_dict["loss"] = train_loss
                    acc_bar.set_postfix(postfix_dict)
                ...
                if target_acc is not None:
                    ...
                    if best_acc >= target_acc:
                        ...
                        if loss == 'categorical_crossentropy':
                            ...
                            postfix_dict["Train loss"] = train_loss
                            acc_bar.set_postfix(postfix_dict)
                        ...
                        print('Activations: ', final_activations)
                        print('Train Accuracy: {:.4f}'.format(train_model[get_acc()]))
                        print('Train Loss: ', train_loss, '\n')
                        
                        return best_weights, model[get_preds()], model[get_acc()], final_activations
