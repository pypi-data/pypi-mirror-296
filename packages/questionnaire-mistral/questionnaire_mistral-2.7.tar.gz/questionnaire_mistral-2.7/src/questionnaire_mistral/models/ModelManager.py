...
class ModelManager:
    ...
    def extract_summaries(self, text: str, document_content: str, chapter_count, chapters):
        """
        Extracts summaries that follow a numbered list format from the given text.

        :param chapter_count: Current count of chapters.
        :param chapters: A dictionary to store chapters and their sections.
        :param document_content: Type of document content to extract (e.g., 'chapters').
        :param text: The text containing the numbered summaries.
        :return: A list of extracted summaries.
        """
        # Regular expression pattern for generic summaries
        pattern = r"Summary: ([^\n]+)\n((?:\s+\* [^\n]+\n)*)"

        if document_content == 'chapters':
            # Regular expression pattern for chapters and sections with the new format
            pattern = r"Chapter: \[([^\n]+)\]\n\s+\* ([^\n]+)\n((?:\s+Section: \[([^\n]+)\]\n\s+\* ([^\n]+)\n)*)"

            # Find all matches based on the new pattern
            matches = re.findall(pattern, text, re.MULTILINE)

            # Process each match
            for match in matches:
                chapter_number = match[0]  # [Chapter number]
                chapter_title = match[1]  # [Chapter name]
                sections_text = match[2]  # Full sections text

                chapters[chapter_title] = []
                chapter_count += 1

                # Extract sections for the chapter
                sections = re.findall(r"Section: \[([^\n]+)\]\n\s+\* ([^\n]+)", sections_text)
                for section_match in sections:
                    section_number = section_match[0]  # [Section number]
                    section_title = section_match[1]  # [Section name]
                    chapters[chapter_title].append(f"Section {section_number}: {section_title}")
                    chapter_count += 1
        else:
            # Generic extraction for non-chapter content
            matches = re.findall(pattern, text, re.MULTILINE)
            for match in matches:
                chapter_title = match[0]  # Summary title
                chapters[chapter_title] = []
                chapter_count += 1
    ...
    def process_rag_result(self, document, DocName, task, **kwargs):
        ...
        if self.return_full_text:
            ...
            replaced_result = result.get('text')
        # Clear memory
        del result
    ...
    def __call__(self, document: str, task: str, document_content: str, task_count: int,
        ...
        def text_generation():
            ...
            self.extract_summaries(replaced_result, document_content, chapter_count, chapters)
            self.chapters[DocName] = chapters
            data = {
                'DocName': DocName,
                'Question': question,
