...
class Abstract(_pro):
    ...
    def __init__(self, *args, **kwds):
        err, s = (InitializedError, "attempt to initialize an abstract class")
        raise err(s)
    def __init_subclass__(cls, *args, **kwds):
        if "_init_sentinel" not in kwds:
            err, s = (InitializedError, "attempt to initialize an abstract class " + cls.__name__)
            raise err(s)
        elif cls.__init__.__code__ != Abstract.__init__.__code__:
            err, s = (LookupError, "cannot remake __init__ function code on " + cls.__name__)
            raise err(s)
