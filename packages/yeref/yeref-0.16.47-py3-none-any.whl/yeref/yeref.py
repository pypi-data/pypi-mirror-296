...
async def create_mem(bot, chat_id, lz, tid, mem_item, username, title, desc, file_photo, MEDIA_D, IMGFLIP_UN, IMGFLIP_PS, mem_type):
    ...
    try:
        ...
        if mem_format == 'static':
            ...
            print(f"{mem_link=}, {mem_top_txt=}, {mem_bottom_txt=}")
            data = {
                'template_id': mem_id,
                'username': IMGFLIP_UN,
                'password': IMGFLIP_PS,
...
async def create_neuro_pack(bot, chat_id, lz, tid, username, title, desc, file_photo, IMGFLIP_UN, IMGFLIP_PS, PROJECT_USERNAME, MEDIA_D, mem_type):
    result = None
    try:
        # region json
        async with aiofiles.open('memes.json', 'r', encoding='utf-8') as file:
        ...
        stickers = []
        if username == '':
            username = f"{random.choice(trg_utms)}_{random.choice(trg_utms)}_{random.randrange(100, 99999)}"
        else:
        ...
        print(f"https://t.me/{mem_type_str}/{username}_by_{PROJECT_USERNAME}")
        ...
        for mem_id, mem_item in memes_data.items():
            ...
            try:
                ...
                try:
                    ...
                    print(e)
                    await asyncio.sleep(round(random.uniform(0, 1), 2))
                    if 'STICKERS_TOO_MUCH' in str(e): return
            finally:
                i += 1
...
async def handle_mem_pack(bot, chat_id, tid, lz, username, title, desc, photo, IMGFLIP_UN, IMGFLIP_PS, PROJECT_USERNAME, MEDIA_D, mem_type):
    file_photo = None
    try:
        if not photo or not desc: return
        fn_ = datetime.datetime.utcnow().strftime(f'%d-%m-%Y_%H-%M-%S-%f-logo.png')
        file_photo = os.path.join(MEDIA_D, str(tid), fn_)

        async with aiohttp.ClientSession() as session:
            async with session.get(photo) as img_response:
                img_content = await img_response.read()

            async with aiofiles.open(file_photo, 'wb') as file:
                await file.write(img_content)
        await create_neuro_pack(bot, chat_id, lz, tid, username, title, desc, file_photo, IMGFLIP_UN, IMGFLIP_PS, PROJECT_USERNAME, MEDIA_D, mem_type)
        # _ = asyncio.get_event_loop().create_task(create_neuro_pack(bot, chat_id, lz, tid, title, username))
    except Exception as e:
        # logger.info(log_ % str(e))
        await asyncio.sleep(round(random.uniform(0, 1), 2))
    finally:
        try:
            if file_photo and os.path.exists(file_photo): os.remove(file_photo)
        except Exception as e:
            # logger.info(log_ % str(e))
            await asyncio.sleep(round(random.uniform(0, 1), 2))
# endregion
