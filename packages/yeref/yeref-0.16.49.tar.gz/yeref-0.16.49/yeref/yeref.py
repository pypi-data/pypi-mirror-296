...
async def create_neuro_pack(bot, chat_id, lz, tid, username, title, desc, file_photo, IMGFLIP_UN, IMGFLIP_PS, PROJECT_USERNAME, MEDIA_D, EXTRA_D, BASE_D, mem_type):
    result = None
    try:
        # region json
...
async def handle_mem_pack(bot, chat_id, tid, lz, username, title, desc, photo, IMGFLIP_UN, IMGFLIP_PS, PROJECT_USERNAME, MEDIA_D, EXTRA_D, BASE_D, mem_type):
    try:
        result = await create_neuro_pack(bot, chat_id, lz, tid, username, title, desc, file_photo, IMGFLIP_UN, IMGFLIP_PS, PROJECT_USERNAME, MEDIA_D, EXTRA_D, BASE_D, mem_type)

        if PROJECT_USERNAME == 'FereyChannelBot' and mem_type == 'regular':
            sql = "UPDATE CHANNEL SET CHANNEL_STICKERPACK=? WHERE CHANNEL_TID=?"
            await db_change(sql, (result,), BASE_D)
        elif PROJECT_USERNAME == 'FereyChannelBot' and mem_type == 'custom_emoji':
