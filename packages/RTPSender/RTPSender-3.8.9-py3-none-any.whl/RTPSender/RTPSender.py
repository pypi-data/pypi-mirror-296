...
class RTPSender:
    def __init__(self, ip_address, port, frame_size, gop=25, hard_encode=False, open_log=False, days=7, stdout=False, log_dir='./logs/', bit_rate=600000):
        ...
        # 如果log_dir不存在，创建log_dir
        if not os.path.exists(log_dir):
            os.makedirs(log_dir)
            if self.open_log:
                logger.info(f"{log_dir} created.")
        ...
        if self.open_log:
            logger.info(f"bit_rate: {self.video_stream.bit_rate}")
        
        if self.open_log:
            logger.info(f"video_stream options: {self.video_stream.options}")
    ...
    def process_video_queue2(self):
        ...
        while not self.stop_event.is_set():
            try:
                ...
                if self.open_log:
                    logger.info(f"len(image_queue2): {self.image_queue2.qsize()}")
            except queue.Empty:
                continue 
            ...
            if self.open_log:
                logger.info(f"No. {self.img_encode_total_cnt} img encode time: {cur_encode_cost_time}ms")
                logger.info(f"img encode avg time: {self.img_encode_total_time / self.img_encode_total_cnt}ms")
            ...
            if self.open_log:
                logger.info(f"No. {self.img_rtp_sent_total_cnt} img rtp send time: {cur_img_send_cost_time}ms")
                logger.info(f"img rtp send avg time: {self.img_rtp_sent_total_time / self.img_rtp_sent_total_cnt}ms")
        ...
        while not self.stop_event.is_set():
            try:
                ...
                if self.open_log:
                    ...
                    logger.info(f"len(audio_queue2): {self.audio_queue2.qsize()}")
            except queue.Empty:
            ...
            if self.open_log:
                logger.info(f"No. {self.audio_rtp_sent_total_cnt} audio rtp send time: {cur_audio_send_cost_time}ms")
                logger.info(f"audio rtp send avg time: {self.audio_rtp_sent_total_time / self.audio_rtp_sent_total_cnt}ms")