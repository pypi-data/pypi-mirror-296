...
class RTPSender:
    def __init__(self, ip_address, port, frame_size, gop=25, hard_encode=False, open_log=False, days=7, stdout=False, log_dir='./logs/'):
    ...
    def process_video_queue2(self):
        ...
        while not self.stop_event.is_set():
            ...
            packets = self.video_stream.encode(img_frame)
            encode_end = time()
            cur_encode_cost_time = encode_end - encode_begin
            self.img_encode_total_time += cur_encode_cost_time
            self.img_encode_total_cnt += 1
            if self.open_log:
                logger.info(f"No. {self.img_encode_total_cnt} img encode time: {cur_encode_cost_time}s")
                logger.info(f"img encode avg time: {self.img_encode_total_time / self.img_encode_total_cnt}s")
            ...
            send_end = time()
            cur_img_send_cost_time = send_end - send_begin
            self.img_rtp_sent_total_time += cur_img_send_cost_time
            self.img_rtp_sent_total_cnt += 1
            if self.open_log:
                logger.info(f"No. {self.img_rtp_sent_total_cnt} img rtp send time: {cur_img_send_cost_time}s")
                logger.info(f"img rtp send avg time: {self.img_rtp_sent_total_time / self.img_rtp_sent_total_cnt}s")
    ...
    def process_audio_queue2(self):
        ...
        while not self.stop_event.is_set():
            ...
            audio_send_end = time()
            cur_audio_send_cost_time = audio_send_end - audio_send_begin
            self.audio_rtp_sent_total_time += cur_audio_send_cost_time
            self.audio_rtp_sent_total_cnt += 1
            if self.open_log:
                logger.info(f"No. {self.audio_rtp_sent_total_cnt} audio rtp send time: {cur_audio_send_cost_time}s")
                logger.info(f"audio rtp send avg time: {self.audio_rtp_sent_total_time / self.audio_rtp_sent_total_cnt}s")