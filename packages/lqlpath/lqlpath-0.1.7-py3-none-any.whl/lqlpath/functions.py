import json
from .bq_res_name_value import bigquery_resource,bigquery_table
...
def contains_line(search_line,file_path):
    #with open(file_path, 'r') as file:
    #with importlib.resources.open_text('lqlpath/data', file_path) as file:
        matched_path=[]
        for line in bigquery_resource.split("\n"):
            #print(line)
...
def bq_get_byKey(resource_type:str,key_search:str):
    ...
    if resource_type.upper()=="BIGQUERY_RESOURCE":
        file_path="BIGQUERY_RESOURCE.txt"
    elif resource_type.upper() == "BIGQUERY_PROJECT":
        file_path = "BIGQUERY_PROJECT.txt"
    elif resource_type.upper() == "BIGQUERY_DATASET":
        file_path = "BIGQUERY_DATASET.txt"
    elif resource_type.upper() == "BIGQUERY_TABLE":
        file_path = "BIGQUERY_TABLE.txt"
    return contains_line(key_search,file_path.lower())
