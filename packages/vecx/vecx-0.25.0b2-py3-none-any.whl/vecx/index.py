...
from .vecx_pb2 import Data, VectorData
...
class Index:
    ...
    def upsert(self, input_array):
        ...
        pb_array = []
        #print("Hello")
        for e in encoded_array:
            #print(type(e), flush=True)
            #print(e)
            vector_data = VectorData(
                id=e["id"],
                filter=json.dumps(e["filter"]),
                meta=e["meta"],
                vector=e["vector"]
            )
            pb_array.append(vector_data)
        pb_data = Data(
                    checksum=checksum,
                    vectors=pb_array
                )
        serialized_data = pb_data.SerializeToString()
        headers = {
            'Authorization': f'{self.token}:{self.name}:{self.checksum}',
            'Content-Type': 'application/octet-stream'
        }
        response = requests.post(f'{self.edge_url}/vector/{self.name}/upsert', headers=headers, data=serialized_data)
        print(response.text)
