...
class DataSystem:
    ...
    def save_data(self, key: any, value: any) -> None:
        """
        Save specified value under a key.
        """

        # Serialize and encrypt the data
        encrypted_data: bytes = self.encrypt_data(value)

        # Save the encrypted data to GitHub
        path: str = f"{key}"
        self.db.write_data(path, encrypted_data, message=f"Saved {key}")

    def load_data(self, key: str) -> None:
