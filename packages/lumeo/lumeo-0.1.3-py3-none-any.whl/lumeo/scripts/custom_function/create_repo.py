...
2. Publish the package to a git repository. If it's a public git repo, skip the token, otherwise use a read-only [personal access token](https://github.com/settings/tokens) with the `repo` scope.
...
{package_name} = Utils.install_import('{package_name}', git_url='https://<token>@github.com/<username>/<repo>.git@<version>')
