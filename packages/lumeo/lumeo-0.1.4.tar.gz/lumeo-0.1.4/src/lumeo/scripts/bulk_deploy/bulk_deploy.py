...
This script uploads media files to Lumeo cloud, (optionally) associates them with a virtual camera,
and queues them for processing. Learn more at https://docs.lumeo.com/docs/universal-bridge
...
Arguments:
    ...
    Source Files (one of pattern, file_list, csv_file, s3_bucket or tag is required):
        ...
        --tag TAG             Tag to apply to uploaded files. Can be tag uuid, tag name or tag path (e.g. "tag1/tag2/tag3").If specified without
                              pattern/file_list/csv_file/s3_bucket, will process existing files with that tag.
    ...
    Deployment Args (applied only when camera not specified):
        ...
        --deployment_config DEPLOYMENT_CONFIG
                              String containing a Deployment config JSON object. Video source in the config will be overridden by source files
                              specified in this script. Ignored if camera_id or camera_external_id specified. Optional.
...
class BulkDeployer:
    ...
    async def run(self):
        ...
        if self._args.tag:
            ...
            try:
                uuid.UUID(self._args.tag)
                tag_id = self._args.tag
            except ValueError:
                tag_id = await self._api_client.get_tag_id_by_path(self._args.tag)
                if not tag_id and (self._args.csv_file or self._args.file_list or self._args.pattern or self._args.s3_bucket):
                    logging.info(f"Tag path '{self._args.tag}' not found. Creating...")
                    tag_id = await self._api_client.create_tag_path(self._args.tag)
                if not tag_id:
                    logging.error(f"Tag path '{self._args.tag}' not found.")
                    return
            self._args.tag = tag_id
    ...
    def _create_args_parser(self):
        parser = argparse.ArgumentParser(description="""Lumeo Bulk Deployer uploads media files to Lumeo cloud, \
