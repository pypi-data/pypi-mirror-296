...
class StressModel:
    ...
    @staticmethod
    def create_learner():
        # doesn't ever finish #svm_classifier = SVC(C=1.0, kernel='rbf', gamma='scale', verbose=True)
        # doesn't ever finish #rf_classifier = RandomForestClassifier(n_estimators=100, max_depth=None, random_state=42, verbose=True)
        ...
        # doesn't ever finish #dt_classifier = DecisionTreeClassifier(max_depth=None, random_state=42)
        # mlp_classifier = MLPClassifier(hidden_layer_sizes=(100,), activation='relu', solver='adam', max_iter=1000, verbose=True)
        lr_classifier = LogisticRegression(penalty='l2', C=1.0, solver='lbfgs', max_iter=1000, verbose=True)
        # ok sgd_classifier = SGDClassifier(loss="log_loss", penalty="l2", shuffle=True, max_iter=500, verbose=True)

        return lr_classifier  #REMEMBER TO CHANGE THE SAVE PATH IN THE RUN CONFIGURATION TO MATCH THE CHOSEN MODEL
