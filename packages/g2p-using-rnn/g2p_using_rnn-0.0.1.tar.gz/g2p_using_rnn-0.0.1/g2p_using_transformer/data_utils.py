...
def load_pronuncation_dictionary(path: str, language: str, prefix: bool = False) -> Dataset:
    ...
    with open(path, 'r', encoding='utf-8') as f:
        for line in f.readlines():
            index, word, pron = line.strip().split('\t')
            # print('Line is -')
            # print(index, word, pron)
            if ',' in pron:
...
def load_all_pronuncation_dictionaries(path: str, prefix: bool = False, mask_prob: float = 0.0,
                                       lower_case: bool = True) -> Dataset:
