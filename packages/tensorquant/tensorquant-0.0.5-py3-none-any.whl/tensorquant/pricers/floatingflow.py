...
class OisCouponDiscounting:
    ...
    def calculate_price(self, term_structure: RateCurve, evaluation_date: date):
        if not self._coupon.has_occurred(evaluation_date):
            ...
            return self.amount(
                term_structure, evaluation_date
...
class FloatingCouponDiscounting:
    ...
    def floating_rate(
        ...
        if self._coupon.fixing_date > evaluation_date:
            ...
            return tf.constant(
                self._coupon.index.fixing(self._coupon.fixing_date), dtype=tf.float64
            )
    ...
    def calculate_price(
        self, disc_curve: RateCurve, est_curve: RateCurve, evaluation_date: date
    ):
        if not self._coupon.has_occurred(evaluation_date):
