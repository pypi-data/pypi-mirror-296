...
template <typename T> void dispatch_cutlass_backward_f16_sm50(T cb, int cc) {
...
template <typename T> void dispatch_cutlass_backward_f32_sm50(T cb, int cc) {
...
void dispatch_cutlass_backward(const ::phi::GPUContext &ctx, T cb) {
    ...
    if (std::is_same<DT, cutlass::half_t>::value && 50 <= cc && cc < 70) {
        dispatch_cutlass_backward_f16_sm50(cb, cc);
    }
    if (std::is_same<DT, float>::value && 50 <= cc && cc < 70) {
        dispatch_cutlass_backward_f32_sm50(cb, cc);
    }
