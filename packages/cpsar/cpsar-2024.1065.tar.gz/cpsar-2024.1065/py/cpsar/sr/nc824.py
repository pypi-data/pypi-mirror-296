...
scan_dir = R.dpath("sr/nc824")
#archive_dir = R.dpath("sr/nc824")
archive_dir = R.dpath("sr/nc824/archive")
error_codes = {
...
class Program(cpsar.shell.Program):
    ...
    def main(self):
        for file in glob("%s/*txt" % scan_dir):
    ...
    def parse(self, data, filepath):
        ...
        while segments.cur[0].strip() == 'OTI':
            ...
            if code[0] == 'I':
                ...
                cursor.execute("""
                    ...
                    WHERE entry_id=%s 
                    """, (segments.cur[3],))
                self._trans_id = cursor.fetchone()
                ...
                cursor.execute("""
                    ...
                    AND ack_code = 'R'
                    """, (segments.cur[3],))
                buf.write(" Transaction ID = %s " % self._trans_id)
                ...
                if cursor.rowcount == 0:
                    ...
                    cursor.execute("""
                        ...
                        SET ack_code=%s,
                            response_date=%s
                        WHERE entry_id=%s
                        """, (code[1], cur_date, self._entry_id))
            ...
            if segments.cur[0] == 'LM': 
                ...
                while segments.cur[0] == 'LQ':
                    ...
                    while segments.cur[0] == 'RED':
                        ...
                        # Some error codes require other than default behaviors
                        self._resolve_conditional_errors(field)
        ...
        if not self.opts.no_email:
            send_email(msg, 'TX DWC 824 Message: %s' % filename, config.customer_service_email())
        else:
            print(msg)
    ...
    def _resolve_conditional_errors(self, field):
        """Some error codes require special actions to fully resolve
        """
        cursor = R.db.cursor()
        self._field = field
        ...
        """Invalid NDC"""
        if self._field == 721:
            print(self._trans_id)
            cursor.execute("""
                UPDATE history
                    set temp_sr_ndc_flag=TRUE
                FROM trans
