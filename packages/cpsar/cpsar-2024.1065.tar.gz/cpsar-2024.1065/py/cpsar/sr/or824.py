...
scan_dir = R.dpath("sr/or824")
archive_dir = R.dpath("sr/or824/archive")
error_codes = {
...
class Program(cpsar.shell.Program):
    ...
    def main(self):
        for file in glob("%s/*824" % scan_dir):
    ...
    def parse(self, data, filepath):
        ...
        if not self.opts.no_email:
            send_email(msg, 'OR WCD 824 Message: %s' % filename, [config.customer_service_email()])
        else:
            print(msg)
