...
'_testing allows for dme billing'
    ...
    def write_segments(self, writer):
        ...
        writer.write_segment([
            ...
            'CX',               # NM101: Entity Identifier Code
                                #        (Billing Provider)
            '2',                # NM102: Entity Type Qualifier (Company)
            client.claim_admin_name.strip(), # NM103: Last Name or Organization Name
            '', '', '', '',     # NM104-NM107: Blank
            'FI',               # NM108: Identification Code Qualifier (FEIN)
    ...
    def write_segments(self, writer, parent_hlevel):
        ...
        if self._testing:
            for item in self._employer_items:
                if (item.entry.entry_id == 4017
                    or item.entry.entry_id == 4018
                    or item.entry.entry_id == 4019):
                    service_item = DMEServiceItem(item)
                elif(item.entry.entry_id == 4020
                    or item.entry.entry_id == 4021
                    or item.entry.entry_id == 4022):
...
class ServiceItem(object):
    ...
    def write_segments(self, writer, parent_hlevel):
        ...
        # Patient Address
        if patient.address_1 or patient.address_2:
            writer.write_segment([
                'N3',
        ...
        writer.write_segment([
            ...
            '',                 # NM106: Blank
            '',                 # NM107: Suffix
            pharmacy_tax_idc,   # NM108: Identification Code Qualifier
            pharmacy_tax_id,    # NM109: Identification Code
            '',                 # NM110: Entity Relationship Code
            ''                  # NM111: Gateway Provider
        ])
        ...
        ## IF we don't have an NPI we'll use dea as special license number (GF)
        doc_identifier = history.doctor_npi_number
        doc_qualifier = '1G'
        if not doc_identifier:
            doc_identifier = history.doctor_dea_number
            doc_qualifier = 'GF'

        writer.write_segment([
            'REF',
            doc_qualifier,       # 01: Reference Identification Qualifier
            doc_identifier 
        ])
        ...
        writer.write_segment([
            ...
            history.days_supply  # DN 571 QTY02: Days Supply
        ])
        writer.write_segment([
            'QTY',
            'QB',               # QTY01: Quantity Qualifier
            history.quantity    # DN 570 QTY02: Days Supply
        ])
