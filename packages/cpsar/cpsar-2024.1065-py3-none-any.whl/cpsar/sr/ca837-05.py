...
def create_837_file():
    ...
    xfile = X12File(items, file_date, working_env, control_number)
    ...
    cursor = db.cursor()
    for item in items:
        #IF file_id is not none, then the record is a cancel
...
class X12File(object):
    def __init__(self, items, file_date, usage_indicator, control_number):
        self._items = items
    ...
    @property
    def path(self):
        return dpath("sr/ca837", self.name)

    @property
    def file_date(self):
        return self._file_date.strftime('%y%m%d%H%M%S')
...
class ISA(object):
    ...
    def write_segments(self, writer):
        ...
        writer.write_segment([
            ...
            'ZZ',               # Interchange ID Qualifier
            '943160882      ',  # Interchange Receiver ID
        ...
        writer.write_segment([
            ...
            '943160882',        # Application Receiver's Code
        ...
        writer.write_segment([
            ...
            xfile.control_number,     # Originator Transaction Identifier
                                #      (DN532 Batch Control Number)
            xfile.yyyymmdd,     # Transaction Set Creation Date
            xfile.hhmm          # Transaction Set Creation Time
        ])
        ...
        writer.write_segment([
            'NM1',
            '10',               # NM101: Entity Identifier Code
        ...
        writer.write_segment([
            ...
            '', '', '', '', '', '', # NM103-NM107 Blank
            'FI',               # NM108: Identification Code Qualifier (FEIN)
            '943160882'         # NM109: Identification Code 
        ])
        ## POSTAL CODE
        writer.write_segment([
            'N4',
            '', '',             # N401-N402: Blank
            '941420603'         # N403: Postal Code
        ])
...
class Carrier(object):
    ...
    def write_segments(self, writer):
        ...
        writer.write_segment([
            ...
            client.client_name, # NM103: Last Name or Organization Name
            '', '', '', '',     # NM104-NM107: Blank
            'FI',               # NM108: Identification Code Qualifier (FEIN)
            client.tin          # NM109: Identification Code
        ])

        ## Client Information - Zip Code
        """
        writer.write_segment([
            'N4',
            '', '',             # N401-N402: Blank
            client.zip_code     # N403: Postal Code
        ])

        ## Client Information (Insurer / Self-Insured) - Carrier
        writer.write_segment([
            'NM1',
...
class Employer(object):
    ...
    def write_segments(self, writer, parent_hlevel):
        ...
        #///////////////////////////////////////////////////////////////////////
        # Employer Hierarchical Information ( Repeat > 1)   
        writer.write_hl_segment(parent_hlevel, 'EM', '1')

        #///////////////////////////////////////////////////////////////////////
        # Employer Named Insured Information (Insured)      
        writer.write_segment([
            'NM1',
        ...
        ## Employer's Secondary Identification
        if employer.policy_number:
            writer.write_segment([
                'REF',
                'IG',                  # REF01: Reference Identification
                                       #        Qualifier
                                       #        (Insurance Policy Number)
                employer.policy_number # REF02: Reference Identification
            ])

        ## Employer's Contact Number
        if employer.phone.strip():
            writer.write_segment([
                'PER',
                'IC',                  # PER01: Contact Function Code (Information Contact)
                '',
                'WP',                  # PER02: Communication Number Qualifier
                                       #        (Work Phone Number)
                employer.phone     # PER03: Communication Number
            ])
        """ 
        my_hlevel = writer.hlevel
        ...
        if self._testing:
            for item in self._employer_items:
                if item.patient.patient_id == 91903 or item.patient.patient_id == 102071:
                    service_item = DMEServiceItem(item)
                elif item.patient.patient_id == 109640 or item.patient.patient_id == 72234:
                    service_item = MOServiceItem(item)
                else:
...
class ServiceItem(object):
    ...
    def write_segments(self, writer, parent_hlevel):
        ...
        if patient.address_1:
            writer.write_segment([
                'N3',
                patient.address_1,   # N301: Employee Mailing Primary Address
                patient.address_2    # N302: Employee Mailing Secondary Address
            ])
        ...
        writer.write_segment([
            ...
            patient.dob,        # DMG02: Birth Date
            patient.sex,        # DMG03: Geneder Code (F/M/U)
            'U',                # DMG04: Marital Status Code (I/K/M/S/U)
        ])
        """
        ...
        writer.write_segment([
            ...
            trans.total,        # CLM02: Monetary Amount (Total Charge Per Bill)
            '',                 # CLM03: Blank
            type_code,               # CLM04: Non-Institutional Claim Type Code
                                #        (DM/MO/RX
            '99:B',             # CLM05: Facility Code Value ( Facility Code or
            ...
            '', '',             # CLM17-CLM18: Blank
            '05'   # CLM19: Claims Submission Reason Codes
        ])
        ...
        writer.write_segment([
            ...
            '',                 # NM106: Blank
            '',                 # NM107: Suffix
            'FI',               # NM108: Identification Code Qualifier
            '631040950',        # NM109: Identification Code
        ...
        writer.write_segment([
            ...
            '',                 # NM107: Suffix
            'FI',               # NM108: Identification Code Qualifier
            pharmacy.tax_id,    # NM109: Identification Code
            '',                 # NM110: Entity Relationship Code
            ''                  # NM111: Gateway Provider
        ])

        ## RENDERING BILL PROVIDER CHARACTERISTICS
        writer.write_segment([
            'PRV',
            'PE',
            'S3',
            '333600000X'        # 01: DN651 RENDERING BILL PROVIDER PRIMARY
                                #           SPECIALTY CODE
                                # We put in the generic pharmacy taxonomy code
        ])

        ## RENDERING BILL PROVIDER GEOGRAPHICAL LOCATION
        writer.write_segment([
            'N4',
            pharmacy.city,      # 01: DN654 RENDERING BILL PROVIDER CITY
            pharmacy.state,     # 02: DN655 RENDERING BILL PROVIDER STATE CODE
            pharmacy.zip_code,     # 03: DN656 RENDERING BILL PROVIDER POSTAL CODE
        ])
        ...
        writer.write_segment([
            'REF',
            '0B',               # 01:       Qualifier
            '999999999'         # 02:       # DN643 State license number
                                #           We don't have this can give all
                                #           9's
        ])

        writer.write_segment([
            'REF',
            '1G',               # 01:       Qualifier (say to use NPI)
            '1437194933'      # 02:       NPI # DN647
        ])
        ...
        writer.write_segment([
            ...
            '34',               # 10: Entity Relationship Code
            'GP'                # 11: Gateway Provider
        ])

        ## Referring Provider's Secondary Identification Number
        writer.write_segment([
            'REF',
            '1G',               # 01: Reference Identification Qualifier
            history.doctor_npi_number 
        ])

        ## LOOP ID - 2320 BILL ADJUST REASONS
        ## SUBSCRIBER INFORMATION
        #XXX: No idea what this is. It was in the sample file.
        writer.write_segment([
            'SBR',
            'P'                 # 01 Payer Responsibility Sequence #
        ])
        #///////////////////////////////////////////////////////////////////////
        ...
        writer.write_segment([
            ...
            'D8',               # DTP02: Date Time Period Format Qualifier
            history._rx_date_yyyymmdd     # DTP03: Service Date(s)
        ])
        ## Quantity
        writer.write_segment([
            'QTY',
        ...
        writer.write_segment([
            ...
            "ND:%s" % drug.ndc_number,  # SVD03: Composite Medical Procedure Identifier
        ])

        # When we have a write off, we have to have a CAS line for the
        # write off
        writer.write_segment([
            'CAS',
    ...
    def _write_SV_segment(self, writer):
        ...
        writer.write_segment([
            ...
            "ND:%s" % drug.ndc_number,      # SV402: Composite Medical Procedure Identifier
            '', '',                         # SV403-SV404: Blank
...
class DMEServiceItem(ServiceItem):
    ...
    def _write_SV_segment(self, writer):
        ...
        writer.write_segment([
            'SV5',
            'HC:K0021:RR',      # SV501 Composite Medical Procedure Identifier 
            'UN',               # SV502 Unit or Basis for Measurement Code
            '3',                # SV503 Quantity
            '24.55',            # SV504 Monetary Amount
