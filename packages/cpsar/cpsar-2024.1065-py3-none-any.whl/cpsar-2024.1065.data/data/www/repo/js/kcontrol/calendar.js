...
function handle_date_blur(elem, format) {
	value = elem.value;
	...
	elem.value = formatDate(d, format);
	return 1;
}
...
function spinner_down(elem) {
    set_spinner_value(elem, 0);
}

function spinner_up(elem) {
    set_spinner_value(elem, 1);
}

function set_spinner_value(elem, offset) {
	if(!document.getElementById(elem).value || document.getElementById(elem).value == '0000-00-00') {
		tdate = new Date();
	} else {
		var tdate = parseDate(document.getElementById(elem).value);
	}
	if(!tdate)
		return;
	var milliseconds = Date.UTC(y2k(tdate.getYear()), tdate.getMonth(), tdate.getDate());
	if(offset)
