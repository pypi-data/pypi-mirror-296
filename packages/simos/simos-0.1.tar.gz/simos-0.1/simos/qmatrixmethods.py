...
def directsum(operator, pre : int, post : int):
    """ Direct sum of an operator with empty matrices of specified dimensions.
    
    :param operator: An operator.
    :param int pre: An integer specifying how many dimensions will be added before the operaor.
    :param int post: An integer specifying how many dimensions will be added after the operaor.
    :return: Direct product of the operators.
    """       

    backend = backends.get_backend(operator)
    return getattr(getattr(backends,backend), 'directsum')(operator, pre, post)
...
def operator_to_vector(op):
    ...
    backend = backends.get_backend(op)
    op = tidyup(op, method = backend)
    return getattr(getattr(backends,backend), 'operator_to_vector')(op)
...
def vector_to_operator(vec):
    ...
    return getattr(getattr(backends,backend), 'vector_to_operator')(vec)
...
def focker_to_dm(v,is_hilbert,dims=None,weights=None):
    """Converts a fock state vector to a density matrix."""
    backend = backends.get_backend(v)
    v = tidyup(v, method = backend)
    return getattr(getattr(backends,backend), 'focker_to_dm')(v,is_hilbert,dims,weights)

def dm_to_focker(rho,dof,try_ket=True):
    """Converts a density matrix to a fock state vector.
    
    :param rho: Density matrix, state vector or superoperator ket.
    :param int dof: Number of operators (degrees of freedom).
    :param bool try_ket: If True, the function will attempt to convert the density matrix to a ket vector.
    """

    backend = backends.get_backend(rho)
    rho = tidyup(rho, method = backend)
    return getattr(getattr(backends,backend), 'dm_to_focker')(rho,dof,try_ket)