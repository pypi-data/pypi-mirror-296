...
class StatefulProjection(keras.layers.Dense):
    ...
    def build(self, input_shape):
        if self.downshape is None:
            ...
            # When downshape is enabled, build the layer with the target shape so that variables are
            # build properly
            super().build(self.downshape)
            # Edit the input_spec so that from a graph point of view, this layer sees the original
            # input shape
