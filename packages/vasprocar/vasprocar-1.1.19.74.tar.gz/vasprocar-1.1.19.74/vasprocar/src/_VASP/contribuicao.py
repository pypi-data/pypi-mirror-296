        ...
        for point_k in range(1, (nk+1)):                       
            ...
            for Band_n in range (1, (int(nb/ispin) +1)):
                ...
                if ((total != 0) and (penetration_plot == 1) and (len(vector_length) >0)):
                   # if ((bands_sn[Band] == 'sim') and (points_sn[point_k] == 'sim')):
                   p_length[n_point_k][Band] = abs( max(vector_length) -min(vector_length) )
                #---------------------------------------------------------------------------
      ...
      for i in range(1, (nk +1)):
          ...
          for j in range (1, (nb+1)):
              if ((points_sn[i] == 'sim') and (bands_sn[j] == 'sim')):          
                 if (j == bands_i): p_file.write(f'{separacao[i]} ')
...
ax.set_box_aspect(1.0/3.0)
    
plt.savefig(dir_output + 'penetration_length.png', dpi = 600, bbox_inches='tight', pad_inches=0)
# plt.show()
