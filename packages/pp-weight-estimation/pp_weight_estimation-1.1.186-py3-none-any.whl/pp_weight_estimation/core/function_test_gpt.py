...
def process_pipeline_new(config_path: str, logger: logging.Logger = None, **kwargs) -> Union[pd.DataFrame, List]:
    ...
    openai_detail = config['gpt_res'].get('openai_detail', 'high')
        ...
        if new_ref_list is None:
            ...
            if len(new_ref_list) > 1:
                ...
                if resize_for_multi_ref:
                    ...
                    multi_ref_res = multi_ref_prompt_openai(local_image_path_updated, new_ref_list_updated, prompt=prompt_multi_ref, api_key=gpt_api_key,detail=openai_detail)
                else:
                    multi_ref_res = multi_ref_prompt_openai(local_image_path, new_ref_list, prompt=prompt_multi_ref, api_key=gpt_api_key,detail=openai_detail)
            ...
            if local_image_path and site_id and food_item:
                ...
                new_csv = pd.DataFrame(entries,columns=['site_id','local_image_path','food_item','local_image_folder','image_dict_name','s3_image_path',
                                                        'reference_image_s3','reference_image_local','reference_image_gt','rework_g','food_item_label'])
    ...
    #results = []
    for index, (site_id, local_image_path, food_item,local_image_folder,image_dict_name,s3_image_path,reference_image_s3,reference_image_local,reference_image_gt,rework_g,
        ...
        if annotation_data_res:
            ...
            if var_img and var_ref_img:
                try:
                    ...
                    ref_image_data = np.array(Image.open(reference_image_local))
                    ...
                    input_df.at[index, 'reference_image_path'] = reference_image_local
