...
def get_country_specs(country):
    ...
    if (country[0] not in string.ascii_letters or
        country[1] not in string.ascii_letters):
        raise IBANError(
            'Invalid IBAN: Invalid Country ISO Code (must be letters)')
    
    try:
        country_specs = dict(cfg_countries.items(country))
    except configparser.NoSectionError:
        # No configuration for this country
        raise IBANError('Invalid IBAN: Country %s not implemented' % country)
    ...
    f = country_specs.get('checksum', False)
    if isinstance(f, str):
        if f.lower() in ['y','yes']:
            f = True
        else:
            f = False
    country_specs['checksum'] = f
...
class IntAccount(object):
    ...
    def set_iban(self, iban):
        ...
        # Validate IBAN checksum
        if checksum(iban) != 1 and self.country_specs['checksum']:
...
def valid_set(attr):
    ...
    def setx(self, value):
        ...
        # FIXME: remove everything except 0-9A-Za-z for all attributes
        ...
        if value is None:
            ...
            attr_spec = self.country_specs[attr]
            if attr_spec is None:
                raise AttributeError("Invalid value: %s / %s" % (
                    value, type(value)))
            value = attr_spec.minfill(value)
