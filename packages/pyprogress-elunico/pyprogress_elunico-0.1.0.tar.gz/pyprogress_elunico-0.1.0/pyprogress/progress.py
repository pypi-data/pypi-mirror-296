...
class ProgressBar:
    ...
    def done(self, message: str) -> 'ProgressBar':
...
listed = os.listdir('/Users/thomaspovinelli')
bar = ProgressBar(0, len(listed), True)
bar.set_message("Reading files...")
for i in listed:
    # print("File: ", i)
    # bar.iterbegin("Processing file: {}...".format(i[0:10]))
    with bar:
        time.sleep(random.random() / 6)
        if not i.startswith('D'):  # i == 'Downloads':
