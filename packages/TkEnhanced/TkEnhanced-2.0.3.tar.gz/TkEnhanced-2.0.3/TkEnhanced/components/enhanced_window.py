...
class MotionUtils:
    ...
    def start_move(self, event: Event) -> None:
        ...
        self.start_x = event.x % self.master.master.winfo_width()
        self.start_y = event.y % self.master.master.winfo_height()
    ...
    def on_move(self, event: Event) -> None:
        ...
        self.master._windows_api.SetWindowPos(window_handle, None, new_x, new_y, width, height, 0)

    def stop_move(self, event: Event) -> None:
        self.start_x = self.start_y = None

    def toggle_maximize(self, event: Event) -> None:
        new_state: str = "zoomed" if self.master.master.wm_state() == "normal" else "normal"
        self.master.master.wm_state(new_state)
...
class WindowUtils:
    ...
    def bind_motion(self, widget: Misc) -> None:
        ...
        widget.bind(sequence="<Double-Button-1>", func=self._motion_utils.toggle_maximize, add=True)
