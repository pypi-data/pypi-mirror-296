...
class EnhancedImage(TransparentMisc, Label):
    ...
    def get_image_size(enhanced_image: "EnhancedImage", image: Image) -> tuple[int, int]:
        ...
        new_image_width: int = int(image_width * scaling_factor) if enhanced_image._width == Constants.AUTO else enhanced_image._width
        new_image_height: int = int(image_height * scaling_factor)-4 if enhanced_image._height == Constants.AUTO else enhanced_image._height
    ...
    def resize_image(self) -> None:
        if self._image is not None:
            new_size: tuple[int, int] = self.get_image_size(self, image=self._image)
            resized_image: Image = self._image.resize(size=new_size)
