...
class IpCoreView(DraggableGraphicsView):
    ...
    def render_ipcore(self, ipcore: IpCore):
        ...
        body_height = 2 * self.margin + max(left_ports_height, right_ports_height) + max_bottom_length + 2 * self._line_width + (self._arrow_size + self._arrow_margin) * has_bottom
        ...
        # draw header name
        self.text(ipcore.name, (body_width - name_width) / 2 + self.margin + self._arrow_margin + self._arrow_size, -self.font_height - self._font_margin, self._font, Qt.black)
        # draw body rect
        self.rectangle(self._arrow_size + self._arrow_margin + self.margin, 0, br_width, br_height, Qt.black, self.line_width)
        ...
        for i in range(len(left_ports)):
            if left_ports[i] is not None:
                ...
                if left_ports[i]['direction'] in ['input', 'inout']:
                    self.arrow(self.margin, self.margin + _y_bias + lslst[i] / 2, self._arrow_size + self.margin, self.margin + _y_bias + lslst[i] / 2, Qt.black, self.line_width, self.line_width + 10)
                if left_ports[i]['direction'] in ['output', 'inout']:
                    self.arrow(self._arrow_size + self.margin, self.margin + _y_bias + lslst[i] / 2, self.margin, self.margin + _y_bias + lslst[i] / 2, Qt.black, self.line_width, self.line_width + 10)
        ...
        for i in range(len(right_ports)):
            if right_ports[i] is not None:
                ...
                if right_ports[i]['direction'] in ['output', 'inout']:
                    self.arrow(body_width - self._arrow_size - self.margin, self.margin + _y_bias + rslst[i] / 2, body_width - self.margin, self.margin + _y_bias + rslst[i] / 2, Qt.black, self.line_width, self.line_width + 10)
                if right_ports[i]['direction'] in ['input', 'inout']:
                    self.arrow(body_width - self.margin, self.margin + _y_bias + rslst[i] / 2, body_width - self._arrow_size - self.margin, self.margin + _y_bias + rslst[i] / 2, Qt.black, self.line_width, self.line_width + 10)
