...
def extract_corva_attributes(event: Any):
    corva_attrs = {
        corva_semattrs.CORVA_ASSET_ID: _extract_attribute(event, "asset_id"),
        corva_semattrs.CORVA_ASSET_TYPE: _extract_attribute(event, "asset_type"),
        corva_semattrs.CORVA_ASSET_NAME: _extract_attribute(event, "asset_name"),
        corva_semattrs.CORVA_COMPANY_NAME: _extract_attribute(event, "company"),
        corva_semattrs.CORVA_COMPANY_ID: _extract_attribute(event, "company_id"),
...
def _set_api_gateway_v2_proxy_attributes(
    ...
    if lambda_event["requestContext"].get("http"):
        if "method" in lambda_event["requestContext"]["http"]:
            span.set_attribute(
                Spancorva_semattrs.HTTP_METHOD,
    ...
    def wrapper(wrapped, instance, args, kwargs):
        ...
        with tracer.start_as_current_span(
            ...
            if span.is_recording():
                span.set_attribute(
                    Spancorva_semattrs.FAAS_EXECUTION,
                    getattr(lambda_context, "aws_request_id", ""),
                )

                span.set_attribute(
                    Resourcecorva_semattrs.FAAS_ID,
                    getattr(lambda_context, "invoked_function_arn", ""),
                )

                span.set_attribute(
                    Resourcecorva_semattrs.CLOUD_ACCOUNT_ID,
                    get_account_id(
                        getattr(lambda_context, "invoked_function_arn", "")
                    ),
                )
            ...
            if isinstance(lambda_event, dict) and lambda_event.get(
                ...
                span.set_attribute(Spancorva_semattrs.FAAS_TRIGGER, "http")
                ...
                if isinstance(result, dict) and result.get("statusCode"):
                    span.set_attribute(
                        Spancorva_semattrs.HTTP_STATUS_CODE,
