...
from mircato.configs import set_num_threads
...
def main(
    ...
    batches = list(array_split(nifti_files, num_workers))
    ...
    with ProcessPoolExecutor(max_workers=num_workers) as executor:
        executor.map(batch_processor, batches)


def process_batch(
    nifti_files: list[str], task_list: list[str], num_threads: int, mark_complete: bool
):
    """Function to process a batch of nifti files
    Parameters
    ----------
    nifti_files : list[str]
        The list of nifti files to process
    ...
    """
    nifti_stats = partial(
        single_nifti_stats, task_list=task_list, mark_complete=mark_complete
    )
    with ThreadPoolExecutor(max_workers=num_threads) as executor:
        executor.map(nifti_stats, nifti_files)
