...
from mircato.configs.logging import timer
...
class DicomFolder(Path):
    ...
    def _check_folder(self) -> None:
        ...
        dicom_files = list(self.glob("*.dcm"))
        if not dicom_files:
            dicom_files = list(self.glob("*"))
    ...
    def _convert(self, nifti_dir: Path) -> None:
        ...
        dicom2nifti.convert_directory(self, nifti_dir, compression=True, reorient=True)
    ...
    def convert_to_nifti(self, output_dir: Path, only_ax: bool, no_mip: bool) -> None:
        ...
        try:
            ...
            if only_ax and self.ref_data["ct_direction"] != "AX":
                ...
                logger.error(
                    f"{self} is not an axial scan and only_ax = True, will not convert",
                    extra=extra_dict,
                )
                return
            if no_mip and self.ref_data["is_mip"]:
                ...
                logger.error(
                    f"{self} is likely a MIP scan and no_mip = True, will not convert",
                    extra=extra_dict,
                )
                return
            ...
            logger.success(
                f"{self} converted to nifti in {convert_time:.2f} seconds. Saved to {nifti_dir}",
                extra=extra_dict,
            )
        except NotADicomFolderError:
            extra_dict["failed_reason"] = "not_a_directory"
            logger.error(f"{self} is not a directory", extra=extra_dict)
        except InsufficientDicomFilesError:
            extra_dict["failed_reason"] = "too_few_files"
            logger.error(
                f"Less than {MIN_FILE_COUNT} DICOM files found in {self}",
                extra=extra_dict,
            )
        except IncompleteDicomReferenceError:
            extra_dict["failed_reason"] = "incomplete_reference"
            logger.error(
                f"Dicom reference data is incomplete for {self}", extra=extra_dict
            )

        except SameFileError as e:
            extra_dict["failed_reason"] = "same_file_error"
            logger.warning(
                f"Nifti file already exists for {self}, will not convert. Error: {str(e)}",
                extra=extra_dict,
            )
        except dicom2nifti.exceptions.ConversionValidationError:
            extra_dict["failed_reason"] = "conversion_validation_error"
            logger.error(
                f"Dicom conversion could not be validated for {self}", extra=extra_dict
            )
        except PermissionError as e:
            extra_dict["failed_reason"] = "permission_error"
            logger.error(
                f"Dicom folder {self} could not be accessed due to a permission error {str(e)}",
                extra=extra_dict,
            )
