...
def generate_checklist(sections, file_format):
    ...
    for section in sections:
        generate = SECTION_FUNCTIONS.get(section)
        if generate:
            # Split the markdown into lines and extract the list items
            lines = generate().split('\n')
            section_title = lines[0].strip('# ').strip()
            items = [line.strip('- [ ] ').strip() for line in lines[1:] if line.startswith('- [ ]')]
            checklist_dict[section_title] = items
        else:
            print(f"Warning: Section '{section}' is not recognized and will be skipped.", file=sys.stderr)
...
def main():
    parser = argparse.ArgumentParser(
        description='Generate a responsible AI checklist file for LLM-based data science projects.'
    )
    ...
    parser.add_argument(
        ...
        help='The output file name with appropriate extension (default: responsible_ai_checklist_llm.md)'
    )
    ...
    # File format argument (supports md, yaml, json)
    parser.add_argument(
        '-f', '--format',
        choices=['md', 'yaml', 'json'],
        default='md',
        help='Specify the output format (supports md, yaml, json)'
    )
    
    args = parser.parse_args()

    # Adjust default output extension based on format if not specified
