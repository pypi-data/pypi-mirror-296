...
def user_decision_on_suggestions(detailed_patient_data, config):
    ...
    although the user decision is persisting. Consider use of class in the future.
    """
    ...
    if proceed:
        # Confirm all suggested endpoints and update 'suggested_endpoint' accordingly
        confirmed_data = MediLink_DataMgmt.confirm_all_suggested_endpoints(detailed_patient_data)
        for data in confirmed_data:
            # Ensure 'suggested_endpoint' matches the confirmed endpoint
            data['suggested_endpoint'] = data.get('confirmed_endpoint', data.get('suggested_endpoint', 'AVAILITY'))
...
def select_and_adjust_files(detailed_patient_data, config):
    ...
    BUG (Med suggested_endpoint) After the user is done making their selection, 
    the suggested_endpoint should update to persist the user selection as priority over its original suggestion.
    Consider Crosswalk persistence in the future.
    """
    ...
    for i in selected_indices:
        ...
        if endpoint_change == 'y':
            ...
            if 0 <= endpoint_index < len(endpoint_keys):
                ...
                selected_endpoint_name = config['MediLink_Config']['endpoints'][selected_endpoint_key]['name']
                data['confirmed_endpoint'] = selected_endpoint_key
                data['suggested_endpoint'] = selected_endpoint_key  # Update suggested_endpoint here
                print("Endpoint changed to {0} for patient {1}.".format(selected_endpoint_name, data['patient_name']))
                
                # Crosswalk persistence is omitted as per current requirements
            else:
                print("Invalid selection. Keeping the suggested endpoint.")
        else:
            # If user does not want to change, ensure 'suggested_endpoint' matches 'confirmed_endpoint' if available
            data['suggested_endpoint'] = data.get('confirmed_endpoint', data.get('suggested_endpoint', 'AVAILITY'))
    ...
    if confirmed_data:  # Proceed if there are confirmed data entries.
        ...
        if MediLink_Up.confirm_transmission(organized_data):
            if MediLink_Up.check_internet_connection():
                # Submit claims (including printing receipt) if internet connectivity is confirmed.
