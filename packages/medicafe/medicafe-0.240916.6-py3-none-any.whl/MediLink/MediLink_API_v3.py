...
class APIClient(BaseAPIClient):
    ...
    def make_api_call(self, endpoint_name, call_type, url_extension="", params=None, data=None, headers=None):
        ...
        if token:
            MediLink_ConfigLoader.log("[Make API Call] Token obtained for {}".format(endpoint_name), level="INFO")
        else:
            MediLink_ConfigLoader.log("[Make API Call] No token obtained for {}".format(endpoint_name), level="ERROR")
            # Optionally, you might want to raise an exception or handle the missing token here
        ...
        try:
            ...
            # Extract status code and response content
            status_code = http_err.response.status_code if http_err.response else 'No Status Code'
            response_content = http_err.response.text if http_err.response else 'No Response Content'
            log_message = (
            ...
            # Handle other requests exceptions (e.g., connection errors, timeouts)
            log_message = (
