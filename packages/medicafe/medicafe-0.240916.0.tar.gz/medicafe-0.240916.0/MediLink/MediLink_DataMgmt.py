...
def load_and_validate_config(config):
    ...
    if not config:
        MediLink_ConfigLoader.log("No config passed. Reloading config...", level="WARNING")
...
def validate_config(config):
    """Ensure essential configuration keys are present."""
    if not config or 'MediLink_Config' not in config:
        MediLink_ConfigLoader.log("Failed to load MediLink configuration.", level="ERROR")
        raise RuntimeError("Config is None or missing 'MediLink_Config'.")
    for key in ['endpoints', 'local_storage_path']:
        if key not in config['MediLink_Config']:
            MediLink_ConfigLoader.log("Config missing required key: {}".format(key), level="ERROR")
            raise ValueError("Config missing required key: {}".format(key))
