...
def format_date(value):
    try:
        ...
        MediLink_ConfigLoader.log("Date format error: {}".format(e))
        return value
...
def format_phone(value):
    ...
    if len(digits) == 10:
        return digits
    MediLink_ConfigLoader.log("Phone number format error: Invalid number of digits")
    return value
    ...
    if len(temp_output) > 30:
        ...
        while len(temp_output) > 30:
            ...
            if last_space_index == -1:
                break
            # Remove the last space and the following 6 characters as per original logic
    ...
    if ',' in value:
        try:
            ...
            if not common_cities:
                MediLink_ConfigLoader.log("No common cities configured.")
            ...
            if city_match:
                ...
                if match:
                    ...
                    parsed_address_components['Zip Code'] = match.group('Zip') or ''
                ...
                if match:
                    # Update parsed address components
                    parsed_address_components['City'] = match.group('City').strip()
                    parsed_address_components['State'] = match.group('State')
            ...
            MediLink_ConfigLoader.log("Address format error: Unable to parse address '{}'. Error: {}".format(value, e), level="ERROR")
...
# Mapping of Medisoft fields to their respective formatting functions
FORMATTERS = {
...
def format_data(medisoft_field, value, csv_data, reverse_mapping, parsed_address_components):
    formatter = FORMATTERS.get(medisoft_field, lambda x: str(x))
    if medisoft_field == 'Street':
        formatted_value = formatter(value, csv_data, reverse_mapping, parsed_address_components)
    else:
