...
from ._utils import get_mock_source_attrs, is_mock_source, set_serialization
...
def source_as_knowledge(
    ...
    ingestion_environment: str = _DEFAULT_BATCH_FEATURE_VIEW_ENVIRONMENT,
    ...
    Args:
        ...
        ingestion_environment: The ingestion environment.
...
"""
    ...
    @tool(name=_SEARCH_TOOL_PREFIX + name, descripton=desc, subtype="search", **skwargs)
...
def _assert_not_null_or_get_from_mock(obj, source, attr_name):
    ...
    if is_mock_source(source):
        mo = get_mock_source_attrs(source)
        if attr_name in mo:
            return mo[attr_name]
