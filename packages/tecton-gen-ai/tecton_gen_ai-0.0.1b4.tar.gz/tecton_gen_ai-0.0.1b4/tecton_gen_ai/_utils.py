...
from tecton_gen_ai.utils.tecton import make_request_source, set_conf

from .constants import _TECTON_MOCK_OBJECT_ATTR
from .utils.extra_attrs import get_attr, has_attr, set_attr

_REQUEST = make_request_source(name=str, input=str)


def is_mock_source(source: Any) -> bool:
    src = _get_orig_source(source)
    return has_attr(src, _TECTON_MOCK_OBJECT_ATTR)


def get_mock_source_attrs(sourcre: Any) -> Dict[str, Any]:
    src = _get_orig_source(sourcre)
    return get_attr(src, _TECTON_MOCK_OBJECT_ATTR)


def set_mock_source_attrs(source: Any, attrs: Dict[str, Any]) -> None:
    src = _get_orig_source(source)
