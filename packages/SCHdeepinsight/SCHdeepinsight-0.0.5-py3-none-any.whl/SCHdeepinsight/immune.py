...
class Immune:
    ...
    def batch_correction(self, input_file, ref_file):
        ...
        # Write barcodes.tsv
        barcodes_path = self.output_prefix / 'barcodes.tsv'
        try:
        ...
        # Write features.tsv
        features_path = self.output_prefix / 'features.tsv'
        try:
        ...
        # Write matrix.mtx
        matrix_path = self.output_prefix / 'matrix.mtx'
        try:
            sc.write_mtx(matrix_path, query)
            # Gzip the matrix files using gzip library
            with open(matrix_path, 'rb') as f_in:
                with gzip.open(f'{matrix_path}.gz', 'wb') as f_out:
                    f_out.writelines(f_in)
        except Exception as e:
            raise RuntimeError(f"Error writing or gzipping matrix to {matrix_path}: {str(e)}")
        ...
        batch_projected_query_path = self.output_prefix / "batch_projected_query.h5ad"
        return batch_projected_query_path
    ...
    def predict(self, batch_size: int = 128, rare_base_threshold=50, rare_detailed_threshold=20):
        ...
        for data in test_loader:
            query = data.to(self.model.device)
        ...
        # Determine potential rare cells using thresholds
        pred_label['is_potential_rare'] = pred_label.groupby('predicted_base_type', group_keys=False).apply(
            self._is_potential_rare, rare_base_threshold=rare_base_threshold, rare_detailed_threshold=rare_detailed_threshold
        )
    ...
    def _is_potential_rare(self, base_type_group, rare_base_threshold=50, rare_detailed_threshold=20):
        # Identify potential rare cells based on user-defined thresholds
