...
class Model:
    ...
    def __format_data(self):
        ...
        for k, v in self.__dict__.items():
            ...
            if(isinstance(v, list) or isinstance(v, dict)):
                data[k] = json.dumps(v)
            if(isinstance(v, EncryptedString)):
                data[k] = v.encrypted_value
            else:
                data[k] = v
        return data
    ...
    def convert_json_dict(self, json_str):
...
class EncryptedString():
    ...
    def __init__(self, encrypted_value=None, decrypted_value=None):
        if encrypted_value is None and decrypted_value is None:
            raise ValueError("You must provide an encrypted or decrypted value")
        if encrypted_value is None:
            self.decrypted_value = decrypted_value
            self.encrypted_value = self.__encrypt()
        if decrypted_value is None:
            self.encrypted_value = encrypted_value
            self.decrypted_value = self.__decrypt()
    ...
    def __decrypt(self):
        ...
        response = kms_client.decrypt(
            CiphertextBlob=bytes(self.encrypted_value, 'utf-8')
        )
    ...
    def __encrypt(self):
        ...
        response = kms_client.encrypt(
            KeyId='alias/aws/ssm',
            Plaintext=bytes(self.decrypted_value, 'utf-8')
        )
