...
from .metrics import confusion_matrix_, test_prediction
...
def run_model(algorithm,
    ...
    trl = pd.DataFrame(train_labels)
    trl.to_excel(f"{p_name}_train_labels.xlsx")
    print('Train labels printed')
    trd = pd.DataFrame(train_data)
    print('Train data printed')
    tel = pd.DataFrame(test_data_labels)
    print('Test data labels printed')
    ted = pd.DataFrame(test_data)
    print('Test data printed')

    trd.to_excel(f"{p_name}_train_data.xlsx")
    tel.to_excel(f"{p_name}_test_labels.xlsx")
    ted.to_excel(f"{p_name}_test_data.xlsx")
    ...
    if algorithm == 'all_ml':
        ...
        combined_df_all = test_prediction(model, labeled_data, labels)
        filename200 = f"{p_name}_data_prediction.xlsx"
        combined_df_all.to_excel(filename200)
        del combined_df_all
