...
def xid(df,
    ...
    result_path = os.path.join(user_documents, "XeroML_results/")
    if not os.path.exists(result_path):
        os.makedirs(result_path)
    f_name = f'{col_label}_{sampling_method}_{param_search}_{model_name}_fit_{control_fitting}'
    ...
    models_needing_standardization = ['ElasticNet', 'LASSO', 'Ridge', 'SVC', 'NuSVC', 'MLPC']
    if normalization == 'auto':
        normalization = 'standardization' if model_name in models_needing_standardization else 'None'
    else:
        normalization = normalization
    data = df.copy()
    labels, labeled_data, unlabeled_data = preprocessing(data=data,
