...
def handle_agent_command(
    ...
    logger.info(
        'Starting agents commands handling thread for pool %s in namespace %s.',
        name,
        namespace,
    )
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        while not stop_event.is_set():
            try:
                command = queue[namespace].get(timeout=1)
                executor.submit(dispatch, *command)
            except Empty:
                continue
            except Exception as err:
                logger.error(
                    'Error in agents commands handling thread for pool %s in namespace %s: %s.',
                    name,
                    namespace,
                    str(err),
                )
    logger.info('Stopping agents commands handling thread in namespace %s.', namespace)


def handle_active_agents(
    stop_event: threading.Event, name: str, namespace: str, spec: Dict[str, Any]
):
    logger = logging.getLogger('handle_active_agents')
    logger.info(
        'Starting active agents handling thread for pool %s in namespace %s.',
        name,
        namespace,
    )
    while not stop_event.is_set():
        try:
            agent, resource_id = ACTIVE_AGENTS[namespace].get(timeout=1)
            ...
            logger.error(
                'Error in active agents handling thread for pool %s in namespace %s: %s',
                name,
                namespace,
                str(err),
            )
    logger.info(
        'Stopping active agents handling thread for pool %s in namespace %s.',
        name,
        namespace,
    )
    ...
    while not stop_event.is_set():
        try:
            ...
            if not kube_agents:
                ...
                continue
            agents = _get_agents(url, headers, logger)
            active_agents = {
            ...
            for agent in active_agents:
                ACTIVE_AGENTS[namespace].put((agent, resource_id))
        except Exception as err:
...
def _start_threads(name: str, namespace: str, spec: Dict[str, Any]):
    ...
    try:
        ...
        ACTIVE_AGENTS.setdefault(namespace, Queue())
