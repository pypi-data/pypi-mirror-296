...
from FlowCyPy.units import (
    ...
    hertz, volt, milliwatt, micrometer, millisecond, meter, second
)
...
ev_ri = distribution.Normal(
    ...
    std_dev=0.03 * refractive_index_unit  # Standard deviation: 0.01
)

ev = Population(
    size=ev_size,                # Particle size distribution
    refractive_index=ev_ri,      # Refractive index distribution
    concentration=1.8e+8 * particle / milliliter,  # Concentration: 1.8e6 particles per milliliter
...
detector_fsc = Detector(
    ...
    numerical_aperture=0.5,          # Numerical aperture: 0.2
    phi_angle=0 * degree,            # Angle: 180 degrees for forward scatter
    sampling_freq=1e7 * hertz,       # Sampling frequency: 1000 Hz
    noise_level=0e-9 * volt,         # Noise level: 1e-9 volts
    saturation_level=1 * microvolt,  # Saturation level: 10 volts
