...
from FlowCyPy.units import nanometer, refractive_index_unit, milliliter, particle, volt, watt, hertz, degree
...
source = Source(
    ...
    optical_power=20e-3         # Optical power: 200 milliwatts
)
...
detector_0 = Detector(
    ...
    responsitivity=1 * volt / watt,     # Responsitivity of the detector (light to signal conversion efficiency)
    sampling_freq=1e4 * hertz,          # Sampling frequency: 10,000 Hz
    noise_level=0e-2 * volt,            # Noise level: 1 millivolt
    baseline_shift=0.01 * volt,         # Baseline shift: 0.01 volts
    saturation_level=20 * volt,         # Saturation level: Large enough to avoid saturation
    n_bins=1024                         # Number of bins for signal discretization: 1024
)

detector_1 = Detector(
    phi_angle=0 * degree,              # Detector angle: 90 degrees (Side Scatter)
    numerical_aperture=0.4,             # Numerical aperture of the detector
