...
def compute_detected_signal(source: Source, detector: Detector, scatterer: Scatterer) -> float:
    ...
    couplings = np.empty_like(scatterer.size_list.magnitude)

    sizes = scatterer.size_list.to(meter).magnitude
    ris = scatterer.refractive_index_list.magnitude

    for index, (size, ri) in enumerate(zip(sizes, ris)):
