...
from FlowCyPy.units import (
    refractive_index_unit, milliliter, particle, nanometer, degree, hertz, volt, watt
)
...
detector_fsc = Detector(
    ...
    phi_angle=180 * degree,          # Angle: 180 degrees for forward scatter
    sampling_freq=1e3 * hertz,       # Sampling frequency: 1000 Hz
    noise_level=1e-9 * volt,         # Noise level: 1e-9 volts
    saturation_level=10 * volt,      # Saturation level: 10 volts
    n_bins=512,                      # Number of discretization bins: 512
    responsitivity=1 * volt / watt   # Responsitivity: 1
)
