...
class Report:
    ...
    def get_flow_cell_section(self):
        ...
        table_data += self.flow.get_properties()
    ...
    def get_scatterer_distribution_section(self):
        """Returns a Table for the scatterer distribution section."""
        style = getSampleStyleSheet()['Heading2']
        pops = self.scatterer_distribution.populations
        data = [
            ['Attribute'] + [p.name for p in pops],
            ['Mean size'] + [f'{p.size_list.mean().to_compact():~.2P}' for p in pops],
            ['STD size'] + [f'{p.size_list.std().to_compact():~.2P}' for p in pops],
            ['Mean RI'] + [f'{p.refractive_index_list.mean().to_compact():~.2P}' for p in pops],
            ['STD RI'] + [f'{p.refractive_index_list.std().to_compact():~.2P}' for p in pops],
            ['N events'] + [p.n_events for p in pops],
        ]

        # Create and return the table
        table = Table(data, hAlign='LEFT', vAlign='TOP')
        table.setStyle(self.get_table_style())

        image = self.get_image_from(self.scatterer_distribution.plot, figure_size=(8, 8), scale=0.5)
    ...
    def get_detector_section(self):
        ...
        style = getSampleStyleSheet()['Heading2']
        detector_0, detector_1 = self.analyzer.detectors
    ...
    def generate_report(self):
        ...
        story = [
            ...
            *self.get_scatterer_distribution_section(), spacer,
