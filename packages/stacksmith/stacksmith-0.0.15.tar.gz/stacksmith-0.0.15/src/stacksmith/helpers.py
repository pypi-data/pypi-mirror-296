...
def update_commit_parent(commit_hash: str, new_parent: str) -> Tuple[str, str, str]:
    return run_command(f"git filter-branch -f --msg-filter 'sed -E \"s/(Created branch .* from ).*/\\1{new_parent}/\"' -- {commit_hash}^..HEAD")
    ...
    while queue:
        ...
        for child_branch in children_branches:
            ...
            if not creation_commit:
                ...
                print(f"Rebased {child_branch} onto {parent_branch}")
