...
def hoist_stack() -> None:
    children_dict, current_branch, root = get_children_dict(), get_current_branch(), f"origin/{get_trunk_name()}"
    ...
    while queue:
        ...
        for child_branch in children_branches:
            ...
            if not creation_commit:
                print(f"Can't hoist {child_branch} since creation commit was not found.")
            else:
