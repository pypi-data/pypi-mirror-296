...
def print_usage():
    ...
    print("\nCustom commands:")
    print("  create <branch_name> [parent_branch]  Create a new branch with an empty initial commit")
    print("  hoist                                 Hoist (rebase) the current branch onto the latest trunk")
    print("  pr [title]                            Create a pull request with automatic parent branch detection")
    print("  propagate [--push]                    Propagate commits from the current branch to all descendant branches")
...
def main():
    ...
    if command == "create":
        if len(args) < 1:
            ...
            sys.exit(1)
        create_branch(args[0], args[1] if len(args) > 1 else None)
    elif command == "hoist":
        hoist_branch()
    elif command == "pr":
        create_pr(args[0] if args else None)
    elif command == "propagate":
        propagate_commits("--push" in args)
