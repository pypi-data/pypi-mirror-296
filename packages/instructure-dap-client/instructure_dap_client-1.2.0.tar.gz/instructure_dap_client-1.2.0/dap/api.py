...
class DAPSession:
    ...
    async def download_resource(
        ...
        async with aiofiles.open(file_path, "wb") as output_file:
            decompressor = zlib.decompressobj(16 + zlib.MAX_WBITS) if decompress else None
            async for stream in self.stream_resource(resource):
                async for chunk in stream.iter_chunked(64 * 1024):
                    await output_file.write(decompressor.decompress(chunk) if decompressor else chunk)
