...
def object_farmhash(obj: BaseModel | typing.Any, exclude_keys: IncEx = None) -> int:
    ...
    def serialize(data: typing.Any) -> typing.Any:
        ...
        try:
            if isinstance(data, BaseModel):
                ...
                o = orjson.dumps(_data)
                return o

            elif isinstance(data, dict):
                o = orjson.dumps(
                    {k: serialize(v) for k, v in sort_object_keys(data).items()}
                )
            elif isinstance(data, list):
                o = orjson.dumps(sorted([serialize(item) for item in data]))
            elif isinstance(data, tuple):
                o = tuple(serialize(item) for item in data)
            elif isinstance(data, set):
                o = frozenset(serialize(item) for item in sorted(data))
            elif isinstance(data, bytes):
                o = farmhash.fingerprint64(z85encode(data).encode("utf-8"))
            else:
                o = farmhash.fingerprint64(str(data))

            return o
        except Exception as e:
            logger.error((e, type(data), type(o), o))
            return id(data)
    ...
    try:
        ...
        logger.error((type(obj), e, traceback, obj))

        return id(obj)
    data_str = str(serialized_data)
    return farmhash.fingerprint64(data_str)
