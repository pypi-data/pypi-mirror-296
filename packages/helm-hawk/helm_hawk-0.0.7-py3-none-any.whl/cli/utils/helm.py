...
class Helm():
    ...
    def helm_template(self,release_name, chart_path,values)->list:
        ...
        try:
            ...
            return helm_template_output.stdout,helm_template_output.returncode
        except subprocess.CalledProcessError as e:
            return e.stderr,1
    ...
    def diff(self,release_name,chart_path,values):
        ...
        try:
            ...
            template,status_code=self.helm_template(release_name,chart_path,values)
            if status_code!=0:
                return template,status_code
            kubectl_diff,status_code= kubectl.diff(template)
            return kubectl_diff,status_code
        except subprocess.CalledProcessError as e:
            return e.stderr,1
