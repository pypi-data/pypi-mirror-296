...
Also include a "reasoning" key in the JSON, with a brief explanation of your choice as the value."""
    ...
    chain = hierarchy_prompt | ChatOllama(
        model=model_name, format="json", temperature=0
    )
    return json.loads(
...
- "reasoning" as key, with a string explaining your decision as value"""
    ...
    chain = relevance_prompt | ChatOllama(
        model=model_name, format="json", temperature=0
    )
...
- "reasoning" as key, with a brief text explaining your reasoning as value"""
    ...
    chain = dynamic_category_prompt | ChatOllama(
        model=model_name, format="json", temperature=0
    )
    return json.loads(chain.invoke({"headline_text": headline_text}).content)
...
def extract_headline_obj_from_html(html_text: str, model_name: str = "mistral-nemo"):
    template = """You are an investment analyst clerck. Your job is to extract the headline, excerpt, and URL from the following HTML text.
...
DO NOT keep the HTML code, keep only the required information."""
    ...
    chain = extraction_prompt | ChatOllama(
