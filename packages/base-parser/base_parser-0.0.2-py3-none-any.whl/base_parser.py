...
class Parser(ABC):
    def __init__(self, cnf: config.Config):
        ...
        self.api_headers = utils.default_headers # headers for receiver API
        self.source_headers = utils.default_headers # headers for source
    ...
    def validate(self):
        pass
        # cnf_json = self.cnf.config
        # if utils.validate_json_structure(json_data, required_structure):
        #     print("JSON is valid.")
        # else:
    ...
    def send_to_receiver(self, vulns_to_send):
        ...
        #     # h.Debug(f"Sent vulns: {json.dumps(jsons_to_send, indent=4, ensure_ascii=False)}")
        # else:
        #     h.Error(f"Vulns were not sent due to error. HTTP code: {r.status_code}. Message: {r.json()}")
