...
fn select_additional_info_data(ip: &str, port: &str,
    ...
    table_name: &str, run_id: &str, path_name:&str) -> PyResult<Vec<(u64, u16, u8, u16, u16, u16, u16, u16, u16, String)>> {
    ...
    let data = match rt.block_on(select_additional_info_data_async(ip, port,user, pw, namespace, db_name, table_name,run_id, path_name)) {
        Ok(data) => data,
        Err(e) => {
...
fn select_measurement_data(ip: &str, port: &str,
    user: &str, pw:&str, namespace: &str, db_name: &str,
    table_name: &str, run_id: &str, path_name: &str) -> PyResult<Vec<(u64, u8, u64, u64, u16, u16, u16, u16, u16, String, String, Vec<String>)>> {
    // Create a Tokio runtime and block on the async function
    let rt = match tokio::runtime::Runtime::new() {
        Ok(rt) => rt,
        Err(e) => {
    ...
    let data = match rt.block_on(select_measurement_data_async(ip, port,user, pw, namespace, db_name, table_name,run_id, path_name)) {
        Ok(data) => data,
        Err(e) => {
...
async fn process_additonal_info_data(result: Response, ip_address: &str, name: &str) -> Result<Vec<(u64, u16, u8, u16, u16, u16, u16, u16, u16, String)>, Box<dyn Error>> {
    let mut data = result;
...
async fn select_additional_info_data_async(
    ...
    let data = process_additonal_info_data(result, &ip,path_name).await?;
    Ok(data)
}
...
async fn select_measurement_data_async(
    ...
    let data = process_measurement_data(result, &ip, path_name).await?;
    Ok(data)
}
...
pub async fn process_measurement_data(result: Response, ip_address: &str, name: &str) -> Result<Vec<(u64, u8, u64, u64, u16, u16, u16, u16, u16, String, String, Vec<String>)>, Box<dyn Error>> {
    let mut data = result;
