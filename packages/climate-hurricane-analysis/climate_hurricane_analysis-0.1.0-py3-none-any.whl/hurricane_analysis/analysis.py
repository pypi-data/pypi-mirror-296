...
def read_evr_data(evr_directory, num_pcs):
    ...
    for file in all_evr_files:
        ...
        evr_matrix.append(evr_values[0:num_pcs])  # Assuming we need the first 3 EVR values
...
def create_yearly_pc_vectors(data_directory, evr_directory, num_pcs, lag_months, target_month):
    # print(f"Looking for SST files in: {data_directory}")
    # print(f"Looking for EVR files in: {evr_directory}")
    ...
    #print("evr_matrix: ", evr_matrix)

    # if not all_files:
    #     print("No SST CSV files found!")
    # else:
    #     print(f"Found SST files: {all_files}")
    ...
    for year in range(1983, 2024):
        ...
        # print(f"\nProcessing Year: {year}, Start Date: {start_date}, End Date: {end_date}")
        yearly_pcs = []
        ...
        for file in all_files:
            ...
            #print(f"Processing file: {file}, Month: {month_number}, EVR: {evr_values}")

            # print(f"\nProcessing year: {year}, Month: {month_number}")
            # print(f"EVR values: {evr_values}, Type: {type(evr_values)}, Shape: {evr_values.shape}")

            monthly_data = read_pca_data(file)
            filtered_data = monthly_data.loc[start_date:end_date]
            # print(f"Filtered data for file {file}: {filtered_data}")
            # if filtered_data.empty:
            #     print(f"No data for the range {start_date} to {end_date} in file {file}")
            # else:
            #     print(f"Found data for the range {start_date} to {end_date} in file {file}. Rows: {len(filtered_data)}")
            ...
            if not filtered_data.empty:
                ...
                # if year == 2010:
                #     print(f"Year: {year}, First 3 PCs before weighting: {first_three_pcs}")
                # print(f"Type of first_three_pcs: {type(first_three_pcs)}, Shape: {first_three_pcs.shape}")

                # Print individual elements before multiplication
                # for i, (pc, evr) in enumerate(zip(first_three_pcs, evr_values)):
                #     print(f"Element {i} of PCs: {pc}, Element {i} of EVR: {evr}")
