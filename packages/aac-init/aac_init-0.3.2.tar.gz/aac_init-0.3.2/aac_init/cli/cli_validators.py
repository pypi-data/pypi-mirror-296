...
logger = setup_logger("cli_validator.log")
...
class CLI_Validator:
    ...
    def __init__(self, data_path: str, output_path: str):
        ...
        logger.debug("CLI Validator initialized successfully!")
    ...
    def _validate_data_path(self):
        ...
        logger.debug(f"Validate YAML directory: {self.data_path} successfully!")
        return True
    ...
    def _validate_yaml(self):
        ...
        logger.debug("Validating input yaml files...")
        ...
        logger.debug(f"Validate YAML files in: {self.data_path} successfully!")
        return True

    def _validate_selections(self, value):
        """Validate CLI selections"""

        logger.debug("Validating cli selections...")
        valid_selections = [
            str(i) for i in range(1, len(settings.DEFAULT_USER_OPTIONS) + 1)
        ]
        ...
        logger.debug(f"Validate cli selections: {self.selections} successfully!")
        return True

    def _validate_global_policy(self):
        """Validate global policy per selection"""

        logger.debug("Validating global policy...")
        ...
        for _, _, schema_files in os.walk(schema_folder):
            for schema in schema_files:
                ...
                try:
                    ...
                    logger.debug(f"Schema {schema} validated successfully.")
                    return True
                except ValueError as e:
                    logger.warning(f"Schema {schema} validated failed!")
                    logger.warning(e)
    ...
    def _validate_fabric_mgmt(self):
        ...
        for item in os.listdir(nac_data_path):
            ...
            if os.path.isfile(item_path) and item in settings.DEFAULT_FABRIC_MGMT_PATH:
                if fabric_mgmt_file:
                    mgs = "Duplicated 01-fabric-mgmt found."
                    logger.error(mgs)
                    self.errors.append(msg)
                    return False
        ...
        if not fabric_mgmt_file:
            mgs = "01-fabric-mgmt is missing!"
            logger.error(mgs)
            self.errors.append(msg)
            return False

        data = yamale.make_data(fabric_mgmt_file)
        schema_folder = os.path.join(settings.SCHEMA_DIR, "01-fabric-mgmt")
        ...
        for _, _, schema_files in os.walk(schema_folder):
            ...
            text = """
            '01-fabric-mgmt' did not meet the requirements of any validation schema.
            This may be due to incorrect configuration in '01-fabric-mgmt',
            or it might be a new configuration scenario that is not supported by the current schema.
            If you confirm that your configuration is correct and represents a new scenario,
            please contact the shlei@cisco.com to update schema.
            """
    ...
    def validate_cli_input(self):
        ...
        logger.debug("Validating cli input data files and selections...")
        if all(
            [
            ...
            logger.debug("cli inputs validated successfully.")
            return True
        else:
            logger.error("cli inputs validated failed!")
