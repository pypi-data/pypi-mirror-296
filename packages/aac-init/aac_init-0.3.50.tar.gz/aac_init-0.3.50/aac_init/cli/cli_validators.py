    ...
    def _validate_global_policy(self):
        ...
        for _, _, schema_files in os.walk(schema_folder):
            for schema_file in schema_files:
                logger.debug(f"Validating schema: {schema_file}")
                schema_file_path = os.path.join(schema_folder, schema_file)
                try:
                    schema = yamale.make_schema(schema_file_path)
                    yamale.validate(schema, data)
                    logger.debug(f"Schema {schema_file} validated successfully.")
                    return True
                except ValueError as e:
                    logger.warning(f"Schema {schema_file} validated failed!")
                    logger.warning(e)
