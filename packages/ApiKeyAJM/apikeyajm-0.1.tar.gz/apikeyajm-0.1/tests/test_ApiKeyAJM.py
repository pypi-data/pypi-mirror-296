...
class TestAPIKey(unittest.TestCase):
    ...
    def test_init_with_api_key(self):
        ...
        api_key = APIKey(api_key=api_key_value, logger=self.mock_logger)

        self.assertEqual(api_key.api_key, api_key_value)
        self.mock_logger.info.assert_called_once_with("APIKey Initialization complete.")

    def test_init_without_api_key(self):
        with open(self.default_key_path, 'w') as file:
            file.write("file_api_key")

        APIKey.DEFAULT_KEY_LOCATION = self.default_key_path

        api_key = APIKey(logger=self.mock_logger)
    ...
    def test_get_api_key(self):
        ...
        api_key_returned = APIKey.get_api_key(api_key=api_key_value, logger=self.mock_logger)

        self.assertEqual(api_key_value, api_key_returned)

    def test_key_file_not_found_error(self):
        with self.assertRaises(FileNotFoundError):
            APIKey(api_key_location="non_existing_file_path", logger=self.mock_logger)
    ...
    def test_io_error(self):
        with patch.object(Path, "is_file", return_value=True), patch('builtins.open', side_effect=IOError(
            ...
            APIKey(api_key_location="error_raising_file_path", logger=self.mock_logger)
