...
from ..gpt_classes import ApiManager
...
def trimData(callData):
    ...
    if callData.get('image') == None and 'image' in callData:
        del callData['image']
    return callData
def get_image_prompt_data(prompt,**kwargs):
    ...
    call_data = {"model": model,"image": image_item,"mask": mask_item,"prompt": prompt,"n": n,"size": size}
    return trimData(call_data)
async def call_image_prompt_async(**kwargs):
    api_mgr = kwargs.get('api_mgr') or ApiManager()
