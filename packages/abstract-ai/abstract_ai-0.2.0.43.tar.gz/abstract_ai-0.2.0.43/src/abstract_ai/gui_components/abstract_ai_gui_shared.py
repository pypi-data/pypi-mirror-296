...
def get_all_models():
    return modelInfoManager().all_model_names
...
def get_settings()->list:
    ...
    def get_model_selection_layout():
        ...
        def get_model_selector()->object:
            return make_component("Frame",'Model',[[make_component("Combo",get_all_models(), default_value=get_all_models()[0], key='-MODEL-', enable_events=True)]])
        def get_token_display()->object:
            return make_component("Frame",'tokens',[[make_component("Input",get_tokens_by_model(get_all_models()[0]),key='-MAX_TOKENS-', readonly=True, enable_events=True,disabled=True,size=(5,1))]])
