...
def get_data_type_range(dtype: np.dtype, verbose=False):
    ...
    if np.isinf(max_value - min_value):
        raise ValueError(f"Type de donnée non supporté car valeures trop grandes pour le type {dtype} min_value = {min_value}, max_value = {max_value}")
...
def calculate_lrsp(image1_path, image2_path, data_range_mode="data_type", verbose=False) -> dict:
    """
