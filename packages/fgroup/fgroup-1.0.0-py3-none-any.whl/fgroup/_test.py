...
if os.name == "nt": # pragma: cover-if-unix
    ...
    def ntify(data: 'T') -> 'T': # pyright: ignore [reportRedeclaration]
        if isinstance(data, str):
            ...
            return tuple(ntify(item) for item in data) # pyright: ignore [reportReturnType]
...
def test_file_tree_helper():
    with file_tree({
        ...
        assert_dir ("folder")
        assert_dir ("folder/subfolder")
        assert_file("folder/subfolder/subfile")
        assert_file("folder/subfolder/subfiles")
        assert_file("folder/subfolder/subfiles 2")
        assert_dir ("folder/sublist")
        assert_file("folder/sublist/a")
        assert_file("folder/sublist/b")
        assert_file("folder/inside file")
        assert_file("file")
        assert_file("file2")
        assert_file("file 3")
...
def test_config_read():
    with file_tree({}):
        with file_config({}) as config:
            ...
            assert_json_equal("out.json", {"unknown": ["."]})
