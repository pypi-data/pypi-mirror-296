...
def main(*sys_argv: str) -> int:
    ...
    parser = argparse.ArgumentParser(
        ...
        description=wrap_full(
            "A helpful utility for organizing and grouping files across many locations.",
            "",
    ...
    parser.add_argument("-d", "--distinct", action="store_true", help=wrap(
        "If set, a parent folder and it's sub-files can be given distinct groups. "
        "Consequently, unmatched files will not be placed in the default group."
    ))
    ...
    parser.add_argument("-m", "--manual", metavar="P:G", nargs="*", help=wrap(
        "File globs (P) executed on the root path. Matching files will be given group (G). "
    ...
    parser.add_argument("-r", "--root", metavar="ROOT", nargs="?", default=False, help=wrap(
        "Changes the root path where files are grouped from. "
    ...
    parser.add_argument("-i", "--indent", metavar="N", nargs="?", type=int, default=False, # Must be False, because using -t without an argument stores None
        help=wrap(f"For formats \"json\" and \"yaml\", indents and nicely formats files. (default: {DEFAULT_INDENT})")
    )
    parser.add_argument("-o", "--override", metavar="G:N", nargs="*", help=wrap(
        "A list of group overrides, separated by space.",
