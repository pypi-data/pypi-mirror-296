...
def run_program():
    ...
    global train_params, train_spec_wl, networks, scaler_spec, scaler_params,
    global samplers, audio_r, note_length
    global c, o, mg, si, s, ca, ti, cr, fe, ni, temp, vel
    global audio_progress, instrument_val, scale_val
    global plot_window, plot_size, image_size
    ...
    ctk.CTkButton(run_frame, text='Run', font=(font, 25),
                  ...
                  border_color="white",
                  corner_radius=5,
                  command=run_sim).grid(column=0, row=0)
    ...
    spec_plot = ctk.CTkImage(light_image=Image.open(config.plot_path),
                              dark_image=Image.open(config.plot_path),
                              size=image_size)

    plot_window = ctk.CTkLabel(output_frame, text="", image=spec_plot)
