...
class BJLEDInstance:
    ...
    async def initialize(self) -> None:
        if not self._mac:
            LOGGER.info("No MAC provided. Searching for LED...")
            self._mac, uuids = await self._scanner.run()
            if not self._mac:
                raise ValueError("LED device not found. Make sure it is powered on and within range.")
            if uuids:
                self._uuid = uuids[0]   # Assuming the first UUID is the one you want to use
                LOGGER.info(f"UUID found: {self._uuid}")
        
        LOGGER.info(f"Initialized LED with MAC: {self._mac} and UUID: {self._uuid}")


    async def _ensure_connected(self) -> None:
        """Ensure the connection to device is established."""
