def _endpoint_warehouse(atconn, warehouse_id: str = None) -> str:
    if warehouse_id:
        suffix = f"/{warehouse_id}"
    else:
        suffix = ""
    return f"{atconn.server}/api/data-warehouses{suffix}"
...
def _endpoint_license_details(atconn):
    """Gets the license for this instance"""
    return f"{atconn.server}/api/license"
