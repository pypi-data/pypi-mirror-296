...
class _Connection:
    ...
    def _validate_license(
        ...
        entitlements = json.loads(base64.b64decode(response.json()["license"]["enc"]).decode()).get(
            "included", []
        )
        ai_link = False
        data_catalog_api = False
        for entitlement in entitlements:
            if specific_feature_flag:
                if entitlement.get("attributes", {}).get("code") == specific_feature_flag:
                    return True
            else:
                if entitlement.get("attributes", {}).get("code") == "FEATURE_AILINK":
                    ai_link = True
                elif entitlement.get("attributes", {}).get("code") == "FEATURE_DATA_CATALOG_API":
                    data_catalog_api = True
        if specific_feature_flag:
            return False
        else:
