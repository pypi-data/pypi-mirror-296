...
class ML:
    def __init__(self):
        ...
        # Normalization values from the training data, in SESAMI_2.0.ipynb
    ...
    def pressure_bin_features(self, data, pressure_bins, isotherm_df):
        ...
        for i, p_bin in enumerate(pressure_bins):
            try:
                val = isotherm_df.loc[
                    (isotherm_df["Pressure"] >= pressure_bins[i][0]) &
                    (isotherm_df["Pressure"] < pressure_bins[i][1]), "Loading"
                ].mean()
                data["c_%d" % i] = val
            except Exception:
    ...
    def add_features(
        self, tr_data, pressure_bins, feature_list, col_list, isotherm_df
    ):
        tr_data = self.pressure_bin_features(tr_data, pressure_bins, isotherm_df)
...
def betml(csv_file):
    ...
    test_data = ml_model.add_features(test_data, pressure_bins, feature_list, col_list, isotherm_df)
  
    empty_bins = [i for i in range(n_bins) if np.isnan(test_data.iloc[0][i + 1])]
