...
def preprocced_image(image, polygon_cords = None, is_show=False, resizeWidth=1024, resizeHeight=1448, cropWidth=1024, cropHeight=362,polygon_cords_check = None,isResize=True):
    ...
    if isResize is not None:
        ...
        resize_objects_img_data = resize_image_data(target_image, resizeWidth, resizeHeight)
        target_image = resize_objects_img_data['resized_image']
        show_img(target_image, 'resize_objects_img',is_show)

        if polygon_cords is not None:
            polygon_cords = resize_image_correct_polygons(resize_objects_img_data, polygon_cords)
            target_image_resized_poly = draw_polygon_on_image(copy.copy(target_image), polygon_cords)
            show_img(target_image_resized_poly, 'target_image_resized_poly',is_show)
        if polygon_cords_check is not None:
            polygon_cords_check = resize_image_correct_polygons(resize_objects_img_data, polygon_cords_check)
            target_image_resized_poly_check = draw_polygon_on_image(copy.copy(target_image), polygon_cords)
...
def resize_image_data(image, target_width, target_height, fill_color=(255, 255, 255)):
    """
...
def resize_image_correct_polygons(resize_info, polygon):
    """
    ...
    :param polygon: List of coordinates in the format [(x1, y1), (x2, y2), (x3, y3), ...].
    ...
    corrected_polygon = []
    for point in polygon:
        if isinstance(point, (list, tuple)) and len(point) == 2:  # Ensure each point is a tuple or list of (x, y)
            x, y = point
            ...
            raise ValueError(f"Invalid point format: {point}. Each point should be a tuple (x, y).")
