...
class LedgerHelper:
    ...
    def read_entry(self, index: tuple = (None, None), column: str = '*', indices: list = None):
        ...
        Handles OR conditions when a comma is present in the index or indices values.
        ...
        def create_or_condition(attr, values):
            or_conditions = [Attr(attr).eq(val.strip()) for val in values.split(',')]
            return or_conditions[0] if len(or_conditions) == 1 else Or(*or_conditions)

        if indices:
            for col, val in indices:
                condition = create_or_condition(col, val) if ',' in val else Attr(col).eq(val)
            ...
            if index[0] and index[1]:
                filter_expression = create_or_condition(index[0], index[1]) if ',' in index[1] else Attr(index[0]).eq(index[1])
