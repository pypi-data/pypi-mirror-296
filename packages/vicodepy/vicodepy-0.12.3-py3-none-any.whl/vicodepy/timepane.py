...
class TimePane(QGraphicsRectItem):
    ...
    def set_cursor_position(self, time, width):
        if isinstance(self.scene.focusItem(), OccurrenceEndHandle):
            shift = -0.5
        else:
            shift = 0.5
        self.time_scale.cursor.setX((time + shift) * width / self.duration)
    ...
    def time(self, time):
        if time != self._time:
            ...
            self.window.video.position = (
                self._time * self.window.frame_duration()
            )
            ...
            if isinstance(self.scene.focusItem(), OccurrenceHandle):
                ...
                self._time = occurrence_handle.change_time(self._time)
        ...
        if not self.occurrence_under_creation:
            for tml in self.timelines():
                for ocr in tml.occurrences():
                    # FIXME : does not acces attribute; use method
                    ocr.pen_width = OCCURRENCE_PEN_WIDTH_OFF_CURSOR
                    if tml.select:
                        if ocr.is_inside(self.time):
        ...
        for timeline in sorted(self.timelines(), key=lambda x: x.name):
            for occurrence in timeline.occurrences():
                ...
                row = [
                    ...
                    f"{fd * occurrence.begin_time:.3f}",
                    f"{fd * (occurrence.end_time + 1):.3f}",
                    comment,
                ]
