...
class MainWindow(QMainWindow):
    ...
    def duration_changed(self, duration):
        ...
        if fd:
            duration = fd * floor(duration / fd)
            self.time_pane.duration = duration + fd
        else:
            self.time_pane.duration = None
    ...
    def move_to_frame(self, nb_frame, checked=False):
        self.time_pane.time = self.time_pane.time + (
            self.frame_duration() * nb_frame
        )

    def disctretize_time(self, time):
        # Truncate time value to a multiple of frame duration
        fd = self.frame_duration()
        return fd * round(time / fd)
