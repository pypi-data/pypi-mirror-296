...
class Occurrence(QGraphicsRectItem):
    ...
    def __init__(
        ...
        self.comment: str = ""
        factor = self.time_pane.scene.width() / self.time_pane.length
        self.set_rect(
            int(self.begin_position * factor),
            int((self.end_position + 1) * factor),
        )
        self.get_bounds()
    ...
    def update_rect(self):
        ...
        factor = new_rect.width() / self.time_pane.length
...
class OccurrenceEndHandle(OccurrenceHandle):
    ...
    def focusInEvent(self, event):
        self.occurrence.time_pane.position = self.occurrence.end_position + 1
        super().focusInEvent(event)
