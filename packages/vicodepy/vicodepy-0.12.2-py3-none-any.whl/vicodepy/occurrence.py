...
class Occurrence(QGraphicsRectItem):
    ...
    def is_inside(self, time):
        hfd = self.frame_duration / 2
        return time + hfd > self.begin_time and time - hfd < self.end_time
...
class OccurrenceEndHandle(OccurrenceHandle):
    ...
    def focusInEvent(self, event):
        self.occurrence.time_pane.time = self.occurrence.end_time
        super().focusInEvent(event)
