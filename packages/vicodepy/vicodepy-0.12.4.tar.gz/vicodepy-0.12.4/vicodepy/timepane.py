...
class TimePane(QGraphicsRectItem):
    ...
    def set_cursor_position(self, position, width):
        if isinstance(self.scene.focusItem(), OccurrenceEndHandle):
            ...
            shift = 0.5
        self.time_scale.cursor.setX((position + shift) * width / self.length)
    ...
    def position(self, position):
        if position != self._position:
            ...
            if self._position > max_length:
                self._position = max_length

            # First, update the occurrence under creation, if it exists. If the
            # cursor position goes beyond the allowed bounds, bring it back and do
            ...
            if ouc:
                ...
                begin_position = ouc.begin_position
                if self._position >= begin_position:
                    ouc.update_end_position(self._position)
                else:
                    ouc.update_end_position(self._position - 1)
