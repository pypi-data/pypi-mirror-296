...
__version__ = "2.2.2"  # Actualizamos la versión por el cambio en la secuencia
...
def fibonacci_list(start, stop, step):
    ...
    # Generamos números de Fibonacci suficientes para cubrir el rango 'max(start, stop)'
    sequence = []
    a, b = 1, 1
    max_limit = max(start, stop)

    while len(sequence) <= max_limit:
        sequence.append(a)
        a, b = b, a + b
    
    # Si el paso es negativo, tomamos los elementos en el rango [stop-1:start] con el paso negativo
    if step < 0:
        return sequence[start:stop:-step][::-1]  # Selecciona e invierte los elementos correctamente
    else:
