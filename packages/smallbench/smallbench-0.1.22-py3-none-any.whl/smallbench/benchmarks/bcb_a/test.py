...
async def score_agent_async(
    ...
    t0 = time.time()
    agent_performance, agent_cost, _ = await agent_benchmark.score_agent_async(agent, split="test", indices=indices, verbose=False)
    t1 = time.time()
    print(f"Time taken: {t1-t0} seconds")
    print(f"Score for {model_name}: " + str(sum(agent_performance)))
    print(f"Cost for {model_name}: " + str(agent_cost))
...
if __name__ == "__main__":
    ...
    asyncio.run(score_agent_async(model_name="gemini-1.5-flash-latest",indices=[i for i in range(0,100)], contexts_for_agent=contexts))
    # bug - fails on sqlite questions when parallelized using docker multithreading
    # flash?
    # deepseek unclear what's going on