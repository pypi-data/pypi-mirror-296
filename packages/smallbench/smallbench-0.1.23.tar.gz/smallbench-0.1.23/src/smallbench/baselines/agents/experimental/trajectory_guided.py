...
class FewshotTrajectoryDemoReActLanguageAgent(Agent):
    ...
    def __init__(self, trajectory_demos: List[str], lm: LLM, contexts: List[Dict], multi_threaded: bool = False):
        self.lm = lm
        ...
        self.trajectory_demos = trajectory_demos
...
## Trajectory Demos
You'll be given a set of trajectory demos, which are examples of past trajectories that the agent took that led to successful outcomes.
```begin_demos
{self.trajectory_demos}
```
