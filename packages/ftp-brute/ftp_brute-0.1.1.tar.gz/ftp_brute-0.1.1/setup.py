...
class CustomInstallCommand(install):
    def run(self):
        match system():
            case 'Windows':
                call(['pyinstaller', '--onefile', 'ftp_brute_force/ftp_brute.py'])
                scripts_dir = join(self.install_scripts, 'ftp_brute.exe')
                rename('dist/ftp_brute.exe', scripts_dir)

            case 'Linux' | 'Darwin':
                call(['pyinstaller', '--onefile', 'ftp_brute_force/ftp_brute.py'])
                scripts_dir = join(self.install_scripts, 'ftp_brute')
