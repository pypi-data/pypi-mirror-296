...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument("--NbBootstraps", type=int,
                        help="Number of bootstraps (only applicable in Mode 2 and Mode 3), default: 1")
    parser.add_argument("--optibyAlpha", type=lambda x: x.lower() == 'true',
                        help="Optimise by alpha - True/False optimize by Error Rate (alpha) also - "
                             "(only applicable in Mode 2 and Mode 3), default: True")
    parser.add_argument("--alphaInit", type=float,
...
def prompt_for_missing_args(args: argparse.Namespace) -> argparse.Namespace:
    if args.mode is None:
        args.mode = int(input("Mode selection (Choose 1, 2, 3, 4, or 5): \n"
                              "1 - Retrieve mutations from each read found in BAM/CRAM files. \n"
                              "2 - Get variants proportions from mode 1 output. \n"
                              "3 - Get variants proportions directly from BAM/CRAM files.\n"
                              "4 - Generate a new M matrix with different lineage choices.\n"
    ...
    if args.mode == 4:
        while not args.full_data:
            args.full_data = input("Enter path or URL to the full data file (default: download from https://raw.githubusercontent.com/hacen-ai/Varaps-data/main/Full_data_latest.zip): ") or "https://raw.githubusercontent.com/hacen-ai/Varaps-data/main/Full_data_latest.zip"
        while not args.tree_file:
            args.tree_file = input("Enter path to the tree file (default: download from https://raw.githubusercontent.com/hacen-ai/Varaps-data/main/Full_data_latest.zip): ") or "https://raw.githubusercontent.com/hacen-ai/Varaps-data/main/Full_data_latest.zip"
        while not args.variant_list:
            args.variant_list = input("Enter path to the variant list file (default: download from https://raw.githubusercontent.com/hacen-ai/Varaps-data/main/Full_data_latest.zip): ") or "https://raw.githubusercontent.com/hacen-ai/Varaps-data/main/Full_data_latest.zip"
    ...
    if args.mode in [2, 3]:
        if args.deconv_method is None:
            args.deconv_method = int(input("Enter deconvolution method: \n"
                                           "1 - Co-occurence based methode\n"
                                           "2 - Count based method\n"
                                           "3 - Frequencies based method\n"
                                           "Deconvolution method (Choose 1, 2 or 3) (default 1): ") or 1)
...
def main():
    ...
    if args.mode == 1:
        ...
        mode2.analyze_file_mode2(
            args.path, args.M, args.output, args.NbBootstraps,
            args.alphaInit, args.optibyAlpha, args.deconv_method
        )
    elif args.mode == 3:
        ...
        mode2.analyze_file_mode2(
            temp_output_dir, args.M, args.output, args.NbBootstraps,
            args.alphaInit, args.optibyAlpha, args.deconv_method
        )
    elif args.mode == 4:
