...
class VariantsProportionFreyja1Sparse:
    def __init__(self, starts_idx, ends_idx, mutation_list, mutationsVariants, tol=1e-3, maxIter=500, alphaInit=0.05, proportionInit=None, readsCount=None):
    ...
    def kern(self, x, params):
        ...
        maf = np.dot(M_, w_)
        # return np.sum(np.square((self.mafH - maf))) #### norm 2 
        return np.sum(np.abs(self.mafH - maf) * self.count_tot) #### norm 2 
