...
def glob(pathstr: str, ext: str = None, recursive: bool = False, sort_method: int = GLOB_SORT_DEFAULT):
    import glob
    ...
    if sort_method == GLOB_SORT_NAT:
        import natsort
        # files = natsort.natsorted(files)
        files = natsort.humansorted(files)
    elif sort_method == GLOB_SORT_SIZE:
...
if __name__ == '__main__':
    ...
    # r = _compare(img1, img2, rmse)
    hash_funcs = [average_hash, colorhash, phash, phash_simple, whash, dhash, dhash_vertical, crop_resistant_hash]
    for hash_func in hash_funcs:
        r = hash_func(to_pil(img1))
        print(r)
