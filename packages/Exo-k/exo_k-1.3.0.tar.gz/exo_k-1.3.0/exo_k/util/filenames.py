...
def parse_filename_HELIOS(filename=None):
    """Parse wn_range, T, P from filename and return it. 
    Example of filename: Out_00000_67000_00500_n566.bin, where wn_range = [0, 67000] cm^{-1}, T=500K, P=1e-566 Bar."""
    split = os.path.basename(os.path.splitext(filename)[0]).split("_")
    prefix, min_wn, max_wn, T, P_str = split
    exponent = int(P_str[1:])/100 # n200 is 10^{-2}
    if P_str[0] == "n":
        P = 10**(-exponent)
    elif P_str[0] == "p":
        P = 10**(exponent)
    # return [int(min_wn), int(max_wn)], 
    P = P * 1e5 # conversion bar to Pascal
    return int(T), P 

def create_fname_tp_grid_Kspectrum_HELIOS(path="."):
    files = glob.glob(path+"/*.bin")
    Ps = []
    Ts = []
    for subfile in files:
        T,P = parse_filename_HELIOS(subfile)
        Ts.append(T)
        Ps.append(P)
    tgrid = np.sort(np.asarray(np.unique(Ts)))
    pgrid = np.sort(np.asarray(np.unique(Ps)))
    file_grid = np.empty((len(pgrid), len(tgrid)), dtype=object)
    for subfile in files:
        T,P = parse_filename_HELIOS(subfile)
        i_P = pgrid.searchsorted(P)
        i_T = tgrid.searchsorted(T)
        file_grid[i_P, i_T] = os.path.basename(subfile)
    return file_grid, pgrid, tgrid 
