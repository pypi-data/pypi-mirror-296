...
from .util.filenames import create_fname_grid_Kspectrum_LMDZ, select_kwargs, create_fname_tp_grid_Kspectrum_HELIOS
    ...
    def __init__(self, *filename_filters, filename=None, xtable=None, path=None,
        ...
        **kwargs):
        ...
        if self.filename is not None:
            ...
            elif self.filename.lower().endswith(('.ncdf', '.nc')):
                self.read_atmo(filename=self.filename, mol=mol, **kwargs)
    ...
    def hires_to_ktable(self, path=None, filename_grid=None,
        ...
        **kwargs):
        ...
        ----------------
            ...
            helios : bool, optional
                Read HELIOS output .bin files.
        ...
        if filename_grid is None:
            if helios:
                filename_grid,self.pgrid,self.tgrid = create_fname_tp_grid_Kspectrum_HELIOS(path)
                # self.pgrid=10**self.logpgrid #in Pa
                self.logpgrid = np.log10(self.pgrid)
                self.Nt = self.tgrid.size
                self.Np = self.pgrid.size
                self.kdata=np.zeros(self.shape) # redefined PT grid
            else:
        ...
        for iP in range(self.Np):
          for iT in range(self.Nt):
            ...
            spec_hr=Hires_spectrum(fname, file_kdata_unit=file_kdata_unit, helios=helios,
