...
class Hires_spectrum(Spectral_object):
    ...
    def __init__(self, filename, file_kdata_unit='unspecified', kdata_unit='unspecified',
        mult_factor=None, binary=False, helios=False, **kwargs):
        ...
        if filename.lower().endswith(('.hdf5', '.h5')):
            ...
            self.read_HELIOS(filename=filename, file_kdata_unit=file_kdata_unit, **kwargs)
            if file_kdata_unit == "unspecified": file_kdata_unit='cm^2/g'
    ...
    def read_turbet(self, filename, data_type='abs_coeff', file_kdata_unit='cm^-1',
            wns=None, binary=False, wns_filename='sigma_grid.bin', 
            skiprows=0, **kwargs):
        ...
        if binary:
            ...
            raw = pd.read_csv(filename, skiprows=skiprows) 
