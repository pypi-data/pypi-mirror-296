...
class Cia_table(Spectral_object):
    ...
    def __init__(self, *filename_filters, filename=None, molecule_pair=None, search_path=None,
            mks=False, remove_zeros=False, old_cia_unit=None, deltalog_min_value=0.,
        ...
        if self.filename is not None:
            if self.filename.lower().endswith(('h5','hdf5')):
                ...
                try:
                    ...
                    self.read_cia_turbet(self.filename, old_cia_unit=old_cia_unit)
            else:
    ...
    def read_hitran_cia(self, filename, old_cia_unit=None):
        ...
        ----------
            ...
            old_cia_unit: str, optional
                Units found in the file. 'cm^5' are assumed if left to None
        """
        tmp_tgrid=[]
        tmp_abs_coeff=[]
        First=True
    ...
    def read_cia_turbet(self, filename, old_cia_unit=None):
        ...
        ----------
            filename: str
                Name of the file to be read.
            old_cia_unit: str, optional
                Units found in the file. 'cm^-1' are assumed if left to None
        """

        #reads temperature array from 1st line of file
        header = open(filename, 'r')
        T_hd=header.readline()
        T_string=T_hd.replace('T=', '')
        T_array=np.fromstring(T_string, dtype=float, sep=' ')
        self.tgrid=T_array
        self.Nt=self.tgrid.size

        #loads abs coeff and wns
        cia_array=np.loadtxt(filename,skiprows=1)
        self.abs_coeff=cia_array[:,1:].T
        self.wns=cia_array[:,0]

        self.wnedges=np.concatenate(([self.wns[0]],0.5*(self.wns[1:]+self.wns[:-1]),[self.wns[-1]]))
        if old_cia_unit is not None:
            self.abs_coeff_unit=old_cia_unit
        else:
            self.abs_coeff_unit='cm^-1'
        self.Nw=self.wns.size

        #reads mol names from file name
        mol=filename.split("/")[-1].split('_')[0]
        molec1,molec2=mol.split("-")
        self.mol1=molec1
        self.mol2=molec2
    ...
    def read_CKD_cia(self, filename, old_cia_unit=None):
        """Reads hitran cia files and load temperature, wavenumber, and absorption coefficient grid.
        ...
        ----------
            filename: str
                Name of the file to be read.
            old_cia_unit: str, optional
                Units found in the file. 'cm^2' are assumed if left to None
        """
