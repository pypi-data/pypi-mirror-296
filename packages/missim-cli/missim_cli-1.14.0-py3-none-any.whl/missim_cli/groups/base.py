...
def build(mode: Mode, clean: bool, services: List[str]):
    ...
    if config.mode == Mode.LOW_FIDELITY:
        docker = DockerClient(
            compose_files=_get_compose_files(
                mode=config.mode, dev_mode=True, network=config.network
            ),
    ...
    docker_ue_editor = DockerClient(
        compose_files=_get_compose_files(Mode.UE_EDITOR, dev_mode=True, network=config.network),
        compose_project_directory=get_project_root(),
    )
    ...
    if config.mode == Mode.UE_STANDALONE:
        ...
        docker_ue_standalone = DockerClient(
            compose_files=_get_compose_files(
                Mode.UE_STANDALONE, dev_mode=True, network=config.network
            ),
            compose_project_directory=get_project_root(),
        )
