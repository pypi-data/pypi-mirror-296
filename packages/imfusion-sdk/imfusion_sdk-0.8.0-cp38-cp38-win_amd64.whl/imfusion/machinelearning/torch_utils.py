...
def to_torch(self: Union[DataElement, imfusion.SharedImageSet, imfusion.SharedImage],
             device: 'torch.device' = None, dtype: 'torch.dtype' = None,
             same_as: 'torch.Tensor' = None) -> 'torch.Tensor':
    """
    ...
    if isinstance(self, (imfusion.SharedImageSet, ml.ImageElement)):
        dimensions = list(range(len(tensor.shape)))
        dimensions.insert(1, dimensions.pop(-1))
        tensor = tensor.permute(dimensions)

    # remove spatial dimension from vector elements
    if isinstance(self, (ml.VectorElement)):
...
def __SIS_from_torch(cls: type, tensor: 'torch.Tensor',
                     get_metadata_from: Optional[imfusion.SharedImageSet] = None) -> imfusion.SharedImageSet:
    """
    ...
    batch dimension and the number of images in the SIS are equal. If :attr:`get_metadata_from` is provided, :attr:`properties`
    will be copied from the SIS and :attr:`matrix`, :attr:`spacing` and :attr:`modality` from the contained SharedImages.
    ...
    # create new instance
    instance = cls(array) if get_metadata_from is None else get_metadata_from.clone(with_data=False)
...
ml.DataElement.torch = to_torch
ml.ImageElement.from_torch = lambda tensor: ml.ImageElement(imfusion.SharedImageSet.from_torch(tensor))
ml.VectorElement.from_torch = lambda tensor: ml.VectorElement(imfusion.SharedImageSet.from_torch(tensor))
