...
def init_logger(log_dir: str, name: str | None = None) -> logging.Logger:
...
class MysqlTableCompareForward:
    ...
    def run(self):
        self.logger = init_logger(self.compare_dir, self.compare_name)
...
def run(args=None):
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=arg_parallel) as executor:
        for src_db, src_tab, dst_db, dst_tab in process_tables:
            compare_name = f"{log_dir}/{src_db}.{src_tab}@{dst_db}.{dst_tab}.forward"

            if os.path.exists(f"{log_dir}/{compare_name}.log.done"):
                compare_success += 1
                continue

            if os.path.exists(f"{log_dir}/{compare_name}.err.log.done"):
                os.remove(f"{log_dir}/{compare_name}.err.log.done")

            mtc = MysqlTableCompareForward(log_dir, compare_name, src_dsn, dst_dsn, src_db, src_tab, dst_db, dst_tab, arg_batch_size, arg_repair, arg_delay_check)
        ...
        for future in concurrent.futures.as_completed(future_to_task):
            ...
            try:
                future.result()
                if os.path.exists(f"{log_dir}/{compare_name}.log"):
                    os.rename(f"{log_dir}/{compare_name}.log", f"{compare_name}.log.done")
            except Exception as e:
                with open(f"{log_dir}/{compare_name}.log", "a+") as f:
                    f.write(str(e))
                # print(f"{get_current_datetime()} {compare_name} generated an exception: {e}")
                if os.path.exists(f"{log_dir}/{compare_name}.log"):
                    os.rename(f"{log_dir}/{compare_name}.log", f"{log_dir}/{compare_name}.err.log.done")
            finally:
