...
class MysqlTableCompare(ABC):
    ...
    def run(self):
        ...
        with connect(**self.source_dsn) as source_con, connect(**self.target_dsn) as target_con:
            ...
            source_table_pkeys, source_table_ukeys = mysql_query_table_keys(source_con, self.src_db, self.src_tab)
            target_table_pkeys, target_table_ukeys = mysql_query_table_keys(target_con, self.dst_db, self.dst_tab)
...
def run(args=None):
    ...
    with connect(**src_dsn) as source_con, connect(**dst_dsn) as target_con:
        for src_db, src_tab, dst_db, dst_tab in process_tables:
            ...
            if arg_mode_reverse:
                ...
                mtcs_reverse.append(
                    MysqlTableCompareReverse(compare_name, src_dsn, dst_dsn, src_db, src_tab, dst_db, dst_tab, arg_batch_size, arg_mode_reverse_clear, arg_delay_check)
                )
            if arg_mode_forward:
                compare_name = f"{arg_work_dir}/{src_db}.{src_tab}__{dst_db}.{dst_tab}.forward"
                mtcs_forward.append(
                    MysqlTableCompareForward(compare_name, src_dsn, dst_dsn, src_db, src_tab, dst_db, dst_tab, arg_batch_size, arg_mode_forward_repair, arg_delay_check)
                )
