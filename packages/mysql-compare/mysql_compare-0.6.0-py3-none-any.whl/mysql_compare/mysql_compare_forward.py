...
def parse_args(external_args=None):
    ...
    parser.add_argument("--dir", type=str, default=datetime.datetime.now().strftime("%Y_%m_%d_%H_%M_%S"), help="Repair the tables")
...
def run(args=None):
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=arg_parallel) as executor:
        for src_db, src_tab, dst_db, dst_tab in process_tables:
            compare_name = f"{arg_dir}/{src_db}.{src_tab}@{dst_db}.{dst_tab}.forward"
    ...
    with open(f"{arg_dir}/summary.json", "w", encoding="utf8") as f:
