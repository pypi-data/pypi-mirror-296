...
def anthropic_messages_to_llm_messages(messages: List[AnthropicMessage]) -> List[Message]:
    messages_to_return = []
...
def anthropic_params_to_llm_params(params: AnthropicParams) -> LLMParams:
    messages = anthropic_messages_to_llm_messages(params.messages)  # They have same structure
    tools = None
    tool_choice = None
    keywordsai_params = {}
    if params.tools:
    ...
    if params.metadata:
        ...
        metadata_in_keywordsai_params = keywordsai_params.pop(
            "metadata", {})  # To avoid conflict of kwargs
