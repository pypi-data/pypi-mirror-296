...
class Protection(APIClientBase):
    ...
    def get_protected_vm(self, vm_syn_id: str):
        if not vm_syn_id:
            raise InvalidInputException(
                f"Expected a valid protected VM id, received: `{vm_syn_id}`"
            )
    ...
    def get_recovery_points(
        self, vm_syn_id: str, limit: int = 100, offset: int = 0, triggers: list = None
    ):
        """
    ...
    def get_recovery_point(self, recovery_point_id: str):
        if not recovery_point_id:
            raise InvalidInputException(
                f"Expected a valid recovery point id, received: `{recovery_point_id}`"
            )
