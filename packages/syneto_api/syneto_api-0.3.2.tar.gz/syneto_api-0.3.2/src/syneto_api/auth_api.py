...
class Authentication(APIClientBase):
    def __init__(self, url_base=None, **kwargs):
        super().__init__(
            url_base or os.environ.get("AUTHORIZATION_SERVICE", ""), **kwargs
        )
    ...
    def login(self, username: str, password: str):
        ...
        return self.post_request(
            "/login", body=body, on_response_callback=self.on_login_response
        )
    ...
    def renew_certificate(
        ...
        return self.post_request(
            relative_url=f"/ca/certs/{name}", body=certificate_data
        )
