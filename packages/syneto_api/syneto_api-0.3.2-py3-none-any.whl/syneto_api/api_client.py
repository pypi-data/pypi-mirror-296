...
def make_url(
    url_base, *path_args, query_args: Union[Dict, List[Tuple]] = None, **path_kwargs
):
...
class APIClientBase:
    ...
    def __init__(
        self, url_base=None, default_protocol="http", insecure_ssl=None, timeout=5 * 60
    ):
    ...
    async def _async_request(
        ...
        try:
            ...
            async for attempt in AsyncRetrying(
                retry=retry_if_not_exception_type(APIException), **self._retry_config
            ):
                with attempt:
            ...
            logger.warning(
                f"Failed {method} request to {url}, network timeout: {str(e)}"
            )
            ...
            async with request_method(url, **method_args) as response:
                ...
                if response.status in success_codes:
                    ...
                    if is_json or self._force_json:
                        ...
                        return (
                            on_response_callback(json_response)
