...
class Storage(APIClientBase):
    ...
    def get_pool(self, pool_name: str, fields: list = None):
        if not pool_name:
            raise InvalidInputException(
                f"Expected a valid pool name, received: `{pool_name}`"
            )
        fields = {"fields": ",".join(fields)} if fields else None
        return self.get_request(
            "/pools/{pool_name}", query_args=fields, pool_name=pool_name
        )
    ...
    def get_volumes(
        self, volume_type: str = None, only_clones: bool = False, label_key: str = ""
    ):
        return self.get_request(
            "/volumes",
            query_args={
    ...
    def get_volume(self, volume_id: str):
        if not volume_id:
            raise InvalidInputException(
                f"Expected a valid volume id, received: `{volume_id}`"
            )
    ...
    def get_volume_meta(self, volume_id: str, key: str, snapshot_name: str = None):
        ...
        return self.get_request(
            "/volumes/{volume_id}/meta", volume_id=volume_id, query_args=query
        )

    def set_volume_meta(
        self, volume_id: str, key: str, value: dict, snapshot_name: str = None
    ):
        if not volume_id:
            raise InvalidInputException(
                f"Expected a valid volume id, received: `{volume_id}`"
    ...
    def get_volume_snapshot(self, volume_id: str, snapshot_name: str):
        ...
        if not snapshot_name:
            raise InvalidInputException(
                f"Expected a valid snapshot name, received: `{snapshot_name}`"
            )
        return self.get_request(
    ...
    def get_volume_files(
        ...
        return self.get_request(
            "/volumes/{volume_id}/files", volume_id=volume_id, query_args=query
        )
