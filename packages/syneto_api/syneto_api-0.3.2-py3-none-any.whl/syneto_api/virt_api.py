...
class Virtualization(APIClientBase):
    ...
    def get_hypervisor(self, hypervisor_id: str):
        if not hypervisor_id:
            raise InvalidInputException(
                f"Expected a valid hypervisor id, received: `{hypervisor_id}`"
            )
    ...
    def get_virtual_machine(self, vm_syn_id: str):
        if not vm_syn_id:
            raise InvalidInputException(
                f"Expected a valid virtual machine id, received: `{vm_syn_id}`"
            )
    ...
    def get_vm_snapshot(self, vm_syn_id: str, snapshot_name: str):
        ...
        snapshots = self.get_request(
            "/vmware/vms/{vm_syn_id}/snapshots", vm_syn_id=vm_syn_id
        )
    ...
    def delete_vm_snapshot(
        self, vm_syn_id, snapshot_ref_id, consolidate=True, remove_children=True
    ):
        if not vm_syn_id:
            raise InvalidInputException(
                f"Expected a valid virtual machine id, received: `{vm_syn_id}`"
    ...
    def create_nas_datastore(
        self, server: str, mountpoint: str, hosts_syn_ids=None, datastore_name=None
    ):
        return self.post_request(
            "/vmware/datastores",
            body={
    ...
    def get_vm_snapshot_by_ref(self, vm_syn_id: str, snapshot_ref: str):
        ...
        if not snapshot_ref:
            raise InvalidInputException(
                f"Expected a valid VM snapshot ref, received: `{snapshot_ref}`"
            )
        return self.get_request(
