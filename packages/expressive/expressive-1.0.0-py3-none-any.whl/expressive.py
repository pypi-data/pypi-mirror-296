...
def signature_from_data(data, dtype_result):
    mapper = []
    for name, column in data.items():
        # mapper.append(f"{column.dtype}[:] {name}")
        mapper.append(f"{column.dtype}[:]")

    # create function signature
    return f"""{dtype_result}[:]({", ".join(mapper)})"""
...
class Expressive:
    ...
    def build(self, data, *, dtype_result=None):  # arch target?
        ...
        signature = signature_from_data(data, dtype_result)
    ...
    def __call__(self, data, dtype_result=None):
        ...
        return self.signatures_mapper[signature](**data)
