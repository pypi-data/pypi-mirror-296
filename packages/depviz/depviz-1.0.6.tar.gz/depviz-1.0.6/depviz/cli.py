...
@click.option(
    "--parser",
    default="bazel",
    type=click.Choice(["bazel", "requirements"]),
...
@click.option(
    ...
    help="Pattern for requirements files (only for the requirements parser)",
)
@click.option(
...
def main(
    ...
    graph = DependencyGraph(
        ...
        requirements_pattern=(
            requirements_pattern if parser == "requirements" else None
        ),  # Only pass pattern if using requirements parser
