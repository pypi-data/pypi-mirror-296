...
import pickle
from pycamps.simulation import NumericalIntegration
...
class Dynamics:
    ...
    Methods:
        get_required_parameters() -> dict:
            Returns a dictionary of required parameters for each module.

        load_new_params(params_directory: Optional[str] = None, custom_module_files: Optional[dict] = None, params_dictionary: Optional[dict] = None, reference_frame: Optional[dict] = None):
            Loads new parameters from a directory, custom mapping, or a dictionary.
            
            Args:
                params_directory (Optional[str]): The directory containing parameter files.
                custom_module_files (Optional[dict]): A dictionary mapping module names to custom parameter files.
                params_dictionary (Optional[dict]): A dictionary containing parameters.
                reference_frame (Optional[dict]): A dictionary specifying the reference frame.

        save_dynamics(file_path: Optional[str] = None):
        ...
        solve_equilibrium(x0: Optional[np.ndarray] = Random, options: Optional[dict] = None, method: str = 'fsolve') -> np.ndarray:
            Solves for the equilibrium values of the system.
            ...
            Args:
                ...
                method (str): The method to use for solving the equilibrium. 
                    You can choose from 'fsolve', 'hybr', 'lm', 'broyden1', 'broyden2', 'anderson', 'linearmixing', 'diagbroyden', 'excitingmixing', 'krylov', 'df-sane'.

        simulate_trajectory(xf: Optional[np.ndarray] = Random, simulation_time: float = 0.1, method: str = 'trapezoid_adaptive') -> tuple:
            Simulates the trajectory of the system.
                   
            Args:
                xf (Optional[np.ndarray]): Final state values.
                simulation_time (float): The duration of the simulation.
                method (str): The method to use for simulating the trajectory.
                    You can choose from 'trapezoid_adaptive', 'LSODA', 'RK45', 'RK23', 'DOP853', 'Radau', 'BDF'.

        linearized_analysis(xf: Optional[np.ndarray] = None) -> tuple:
            Performs a linearized analysis of the system.
            
            Args:
                xf (Optional[np.ndarray]): Final state values for the linearized analysis.
    ...
    def load_new_params(self, params_directory=None, custom_module_files=None, params_dictionary=None, reference_frame={sp.Symbol('dphidt', real=True): 1, sp.Symbol('phi', real=True): 0}):
        '''
        There are three supported options to load in parameters:
        1. Load in parameters from a single directory, where the file name is 'module_name.mat' or 'module_name.json'
            and the parameters are the variables in the file.
        2. Load in a custom mapping of each module_name to the full file path containing that module's parameters.
        2 replacement: Load in a single file containing all parameters.
