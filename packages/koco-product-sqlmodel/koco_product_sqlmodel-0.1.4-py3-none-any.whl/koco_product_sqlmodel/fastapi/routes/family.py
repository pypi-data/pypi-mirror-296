...
router = APIRouter(dependencies=[Depends(sec.get_current_active_user)],tags=["Endpoints to FAMILY-data"])
# router = APIRouter()
...
def create_family(fam: sqlm.CFamilyPost) -> mdb_fam.CFamilyGet:
    new_fam = mdb_fam.create_family_DB(
        family=sqlm.CFamily(**fam.model_dump())
    )
    return mdb_fam.CFamilyGet(**new_fam.model_dump())

@router.patch("/{id}/", dependencies=[Depends(sec.has_post_rights),])
def update_family(id: int, fam: sqlm.CFamilyPost) -> mdb_fam.CFamilyGet:
