...
CMD_TEMPLATES = {'hython': "{hserver}hython \"{script}\" -f {first} {last} {step} -d {driver} \"{hipfile}\"",
                 'husk':   "husk --make-output-path -no-mplay --threads 0 --verbose 9aC --fast-exit 0 -s {render_settings} -R {hydra_delegate} --frame {first} --frame-count {count} --frame-inc {step} {usd_file}"
                }
