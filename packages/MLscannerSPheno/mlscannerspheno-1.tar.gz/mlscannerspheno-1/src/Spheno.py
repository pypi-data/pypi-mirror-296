...
class scan():
    import sklearn
    def __init__(self,iteration,L1,L,K, th_value,period,frac,K_smote):
...
def RFC(iteration=10,L1=100,L=1000,K=100,period=1,frac=0.2,K_smote=1,learning_rate=0.01,n_estimators=300,max_depth=50,print_output=True):
  ...
  ''' 
  model = scan(iteration,L1,L,K, th_value,period,frac,K_smote)  
  model.run_RFC(learning_rate=learning_rate,n_estimators=n_estimators,max_depth=n_estimators,print_output=n_estimators)
  return
