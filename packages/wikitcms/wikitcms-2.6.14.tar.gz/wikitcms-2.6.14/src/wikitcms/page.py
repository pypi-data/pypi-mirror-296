...
class AMIPage(Page):
    ...
    def seedtext(self):
        ...
        start = datetime.datetime.strptime(ciddic["date"], "%Y%m%d").replace(tzinfo=pytz.utc)
        end = start + datetime.timedelta(days=2)
        topics = ["aws.Cloud_Base"]
        topics.extend([f"aws.Cloud_Base.{ciddic['version']}.{arch}" for arch in arches])
