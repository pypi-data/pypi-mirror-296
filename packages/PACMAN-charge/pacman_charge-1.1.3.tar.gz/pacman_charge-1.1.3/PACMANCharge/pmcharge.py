...
def predict(cif_file,charge_type="DDEC6",digits=6,atom_type=True,neutral=True):
    ...
    try:
        ...
        gcn.to(device)
        chkpt_ddec = torch.load(charge_model_name, map_location=torch.device(device), weights_only=True)
...
def Energy(cif_file):
    ...
    try:
        ...
        checkpoint = torch.load(pbe_model_name, map_location=torch.device(device), weights_only=True)
