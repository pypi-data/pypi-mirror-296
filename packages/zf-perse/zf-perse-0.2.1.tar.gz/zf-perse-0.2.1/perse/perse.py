...
def simmer(content: Union[str, BeautifulSoup], exclude_tags: Set[str] = None) -> BeautifulSoup:
    """
...
def simmers(content: Union[str, BeautifulSoup], exclude_tags: Set[str] = None) -> str:
    """
    Removes unnecessary tags and attributes from the HTML content.
    ...
    """
    soup = simmer(content, exclude_tags)
    if not soup:
        raise PerseError("HTML soup is empty")
...
def perse(content: str, exclude_tags: Set[str] = None) -> Dict[str, Any]:
    ...
    cleaned_content = simmers(content=content, exclude_tags=exclude_tags)
