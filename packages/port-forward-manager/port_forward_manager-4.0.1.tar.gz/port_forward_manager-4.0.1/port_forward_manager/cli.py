...
def yaml_import(
    ...
    force: bool = typer.Option(False, help="Wipe DB"),
):
    """Import groups, schemas and sessions from configuration file"""
    ...
    for schema_definition in settings.get('schemas', []):
        ...
        if not group:
            rich.print(f"IGNORING schema - Error could not find group {tmp_group.get('label')}")
            continue

        schemas[schema_definition.id] = schema_definition.label

        schema = models.Schema.find_by_label(schema_definition.label)
        if not schema:
            change_count += 1
