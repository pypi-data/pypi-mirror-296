...
def init_database():
    ...
    group = Group.find_by_label("port_forward_manager-system")
    if not group:
        print(f"Creating system group 'port_forward_manager-system'")
        group = Group(name="port_forward_manager-system", label="System", visible=False)
        schema = Schema(name="port_forward_manager-ephemeral", label="Ephemeral sessions")
