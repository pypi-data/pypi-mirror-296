...
from chrori.utils import prepare_cmd, time_stamp
from chrori.refindex import RefIndex
from chrori.haplocall import HaploCall
from chrori.mergevcf import MergeVcf
...
class MKVcf(object):
    ...
    def __init__(self, args):
        ...
        elf.stem = args.output
        self.dir = '{}_mkvcf'.format(self.stem)
        self.middir = '{}_mkvcf/intermediate'.format(self.stem)s
    ...
    def mvoutputfiles(self):
        cmd1 = 'cp {}/vcf_2nd/Merged_filtered_variants.vcf {}/{}_ready_for_chrori.vcf'.format(self.middir, self.dir, self.stem)
        cmd1 = prepare_cmd(cmd1)
        try:
            sbp.run(cmd1, stdout=sbp.DEVNULL, stderr=sbp.DEVNULL,
                    shell=True, check=True)
...
def main():
    print(time_stamp(), 'MKVcf for chrori started.', flush=True)
    ...
    print(time_stamp(), 'MKVcf for chrori successfully finished.\n', flush=True)
